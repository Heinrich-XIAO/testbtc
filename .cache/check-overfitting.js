...
// src/strategies/strat_sr_no_trend_filter_302.ts
...
`);
  ...
  for (const { name, file } of STRATEGIES) {
    const paramsPath = path2.join(process.cwd(), "src/strategies", file);
    if (!fs2.existsSync(paramsPath)) {
      console.log(`| ${name} | FILE NOT FOUND | | | | | | | | | |`);
      continue;
    }
    const paramsContent = fs2.readFileSync(paramsPath, "utf-8");

...
import { SRNoTrendCombo2369Strategy } from '../src/strategies/strat_sr_no_trend_combo2_369';
...
async function main() {
  ...
  const newStrategy = new SRNoTrendCombo2369Strategy({});
  const engine = new BacktestEngine(data, newStrategy, { initialCapital: 1000, feeRate: 0 });
  ...
  const row = '369 (Combo2)'.padEnd(30) +

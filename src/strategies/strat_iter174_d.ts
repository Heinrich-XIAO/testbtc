...
export interface StratIter174DParams extends StrategyParams { sr_lookback: number; entry_threshold: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; }
export class StratIter174DStrategy extends BaseIterStrategy<StratIter174DParams> {
  constructor(params: Partial<StratIter174DParams> = {}) { super('strat_iter174_d.params.json', { sr_lookback: 30, entry_threshold: 0.02, stop_loss: 0.08, profit_target: 0.15, max_hold_bars: 24, risk_percent: 0.25 }, params); }

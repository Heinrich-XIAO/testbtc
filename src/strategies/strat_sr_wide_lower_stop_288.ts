...
/**
 * SR Wide Lower Stop 288
 * - Wide stochastic bands + lower stop loss (0.05)
 * - Cut losses faster
 */

export interface SRWideLowerStop288Params extends StrategyParams {
...
const defaultParams: SRWideLowerStop288Params = {
  base_lookback: 18,
  min_lookback: 8,
  max_lookback: 36,
  ...
  min_bounce_bars: 2,
  stop_loss: 0.05, // Lower stop loss (was 0.075)
  trailing_stop: 0.04, // Tighter trailing too
  profit_target: 0.15,
  max_hold_bars: 35,
  risk_percent: 0.22,
};

function loadSavedParams(): Partial<SRWideLowerStop288Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_wide_lower_stop_288.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRWideLowerStop288Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRWideLowerStop288Params] = value;
        }
      }
    }
...
export class SRWideLowerStop288Strategy implements Strategy {
  params: SRWideLowerStop288Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRWideLowerStop288Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRWideLowerStop288Params;
  }

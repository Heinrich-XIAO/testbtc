...
export interface ComboV13StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV13StrategyParams = {
  bb_period: 7,
  rsi_period: 5,
  std_mult: 1.8,
  rsi_oversold: 28,
...
function loadSavedParams(): Partial<ComboV13StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v13_146.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV13StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV13StrategyParams] = value;
        }
      }
    }
...
export class ComboV13Strategy implements Strategy {
  params: ComboV13StrategyParams;
  ...
  constructor(params: Partial<ComboV13StrategyParams> = {}) {

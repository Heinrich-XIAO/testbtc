...
/**
 * EMA cross with very tight trailing stop
 */

export interface EMATightStopStrategyParams extends StrategyParams {
  fast_period: number;
  slow_period: number;
  stop_loss: number;
...
const defaultParams: EMATightStopStrategyParams = {
  fast_period: 4,
  slow_period: 10,
  stop_loss: 0.03,
  trailing_stop: 0.02,
...
function loadSavedParams(): Partial<EMATightStopStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_ema_tight_14.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EMATightStopStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EMATightStopStrategyParams] = value;
        }
      }
    }
...
export class EMATightStopStrategy implements Strategy {
  params: EMATightStopStrategyParams;
  ...
  constructor(params: Partial<EMATightStopStrategyParams> = {}) {

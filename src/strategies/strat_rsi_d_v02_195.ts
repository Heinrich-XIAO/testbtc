...
export interface RsiDV02StrategyParams extends StrategyParams {
  rsi_period: number;
...
const defaultParams: RsiDV02StrategyParams = {
  rsi_period: 4,
...
function loadSavedParams(): Partial<RsiDV02StrategyParams> | null {
  ...
  try {
    ...
    const params: Partial<RsiDV02StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof RsiDV02StrategyParams] = value;
        }
      }
    }
...
export class RsiDV02Strategy implements Strategy {
  params: RsiDV02StrategyParams;
  private rsiMap: Map<string, RSI> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private rsiHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<RsiDV02StrategyParams> = {}) {

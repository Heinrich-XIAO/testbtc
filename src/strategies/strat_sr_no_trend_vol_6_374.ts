...
/**
 * SR No Trend Vol 6 - 374
 * - Based on 302
 * - Lower volatility period (6 instead of 12) for more adaptive lookback
 */

export interface SRNoTrendVol6374Params extends StrategyParams {
...
const defaultParams: SRNoTrendVol6374Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
  volatility_period: 6,
...
function loadSavedParams(): Partial<SRNoTrendVol6374Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_vol_6_374.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendVol6374Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendVol6374Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendVol6374Strategy implements Strategy {
  params: SRNoTrendVol6374Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendVol6374Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendVol6374Params;
  }

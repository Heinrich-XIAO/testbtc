...
export interface EnvV11StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV11StrategyParams = {
  ma_period: 4,
  envelope_pct: 0.015,
  stop_loss: 0.06,
  risk_percent: 0.25,
...
function loadSavedParams(): Partial<EnvV11StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_v11_104.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvV11StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV11StrategyParams] = value;
        }
      }
    }
...
export class EnvV11Strategy implements Strategy {
  params: EnvV11StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV11StrategyParams> = {}) {

...
export interface ChanV09StrategyParams extends StrategyParams {
...
const defaultParams: ChanV09StrategyParams = {
  channel_period: 12,
  channel_width: 0.6,
  stop_loss: 0.07,
  trailing_stop: 0.05,
...
function loadSavedParams(): Partial<ChanV09StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_v09_122.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanV09StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanV09StrategyParams] = value;
        }
      }
    }
...
export class ChanV09Strategy implements Strategy {
  params: ChanV09StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanV09StrategyParams> = {}) {

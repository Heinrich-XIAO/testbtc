...
export interface MRRsiV13StrategyParams extends StrategyParams {
  ma_period: number;
  rsi_period: number;
...
const defaultParams: MRRsiV13StrategyParams = {
  ma_period: 6,
  rsi_period: 5,
...
function loadSavedParams(): Partial<MRRsiV13StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_mr_rsi_v13_66.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<MRRsiV13StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof MRRsiV13StrategyParams] = value;
        }
      }
    }
...
export class MRRsiV13Strategy implements Strategy {
  params: MRRsiV13StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private rsiMap: Map<string, RSI> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<MRRsiV13StrategyParams> = {}) {

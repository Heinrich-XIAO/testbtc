...
export interface SRAdaptive225Params extends StrategyParams {
...
const defaultParams: SRAdaptive225Params = {
  base_lookback: 15,
  min_lookback: 8,
  max_lookback: 30,
...
function loadSavedParams(): Partial<SRAdaptive225Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_adaptive_225.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRAdaptive225Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRAdaptive225Params] = value;
        }
      }
    }
...
export class SRAdaptive225Strategy implements Strategy {
  params: SRAdaptive225Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRAdaptive225Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRAdaptive225Params;
  }
  ...
  private getAdaptiveLookback(data: TokenData): number {
    ...
    // Higher volatility = shorter lookback (more reactive)

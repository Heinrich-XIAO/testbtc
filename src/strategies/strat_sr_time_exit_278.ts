...
/**
 * SR Time Exit 278
 * - Forced exit at specific bar count
 * - No profit target - just time-based exit with stop loss
 */

export interface SRTimeExit278Params extends StrategyParams {
  ...
  exit_bar: number; // Exit at exactly this bar count
  risk_percent: number;
}

const defaultParams: SRTimeExit278Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
  ...
  stop_loss: 0.075,
  trailing_stop: 0.055,
  exit_bar: 28, // Exit after 28 bars regardless
  risk_percent: 0.20,
};

function loadSavedParams(): Partial<SRTimeExit278Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_time_exit_278.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRTimeExit278Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRTimeExit278Params] = value;
        }
      }
    }
...
export class SRTimeExit278Strategy implements Strategy {
  params: SRTimeExit278Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRTimeExit278Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRTimeExit278Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        // Time-based exit - exit at exactly this bar
        if (bars >= this.params.exit_bar) {
          this.closePosition(ctx, bar.tokenId);
          return;
        }
      }

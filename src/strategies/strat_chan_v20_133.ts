...
export interface ChanV20StrategyParams extends StrategyParams {
...
const defaultParams: ChanV20StrategyParams = {
  channel_period: 18,
  channel_width: 1,
  stop_loss: 0.1,
  trailing_stop: 0.07,
...
function loadSavedParams(): Partial<ChanV20StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_v20_133.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanV20StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanV20StrategyParams] = value;
        }
      }
    }
...
export class ChanV20Strategy implements Strategy {
  params: ChanV20StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanV20StrategyParams> = {}) {

...
export interface WillRV11StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV11StrategyParams = {
  period: 4,
  oversold_level: -75,
  overbought_level: -25,
  stop_loss: 0.06,
...
function loadSavedParams(): Partial<WillRV11StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v11_84.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV11StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV11StrategyParams] = value;
        }
      }
    }
...
export class WillRV11Strategy implements Strategy {
  params: WillRV11StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV11StrategyParams> = {}) {

...
export interface MRRsiV20StrategyParams extends StrategyParams {
  ma_period: number;
  rsi_period: number;
...
const defaultParams: MRRsiV20StrategyParams = {
  ma_period: 14,
  rsi_period: 10,
...
function loadSavedParams(): Partial<MRRsiV20StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_mr_rsi_v20_73.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<MRRsiV20StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof MRRsiV20StrategyParams] = value;
        }
      }
    }
...
export class MRRsiV20Strategy implements Strategy {
  params: MRRsiV20StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private rsiMap: Map<string, RSI> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<MRRsiV20StrategyParams> = {}) {

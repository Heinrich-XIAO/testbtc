...
export interface StratIter10CParams extends StrategyParams {
...
const defaultParams: StratIter10CParams = {
  stoch_k_period: 14,
  stoch_oversold: 16,
  stoch_overbought: 86,
  stop_loss: 0.08,
...
function loadSavedParams(): Partial<StratIter10CParams> | null {
  const paramsPath = path.join(__dirname, 'strat_iter10_c.params.json');
...
export class StratIter10CStrategy implements Strategy {
  params: StratIter10CParams;
  ...
  constructor(params: Partial<StratIter10CParams> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as StratIter10CParams;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    } else if (bar.close > 0 && bar.close < 1) {
      if (kVals.length >= 2) {

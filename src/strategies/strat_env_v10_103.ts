...
export interface EnvV10StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV10StrategyParams = {
  ma_period: 12,
  envelope_pct: 0.06,
  stop_loss: 0.1,
  risk_percent: 0.2,
...
function loadSavedParams(): Partial<EnvV10StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_v10_103.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvV10StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV10StrategyParams] = value;
        }
      }
    }
...
export class EnvV10Strategy implements Strategy {
  params: EnvV10StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV10StrategyParams> = {}) {

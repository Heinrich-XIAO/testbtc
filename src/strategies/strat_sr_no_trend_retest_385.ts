...
/**
 * SR No Trend Retest 385
 * - Based on 362 but requires double bottom / support retest
 * - Requires a second touch of support before entry
 */

export interface SRNoTrendRetest385Params extends StrategyParams {
  ...
  risk_percent: number;
  retest_bars: number;
  retest_threshold: number;
}

const defaultParams: SRNoTrendRetest385Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
...
function loadSavedParams(): Partial<SRNoTrendRetest385Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_retest_385.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendRetest385Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendRetest385Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
interface TokenData {
  ...
  lastSupportTest: { price: number; bar: number } | null;
  currentBar: number;
}

export class SRNoTrendRetest385Strategy implements Strategy {
  params: SRNoTrendRetest385Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendRetest385Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendRetest385Params;
  }
  ...
  private closePosition(ctx: BacktestContext, tokenId: string, data: TokenData, supports: number[]): void {
    ctx.close(tokenId);
    if (supports.length > 0) {
      const nearestSupport = supports.reduce((prev, curr) => 
        Math.abs(curr - data.prices[data.prices.length - 1]) < Math.abs(prev - data.prices[data.prices.length - 1]) ? curr : prev
      );
      data.lastSupportTest = { price: nearestSupport, bar: data.currentBar };
    }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        if (bar.close < entry * (1 - this.params.stop_loss)) { this.closePosition(ctx, bar.tokenId, data, supports); return; }
        if (bar.close < newHighest * (1 - this.params.trailing_stop)) { this.closePosition(ctx, bar.tokenId, data, supports); return; }
        if (bar.close >= entry * (1 + this.params.profit_target)) { this.closePosition(ctx, bar.tokenId, data, supports); return; }
        if (bars >= this.params.max_hold_bars) { this.closePosition(ctx, bar.tokenId, data, supports); return; }
        if ((resistance !== null && bar.close >= resistance) || stoch.k >= this.params.stoch_overbought) {
          this.closePosition(ctx, bar.tokenId, data, supports);
        }
      }
      ...
      const hasValidRetest = this.checkRetest(data, supports, bar.close);

      if (nearSupport && !hasValidRetest && supports.length > 0) {
        const nearestSupport = supports.reduce((prev, curr) => 
          Math.abs(curr - bar.close) < Math.abs(prev - bar.close) ? curr : prev
        );
        data.lastSupportTest = { price: nearestSupport, bar: data.currentBar };
      }

      if (nearSupport && multiBarBounce && stochOversold && momentumOk && hasValidRetest) {
  ...
  private checkRetest(data: TokenData, supports: number[], currentPrice: number): boolean {
    if (!data.lastSupportTest) return false;
    
    const barsSinceTest = data.currentBar - data.lastSupportTest.bar;
    if (barsSinceTest > this.params.retest_bars) return false;

    const nearPreviousSupport = supports.some(s => 
      Math.abs(s - data.lastSupportTest!.price) / data.lastSupportTest!.price < this.params.retest_threshold
    );
    
    return nearPreviousSupport;
  }

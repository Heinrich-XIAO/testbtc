...
/**
 * Wide channel mean reversion
 */

export interface ChanWideStrategyParams extends StrategyParams {
...
const defaultParams: ChanWideStrategyParams = {
  channel_period: 15,
  channel_width: 0.7,
  stop_loss: 0.08,
  trailing_stop: 0.06,
...
function loadSavedParams(): Partial<ChanWideStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_wide_51.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanWideStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanWideStrategyParams] = value;
        }
      }
    }
...
export class ChanWideStrategy implements Strategy {
  params: ChanWideStrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanWideStrategyParams> = {}) {

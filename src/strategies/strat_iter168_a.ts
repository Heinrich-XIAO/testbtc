...
export interface StratIter168AParams extends StrategyParams {
  rsi_period: number;
  rsi_oversold: number;
  rsi_recover: number;
  stop_loss: number;
...
export class StratIter168AStrategy implements Strategy {
  params: StratIter168AParams;
  private series: Map<string, TokenSeries> = new Map();
  private bars: Map<string, number> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private entryBar: Map<string, number> = new Map();

  constructor(params: Partial<StratIter168AParams> = {}) {
    const saved = loadSavedParams<StratIter168AParams>('strat_iter168_a.params.json');
    this.params = {
      rsi_period: 14,
      rsi_oversold: 30,
      rsi_recover: 40,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    // Entry: RSI was below oversold, now recovered above rsi_recover

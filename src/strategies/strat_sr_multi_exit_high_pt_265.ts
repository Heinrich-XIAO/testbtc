...
/**
 * SR Multi Exit High PT 265
 * - Tweak of sr_strict_multi_exit_260 ($4,414 best)
 * - Higher profit target for bigger wins
 */

export interface SRMultiExitHighPT265Params extends StrategyParams {
...
const defaultParams: SRMultiExitHighPT265Params = {
  base_lookback: 16,
  min_lookback: 6,
  max_lookback: 35,
  volatility_period: 8,
...
function loadSavedParams(): Partial<SRMultiExitHighPT265Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_multi_exit_high_pt_265.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRMultiExitHighPT265Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRMultiExitHighPT265Params] = value;
        }
      }
    }
...
export class SRMultiExitHighPT265Strategy implements Strategy {
  params: SRMultiExitHighPT265Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRMultiExitHighPT265Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRMultiExitHighPT265Params;
  }

...
export interface SRRsiExit272Params extends StrategyParams {
...
const defaultParams: SRRsiExit272Params = {
  base_lookback: 16,
  min_lookback: 6,
  max_lookback: 35,
  volatility_period: 8,
...
function loadSavedParams(): Partial<SRRsiExit272Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_rsi_exit_272.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRRsiExit272Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRRsiExit272Params] = value;
        }
      }
    }
...
export class SRRsiExit272Strategy implements Strategy {
  params: SRRsiExit272Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRRsiExit272Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRRsiExit272Params;
  }

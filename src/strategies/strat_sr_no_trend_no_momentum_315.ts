...
/**
 * SR No Trend No Momentum 315
 * - Based on 302 but REMOVES momentum requirement
 * - Tests if momentum filter was blocking good entries
 * - When price is at support and stochastic is oversold,
 *   momentum is naturally low but about to reverse
 */

export interface SRNoTrendNoMomentum315Params extends StrategyParams {
...
const defaultParams: SRNoTrendNoMomentum315Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendNoMomentum315Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_no_momentum_315.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendNoMomentum315Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendNoMomentum315Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendNoMomentum315Strategy implements Strategy {
  params: SRNoTrendNoMomentum315Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendNoMomentum315Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendNoMomentum315Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (nearSupport && multiBarBounce && stochOversold) {

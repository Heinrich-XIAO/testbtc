...
/**
 * SR Weighted Trend 263
 * - Weighted combination of trend + momentum
 * - Score must exceed threshold for entry
 */

export interface SRWeightedTrend263Params extends StrategyParams {
...
const defaultParams: SRWeightedTrend263Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRWeightedTrend263Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_weighted_trend_263.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRWeightedTrend263Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRWeightedTrend263Params] = value;
        }
      }
    }
...
export class SRWeightedTrend263Strategy implements Strategy {
  params: SRWeightedTrend263Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRWeightedTrend263Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRWeightedTrend263Params;
  }
  ...
  private getWeightedScore(trend: number, momentum: number): number {

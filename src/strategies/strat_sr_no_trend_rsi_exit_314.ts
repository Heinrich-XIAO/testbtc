...
/**
 * SR No Trend RSI Exit 314
 * - Based on 302, adds RSI-based exit
 * - Exits when RSI becomes overbought (catches exits earlier than stochastic)
 */

export interface SRNoTrendRSIExit314Params extends StrategyParams {
  ...
  rsi_exit_min_profit: number;
}

const defaultParams: SRNoTrendRSIExit314Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
  ...
  rsi_period: 14,
  rsi_overbought: 70,
  rsi_exit_min_profit: 0.03,
};

function loadSavedParams(): Partial<SRNoTrendRSIExit314Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_rsi_exit_314.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendRSIExit314Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendRSIExit314Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendRSIExit314Strategy implements Strategy {
  params: SRNoTrendRSIExit314Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendRSIExit314Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendRSIExit314Params;
  }
  ...
  private getOrCreateData(tokenId: string): TokenData {
    if (!this.tokenData.has(tokenId)) {
      this.tokenData.set(tokenId, { prices: [], highs: [], lows: [], kValues: [], consecutiveBounces: 0, prevRsi: null });
    }
  ...
  private getRSI(prices: number[], period: number): number | null {
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const rsi = this.getRSI(data.prices, this.params.rsi_period);
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        if (rsi !== null && data.prevRsi !== null) {
          const hasProfit = bar.close >= entry * (1 + this.params.rsi_exit_min_profit);
          if (hasProfit && data.prevRsi >= this.params.rsi_overbought && rsi < data.prevRsi) {
            this.closePosition(ctx, bar.tokenId);
            return;
          }
        }
        if (rsi !== null) data.prevRsi = rsi;
      }

...
/**
 * SR Long Trend 276
 * - Longer trend period (40-60 bars) for more stable trend identification
 * - Better for catching longer-term moves
 */

export interface SRLongTrend276Params extends StrategyParams {
  ...
  trend_period: number; // 40-60 bars
...
const defaultParams: SRLongTrend276Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 50,
  ...
  stoch_oversold: 22,
  stoch_overbought: 75,
  trend_period: 48, // Much longer trend period
  ...
  trailing_stop: 0.055,
  profit_target: 0.15,
  max_hold_bars: 45, // Longer hold for longer trends
  risk_percent: 0.20,
};

function loadSavedParams(): Partial<SRLongTrend276Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_long_trend_276.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRLongTrend276Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRLongTrend276Params] = value;
        }
      }
    }
...
export class SRLongTrend276Strategy implements Strategy {
  params: SRLongTrend276Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRLongTrend276Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRLongTrend276Params;
  }

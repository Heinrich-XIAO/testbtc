...
export interface SRNoTrendATRStop322Params extends StrategyParams {
...
const defaultParams: SRNoTrendATRStop322Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendATRStop322Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_atr_stop_322.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendATRStop322Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendATRStop322Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendATRStop322Strategy implements Strategy {
  params: SRNoTrendATRStop322Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendATRStop322Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendATRStop322Params;
  }
  ...
  private calculateATR(data: TokenData, high: number, low: number, close: number): number {
    ...
    if (data.trueRanges.length > this.params.atr_period) data.trueRanges.shift();
    
    if (data.trueRanges.length < this.params.atr_period) {
      return data.trueRanges.reduce((a, b) => a + b, 0) / data.trueRanges.length;
    }
    
    if (data.atr === 0) {
      data.atr = data.trueRanges.reduce((a, b) => a + b, 0) / this.params.atr_period;
    } else {
      const alpha = 1 / this.params.atr_period;
      data.atr = data.atr * (1 - alpha) + tr * alpha;
    }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const atr = this.calculateATR(data, bar.high, bar.low, bar.close);
    data.prevClose = bar.close;
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        const atrTrailPct = atr > 0 ? (atr * this.params.atr_multiplier) / newHighest : this.params.trailing_stop;
        if (bar.close < newHighest * (1 - atrTrailPct)) { this.closePosition(ctx, bar.tokenId); return; }

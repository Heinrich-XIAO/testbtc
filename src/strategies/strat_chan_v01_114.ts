...
export interface ChanV01StrategyParams extends StrategyParams {
...
const defaultParams: ChanV01StrategyParams = {
  channel_period: 4,
  channel_width: 0.3,
  stop_loss: 0.03,
  trailing_stop: 0.02,
...
function loadSavedParams(): Partial<ChanV01StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_v01_114.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanV01StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanV01StrategyParams] = value;
        }
      }
    }
...
export class ChanV01Strategy implements Strategy {
  params: ChanV01StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanV01StrategyParams> = {}) {

...
export interface ChanV11StrategyParams extends StrategyParams {
...
const defaultParams: ChanV11StrategyParams = {
  channel_period: 4,
  channel_width: 0.3,
  stop_loss: 0.06,
  trailing_stop: 0.04,
...
function loadSavedParams(): Partial<ChanV11StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_v11_124.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanV11StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanV11StrategyParams] = value;
        }
      }
    }
...
export class ChanV11Strategy implements Strategy {
  params: ChanV11StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanV11StrategyParams> = {}) {

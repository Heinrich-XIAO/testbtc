...
/**
 * SR Momentum Exit 271
 * - Exit when momentum reverses
 * - Momentum-accelerated trailing stop
 */

export interface SRMomentumExit271Params extends StrategyParams {
  ...
  stop_loss: number;
  base_trailing_stop: number;
  momentum_trail_scale: number;
  profit_target: number;
  max_hold_bars: number;
  risk_percent: number;
}

const defaultParams: SRMomentumExit271Params = {
  base_lookback: 16,
  min_lookback: 6,
  max_lookback: 35,
  volatility_period: 8,
  bounce_threshold: 0.030,
  stoch_k_period: 16,
  stoch_d_period: 3,
  stoch_oversold: 20,
  stoch_overbought: 76,
...
function loadSavedParams(): Partial<SRMomentumExit271Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_momentum_exit_271.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRMomentumExit271Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRMomentumExit271Params] = value;
        }
      }
    }
...
export class SRMomentumExit271Strategy implements Strategy {
  params: SRMomentumExit271Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRMomentumExit271Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRMomentumExit271Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        const dynamicTrail = momentum < 0 
          ? this.params.base_trailing_stop * (1 - this.params.momentum_trail_scale)
          : this.params.base_trailing_stop;

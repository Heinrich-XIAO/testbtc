...
function decycler(closes: number[], period: number): number | null {
  if (closes.length < period) return null;
  
  const alpha = (Math.cos(0.707 * 2 * Math.PI / period) + Math.sin(0.707 * 2 * Math.PI / period) - 1) / Math.cos(0.707 * 2 * Math.PI / period);
  
  const prev = closes[closes.length - 2] || closes[closes.length - 1];
  return (1 - alpha / 2) * (1 - alpha / 2) * closes[closes.length - 1] + 2 * (1 - alpha) * prev - (1 - alpha) * (1 - alpha) * prev;
}
...
export interface StratIter139BParams extends StrategyParams { decycler_period: number; deviation: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; sr_lookback: number; }
export class StratIter139BStrategy extends BaseIterStrategy<StratIter139BParams> {
  constructor(params: Partial<StratIter139BParams> = {}) { super('strat_iter139_b.params.json', { decycler_period: 20, deviation: -0.03, stop_loss: 0.08, profit_target: 0.18, max_hold_bars: 28, risk_percent: 0.25, sr_lookback: 50 }, params); }
  onNext(ctx: BacktestContext, bar: Bar): void {
    const { series, barNum } = this.nextBar(bar); const dec = decycler(series.closes, this.params.decycler_period); const sr = priorSupportResistance(series.highs, series.lows, this.params.sr_lookback);
    ...
    if (shouldSkipPrice(bar.close) || !sr || dec === null) return;
    const nearSupport = bar.low <= sr.support * 1.015; const belowDecycler = (bar.close - dec) / dec < this.params.deviation;
    if (nearSupport && belowDecycler) { this.open(ctx, bar, barNum, this.params.risk_percent); }
  }
}
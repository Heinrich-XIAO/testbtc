...
export interface EnvV01StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV01StrategyParams = {
  ma_period: 4,
  envelope_pct: 0.01,
  stop_loss: 0.03,
  risk_percent: 0.08,
...
function loadSavedParams(): Partial<EnvV01StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_v01_94.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvV01StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV01StrategyParams] = value;
        }
      }
    }
...
export class EnvV01Strategy implements Strategy {
  params: EnvV01StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV01StrategyParams> = {}) {

...
export interface ComboV11StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV11StrategyParams = {
  bb_period: 4,
  rsi_period: 3,
  std_mult: 1.3,
  rsi_oversold: 35,
...
function loadSavedParams(): Partial<ComboV11StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v11_144.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV11StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV11StrategyParams] = value;
        }
      }
    }
...
export class ComboV11Strategy implements Strategy {
  params: ComboV11StrategyParams;
  ...
  constructor(params: Partial<ComboV11StrategyParams> = {}) {

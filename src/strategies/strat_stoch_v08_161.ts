...
export interface StochV08StrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochV08StrategyParams = {
  k_period: 12,
  d_period: 5,
  oversold: 25,
  overbought: 75,
...
function loadSavedParams(): Partial<StochV08StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_v08_161.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochV08StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochV08StrategyParams] = value;
        }
      }
    }
...
export class StochV08Strategy implements Strategy {
  params: StochV08StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochV08StrategyParams> = {}) {

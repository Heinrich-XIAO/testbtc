...
export interface StratIter85DParams extends StrategyParams {
  window_size: number;
  k_sigma: number;
...
const defaultParams: StratIter85DParams = {
  window_size: 30,
  k_sigma: 2.5,
  stoch_oversold: 16,
  stop_loss: 0.08,
...
function loadSavedParams(): Partial<StratIter85DParams> | null {
  const paramsPath = path.join(__dirname, 'strat_iter85_d.params.json');
...
export class StratIter85DStrategy implements Strategy {
  params: StratIter85DParams;
  ...
  constructor(params: Partial<StratIter85DParams> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as StratIter85DParams;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const lcl = mu - this.params.k_sigma * sigma;

    const k = this.stochasticK(closes, highs, lows, 14);

    if (bar.close < lcl && k !== null && k <= this.params.stoch_oversold) {

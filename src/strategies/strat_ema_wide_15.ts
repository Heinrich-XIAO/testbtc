...
/**
 * EMA cross with wide stops for volatile markets
 */

export interface EMAWideStopStrategyParams extends StrategyParams {
  fast_period: number;
  slow_period: number;
  stop_loss: number;
...
const defaultParams: EMAWideStopStrategyParams = {
  fast_period: 5,
  slow_period: 12,
  stop_loss: 0.1,
  trailing_stop: 0.08,
...
function loadSavedParams(): Partial<EMAWideStopStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_ema_wide_15.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EMAWideStopStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EMAWideStopStrategyParams] = value;
        }
      }
    }
...
export class EMAWideStopStrategy implements Strategy {
  params: EMAWideStopStrategyParams;
  ...
  constructor(params: Partial<EMAWideStopStrategyParams> = {}) {

...
function ehlersCycle(closes: number[], period: number): number | null {
  if (closes.length < period + 4) return null;
  
  const alpha = (1 - Math.sin(2 * Math.PI / period)) / Math.cos(2 * Math.PI / period);
  
  let smooth = 0, cycle = 0;
  for (let i = closes.length - period - 4; i < closes.length; i++) {
    const idx = i;
    if (idx >= 3) {
      smooth = (closes[idx] + 2 * closes[idx - 1] + 2 * closes[idx - 2] + closes[idx - 3]) / 6;
...
export interface StratIter139AParams extends StrategyParams { cycle_threshold: number; cycle_period: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; sr_lookback: number; }
export class StratIter139AStrategy extends BaseIterStrategy<StratIter139AParams> {
  constructor(params: Partial<StratIter139AParams> = {}) { super('strat_iter139_a.params.json', { cycle_threshold: -0.01, cycle_period: 20, stop_loss: 0.08, profit_target: 0.18, max_hold_bars: 28, risk_percent: 0.25, sr_lookback: 50 }, params); }
  onNext(ctx: BacktestContext, bar: Bar): void {
    const { series, barNum } = this.nextBar(bar); const cycle = ehlersCycle(series.closes, this.params.cycle_period); const sr = priorSupportResistance(series.highs, series.lows, this.params.sr_lookback);
    ...
    if (shouldSkipPrice(bar.close) || !sr || cycle === null) return;
    const nearSupport = bar.low <= sr.support * 1.015; const cycleLow = cycle < this.params.cycle_threshold;
    if (nearSupport && cycleLow) { this.open(ctx, bar, barNum, this.params.risk_percent); }
  }
}
...
/**
 * SR No Trend Triple 350
 * - Based on 302 but adds RSI < 30 as stricter filter
 * - Triple confirmation: near support AND RSI < 30 AND stochastic oversold AND momentum positive
 */

export interface SRNoTrendTriple350Params extends StrategyParams {
...
const defaultParams: SRNoTrendTriple350Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendTriple350Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_triple_350.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendTriple350Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendTriple350Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendTriple350Strategy implements Strategy {
  params: SRNoTrendTriple350Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendTriple350Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendTriple350Params;
  }
  ...
  private getOrCreateData(tokenId: string): TokenData {
    if (!this.tokenData.has(tokenId)) {
      this.tokenData.set(tokenId, { prices: [], highs: [], lows: [], kValues: [], rsiValues: [], consecutiveBounces: 0 });
    }
  ...
  private getRSI(data: TokenData): number | null {
    ...
    if (data.rsiValues.length > 2) data.rsiValues.shift();
    return rsi;
  }

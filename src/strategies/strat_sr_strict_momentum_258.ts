...
/**
 * SR Strict Momentum 258
 * - Tweak of sr_strict_entry_256 ($4,098 best)
 * - Looser momentum threshold
 * - Adjusted trend threshold
 */

export interface SRStrictMomentum258Params extends StrategyParams {
...
const defaultParams: SRStrictMomentum258Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRStrictMomentum258Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_strict_momentum_258.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRStrictMomentum258Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRStrictMomentum258Params] = value;
        }
      }
    }
...
export class SRStrictMomentum258Strategy implements Strategy {
  params: SRStrictMomentum258Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRStrictMomentum258Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRStrictMomentum258Params;
  }

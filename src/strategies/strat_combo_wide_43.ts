...
/**
 * RSI + BB combo with wide thresholds
 */

export interface ComboWideStrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboWideStrategyParams = {
  bb_period: 12,
  rsi_period: 7,
  std_mult: 2.2,
  rsi_oversold: 25,
...
function loadSavedParams(): Partial<ComboWideStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_wide_43.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboWideStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboWideStrategyParams] = value;
        }
      }
    }
...
export class ComboWideStrategy implements Strategy {
  params: ComboWideStrategyParams;
  ...
  constructor(params: Partial<ComboWideStrategyParams> = {}) {

...
/**
 * SR No Trend Two Level 344
 * - Based on 302 but requires TWO support levels for entry
 * - Short-term support (recent lows) AND long-term support (historical lows)
 * - Only enter when price is near BOTH support levels (confluence)
 */

export interface SRNoTrendTwoLevel344Params extends StrategyParams {
  base_lookback_short: number;
  base_lookback_long: number;
  min_lookback: number;
  max_lookback: number;
...
const defaultParams: SRNoTrendTwoLevel344Params = {
  base_lookback_short: 8,
  base_lookback_long: 24,
  min_lookback: 6,
  max_lookback: 36,
  ...
  max_hold_bars: 28,
  risk_percent: 0.30,
  confluence_threshold: 0.035,
};

function loadSavedParams(): Partial<SRNoTrendTwoLevel344Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_two_level_344.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendTwoLevel344Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendTwoLevel344Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendTwoLevel344Strategy implements Strategy {
  params: SRNoTrendTwoLevel344Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendTwoLevel344Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendTwoLevel344Params;
  }
  ...
  private getAdaptiveLookbackShort(data: TokenData): number {
    if (data.prices.length < this.params.volatility_period) return this.params.base_lookback_short;
    ...
    if (historicalVol === 0) return this.params.base_lookback_short;
    const volRatio = recentVol / historicalVol;
    let adaptiveLookback = Math.round(this.params.base_lookback_short / volRatio);
    return Math.max(this.params.min_lookback, Math.min(this.params.base_lookback_short * 2, adaptiveLookback));
  }

  private getAdaptiveLookbackLong(data: TokenData): number {
    if (data.prices.length < this.params.volatility_period) return this.params.base_lookback_long;
    const recentVol = this.calcVolatility(data.prices.slice(-this.params.volatility_period));
    const historicalVol = this.calcVolatility(data.prices.slice(-this.params.max_lookback));
    if (historicalVol === 0) return this.params.base_lookback_long;
    const volRatio = recentVol / historicalVol;
    let adaptiveLookback = Math.round(this.params.base_lookback_long / volRatio);
    return Math.max(this.params.base_lookback_short * 2, Math.min(this.params.max_lookback, adaptiveLookback));
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const adaptiveLookbackShort = this.getAdaptiveLookbackShort(data);
    const adaptiveLookbackLong = this.getAdaptiveLookbackLong(data);
    const momentum = this.getMomentum(data.prices);
    const supportsShort = this.findSupportLevels(data, adaptiveLookbackShort);
    const supportsLong = this.findSupportLevels(data, adaptiveLookbackLong);
    const resistance = this.findResistanceLevel(data, adaptiveLookbackLong);
    
    if (!stoch || supportsShort.length === 0 || supportsLong.length === 0) return;
    ...
    if (position && position.size > 0) {
      ...
      const nearShortSupport = supportsShort.some(s => Math.abs(bar.close - s) / s < this.params.bounce_threshold);
      const nearLongSupport = supportsLong.some(s => Math.abs(bar.close - s) / s < this.params.confluence_threshold);
      const atConfluence = nearShortSupport && nearLongSupport;
      ...
      if (atConfluence && multiBarBounce && stochOversold && momentumOk) {

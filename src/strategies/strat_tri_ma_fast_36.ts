...
/**
 * Fast triple MA alignment
 */

export interface TriMAFastStrategyParams extends StrategyParams {
  fast_period: number;
  mid_period: number;
  slow_period: number;
  stop_loss: number;
...
const defaultParams: TriMAFastStrategyParams = {
  fast_period: 3,
  mid_period: 6,
  slow_period: 12,
  stop_loss: 0.05,
...
function loadSavedParams(): Partial<TriMAFastStrategyParams> | null {
  ...
  try {
    ...
    const params: Partial<TriMAFastStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof TriMAFastStrategyParams] = value;
        }
      }
    }
...
export class TriMAFastStrategy implements Strategy {
  params: TriMAFastStrategyParams;
  ...
  constructor(params: Partial<TriMAFastStrategyParams> = {}) {

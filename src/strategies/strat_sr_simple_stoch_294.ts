...
/**
 * SR Simple Stoch 294
 * - Simplified entry: just stochastic + support bounce
 * - No momentum, no trend filter - testing if simpler = more trades + better test returns
 * - Moderate stochastic (oversold=24, overbought=80)
 */

export interface SRSimpleStoch294Params extends StrategyParams {
...
const defaultParams: SRSimpleStoch294Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRSimpleStoch294Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_simple_stoch_294.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRSimpleStoch294Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRSimpleStoch294Params] = value;
        }
      }
    }
...
export class SRSimpleStoch294Strategy implements Strategy {
  params: SRSimpleStoch294Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRSimpleStoch294Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRSimpleStoch294Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      // SIMPLIFIED: just stochastic oversold + near support

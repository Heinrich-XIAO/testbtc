 ...
 * - Fewer bounce bars required (more trades)
 * - Slightly looser stoch oversold
 */

export interface SRStrictLoose257Params extends StrategyParams {
...
const defaultParams: SRStrictLoose257Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRStrictLoose257Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_strict_loose_257.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRStrictLoose257Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRStrictLoose257Params] = value;
        }
      }
    }
...
export class SRStrictLoose257Strategy implements Strategy {
  params: SRStrictLoose257Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRStrictLoose257Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRStrictLoose257Params;
  }

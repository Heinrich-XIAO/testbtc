...
/**
 * SR Very Wide No Mom 303
 * - Very wide stochastic bands (oversold=28, overbought=75)
 * - No momentum filter
 * - Goal: Maximum trade opportunities
 */

export interface SRVeryWideNoMom303Params extends StrategyParams {
...
const defaultParams: SRVeryWideNoMom303Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRVeryWideNoMom303Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_very_wide_no_mom_303.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRVeryWideNoMom303Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRVeryWideNoMom303Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRVeryWideNoMom303Strategy implements Strategy {
  params: SRVeryWideNoMom303Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRVeryWideNoMom303Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRVeryWideNoMom303Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      // VERY WIDE stoch bands (28/75) + NO momentum

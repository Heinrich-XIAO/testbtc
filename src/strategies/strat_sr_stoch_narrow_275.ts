...
/**
 * SR Stoch Narrow 275
 * - Narrower stochastic levels (30/65) for more frequent signals
 * - Trade more often with tighter bands
 */

export interface SRStochNarrow275Params extends StrategyParams {
  ...
  stoch_oversold: number; // 28-35
  stoch_overbought: number; // 62-70
  trend_period: number;
...
const defaultParams: SRStochNarrow275Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
  ...
  stoch_k_period: 14,
  stoch_d_period: 4,
  stoch_oversold: 32, // Narrower - more signals
  stoch_overbought: 65, // Narrower - earlier exits
  trend_period: 22,
...
function loadSavedParams(): Partial<SRStochNarrow275Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_stoch_narrow_275.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRStochNarrow275Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRStochNarrow275Params] = value;
        }
      }
    }
...
export class SRStochNarrow275Strategy implements Strategy {
  params: SRStochNarrow275Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRStochNarrow275Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRStochNarrow275Params;
  }

...
export interface MRRsiV07StrategyParams extends StrategyParams {
  ma_period: number;
  rsi_period: number;
...
const defaultParams: MRRsiV07StrategyParams = {
  ma_period: 10,
  rsi_period: 8,
...
function loadSavedParams(): Partial<MRRsiV07StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_mr_rsi_v07_60.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<MRRsiV07StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof MRRsiV07StrategyParams] = value;
        }
      }
    }
...
export class MRRsiV07Strategy implements Strategy {
  params: MRRsiV07StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private rsiMap: Map<string, RSI> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<MRRsiV07StrategyParams> = {}) {

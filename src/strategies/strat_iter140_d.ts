...
function ema(values: number[], period: number): number | null {
  ...
  for (let i = period; i < values.length; i++) { emaVal = (values[i] - emaVal) * multiplier + emaVal; }
  return emaVal;
}
...
function tradingSystem2(closes: number[], emaPeriod: number, stochPeriod: number): { trend: number; oscillator: number } | null {
  if (closes.length < Math.max(emaPeriod, stochPeriod)) return null;
  
  const emaVal = ema(closes, emaPeriod);
  const stoch = stochK(closes, closes, closes, stochPeriod);
  
  if (emaVal === null || stoch === null) return null;
  
  return {
    trend: closes[closes.length - 1] > emaVal ? 1 : -1,
    oscillator: stoch
  };
}
...
export interface StratIter140DParams extends StrategyParams { stoch_oversold: number; ema_period: number; stoch_period: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; sr_lookback: number; }
export class StratIter140DStrategy extends BaseIterStrategy<StratIter140DParams> {
  constructor(params: Partial<StratIter140DParams> = {}) { super('strat_iter140_d.params.json', { stoch_oversold: 20, ema_period: 20, stoch_period: 14, stop_loss: 0.08, profit_target: 0.18, max_hold_bars: 28, risk_percent: 0.25, sr_lookback: 50 }, params); }
  onNext(ctx: BacktestContext, bar: Bar): void {
    const { series, barNum } = this.nextBar(bar); const ts2 = tradingSystem2(series.closes, this.params.ema_period, this.params.stoch_period); const sr = priorSupportResistance(series.highs, series.lows, this.params.sr_lookback);
    ...
    if (shouldSkipPrice(bar.close) || !sr || ts2 === null) return;
    const nearSupport = bar.low <= sr.support * 1.015;
    const buySignal = ts2.trend > 0 && ts2.oscillator < this.params.stoch_oversold;

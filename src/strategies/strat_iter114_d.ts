...
export interface StratIter114DParams extends StrategyParams {
  adx_period: number;
...
export class StratIter114DStrategy extends BaseIterStrategy<StratIter114DParams> {
  constructor(params: Partial<StratIter114DParams> = {}) {
    super('strat_iter114_d.params.json', {
      adx_period: 14,
      adx_threshold: 25,
      stop_loss: 0.08,
      profit_target: 0.18,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const adxVal = adx(series.highs, series.lows, series.closes, this.params.adx_period);
    ...
    if (shouldSkipPrice(bar.close) || !sr || adxVal === null) return;

    const nearSupport = bar.low <= sr.support * 1.015;
    const strongTrend = adxVal > this.params.adx_threshold;

    if (nearSupport && strongTrend) {

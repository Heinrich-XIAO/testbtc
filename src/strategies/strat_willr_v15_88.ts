...
export interface WillRV15StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV15StrategyParams = {
  period: 7,
  oversold_level: -80,
  overbought_level: -20,
  stop_loss: 0.02,
...
function loadSavedParams(): Partial<WillRV15StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v15_88.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV15StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV15StrategyParams] = value;
        }
      }
    }
...
export class WillRV15Strategy implements Strategy {
  params: WillRV15StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV15StrategyParams> = {}) {

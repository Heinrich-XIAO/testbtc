...
export interface StratIter183DParams extends StrategyParams { sr_lookback: number; entry_threshold: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; }
export class StratIter183DStrategy extends BaseIterStrategy<StratIter183DParams> {
  constructor(params: Partial<StratIter183DParams> = {}) { super('strat_iter183_d.params.json', { sr_lookback: 20, entry_threshold: 0.02, stop_loss: 0.08, profit_target: 0.15, max_hold_bars: 24, risk_percent: 0.25 }, params); }

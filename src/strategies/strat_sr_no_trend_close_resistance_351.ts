...
/**
 * SR No Trend Close Resistance 351
 * - Based on 302
 * - Exit when price is within 2% of resistance (instead of waiting for actual touch)
 */

export interface SRNoTrendCloseResistance351Params extends StrategyParams {
...
const defaultParams: SRNoTrendCloseResistance351Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendCloseResistance351Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_close_resistance_351.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendCloseResistance351Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendCloseResistance351Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendCloseResistance351Strategy implements Strategy {
  params: SRNoTrendCloseResistance351Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendCloseResistance351Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendCloseResistance351Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        const nearResistance = resistance !== null && bar.close >= resistance * 0.98;
        if (nearResistance || stoch.k >= this.params.stoch_overbought) {

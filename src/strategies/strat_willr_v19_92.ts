...
export interface WillRV19StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV19StrategyParams = {
  period: 3,
  oversold_level: -80,
  overbought_level: -20,
  stop_loss: 0.04,
...
function loadSavedParams(): Partial<WillRV19StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v19_92.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV19StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV19StrategyParams] = value;
        }
      }
    }
...
export class WillRV19Strategy implements Strategy {
  params: WillRV19StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV19StrategyParams> = {}) {

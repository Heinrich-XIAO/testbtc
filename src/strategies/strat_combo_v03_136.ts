...
export interface ComboV03StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV03StrategyParams = {
  bb_period: 6,
  rsi_period: 4,
  std_mult: 2,
  rsi_oversold: 25,
...
function loadSavedParams(): Partial<ComboV03StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v03_136.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV03StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV03StrategyParams] = value;
        }
      }
    }
...
export class ComboV03Strategy implements Strategy {
  params: ComboV03StrategyParams;
  ...
  constructor(params: Partial<ComboV03StrategyParams> = {}) {

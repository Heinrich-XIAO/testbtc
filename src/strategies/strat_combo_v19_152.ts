...
export interface ComboV19StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV19StrategyParams = {
  bb_period: 3,
  rsi_period: 3,
  std_mult: 1.2,
  rsi_oversold: 30,
...
function loadSavedParams(): Partial<ComboV19StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v19_152.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV19StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV19StrategyParams] = value;
        }
      }
    }
...
export class ComboV19Strategy implements Strategy {
  params: ComboV19StrategyParams;
  ...
  constructor(params: Partial<ComboV19StrategyParams> = {}) {

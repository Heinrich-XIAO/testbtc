...
/**
 * SR No Trend Vol 20 373
 * - Based on 302
 * - Changed volatility_period from 12 to 20 (longer period for adaptive lookback)
 */

export interface SRNoTrendVol20373Params extends StrategyParams {
...
const defaultParams: SRNoTrendVol20373Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendVol20373Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_vol_20_373.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendVol20373Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendVol20373Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendVol20373Strategy implements Strategy {
  params: SRNoTrendVol20373Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendVol20373Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendVol20373Params;
  }

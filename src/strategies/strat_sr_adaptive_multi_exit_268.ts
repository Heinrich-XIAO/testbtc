...
/**
 * SR Adaptive Multi Exit 268
 * - Combines adaptive thresholds (264) with multi-exit (260)
 * - Volatility-scaled bounce threshold + profit/hold exits
 */

export interface SRAdaptiveMultiExit268Params extends StrategyParams {
...
const defaultParams: SRAdaptiveMultiExit268Params = {
  base_lookback: 16,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRAdaptiveMultiExit268Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_adaptive_multi_exit_268.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRAdaptiveMultiExit268Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRAdaptiveMultiExit268Params] = value;
        }
      }
    }
...
export class SRAdaptiveMultiExit268Strategy implements Strategy {
  params: SRAdaptiveMultiExit268Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRAdaptiveMultiExit268Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRAdaptiveMultiExit268Params;
  }
  ...
  private getAdaptiveStochOversold(data: TokenData): number {
    if (data.prices.length < this.params.volatility_period) {

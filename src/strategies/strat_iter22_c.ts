...
export interface StratIter22CParams extends StrategyParams {
...
const defaultParams: StratIter22CParams = {
  stoch_k_period: 14,
  stoch_d_period: 3,
  stoch_oversold: 14,
  stoch_overbought: 86,
  support_threshold: 0.01,
  resistance_threshold: 0.99,
  stop_loss: 0.08,
  profit_target: 0.18,
  max_hold_bars: 36,
  risk_percent: 0.25,
...
function loadSavedParams(): Partial<StratIter22CParams> | null {
  const paramsPath = path.join(__dirname, 'strat_iter22_c.params.json');
...
export class StratIter22CStrategy implements Strategy {
  params: StratIter22CParams;
  ...
  constructor(params: Partial<StratIter22CParams> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as StratIter22CParams;
  }

...
/**
 * SR No Trend Multi TP 319
 * - Based on 302 but adds multi-take-profit logic
 * - Closes 50% at first profit target, trails remaining 50%
 * - Tests if partial exits improve risk-adjusted returns
 */

export interface SRNoTrendMultiTP319Params extends StrategyParams {
  ...
  first_tp_percent: number;
  second_tp_percent: number;
  max_hold_bars: number;
  risk_percent: number;
}

const defaultParams: SRNoTrendMultiTP319Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
  ...
  stop_loss: 0.08,
  trailing_stop: 0.07,
  first_tp_percent: 0.08,
  second_tp_percent: 0.15,
  max_hold_bars: 28,
  risk_percent: 0.30,
};

function loadSavedParams(): Partial<SRNoTrendMultiTP319Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_multi_tp_319.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendMultiTP319Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendMultiTP319Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendMultiTP319Strategy implements Strategy {
  params: SRNoTrendMultiTP319Params;
  private tokenData: Map<string, TokenData> = new Map();
  private positionData: Map<string, PositionData> = new Map();

  constructor(params: Partial<SRNoTrendMultiTP319Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendMultiTP319Params;
  }
  ...
  private closePosition(ctx: BacktestContext, tokenId: string): void {
    ctx.close(tokenId);
    this.positionData.delete(tokenId);
  }

  private closePartialPosition(ctx: BacktestContext, tokenId: string, percent: number): void {
    const position = ctx.getPosition(tokenId);
    if (!position || position.size <= 0) return;

    const closeSize = position.size * percent;
    if (closeSize <= 0) return;

    ctx.sell(tokenId, closeSize);
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      if (!this.positionData.has(bar.tokenId)) {
        this.positionData.set(bar.tokenId, {
      ...
      const posData = this.positionData.get(bar.tokenId)!;
      posData.barsHeld++;
      posData.highestPrice = Math.max(posData.highestPrice, bar.close);

      const firstTpPrice = posData.entryPrice * (1 + this.params.first_tp_percent);
      const secondTpPrice = posData.entryPrice * (1 + this.params.second_tp_percent);

      if (bar.close < posData.entryPrice * (1 - this.params.stop_loss)) {
        this.closePosition(ctx, bar.tokenId);
        return;
      }

      if (!posData.firstTpHit && bar.close >= firstTpPrice) {
        this.closePartialPosition(ctx, bar.tokenId, 0.5);
        posData.firstTpHit = true;
        return;
      }

      if (posData.firstTpHit) {
        if (bar.close < posData.highestPrice * (1 - this.params.trailing_stop)) {
          this.closePosition(ctx, bar.tokenId);
          return;
        }

        if (bar.close >= secondTpPrice) {
          this.closePosition(ctx, bar.tokenId);
          return;
        }

        if (posData.barsHeld >= this.params.max_hold_bars) {
          this.closePosition(ctx, bar.tokenId);
          return;
        }

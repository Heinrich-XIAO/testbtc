...
export interface ChanV14StrategyParams extends StrategyParams {
...
const defaultParams: ChanV14StrategyParams = {
  channel_period: 9,
  channel_width: 0.6,
  stop_loss: 0.06,
  trailing_stop: 0.05,
...
function loadSavedParams(): Partial<ChanV14StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_v14_127.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanV14StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanV14StrategyParams] = value;
        }
      }
    }
...
export class ChanV14Strategy implements Strategy {
  params: ChanV14StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanV14StrategyParams> = {}) {

...
/**
 * SR No Trend Tight Trail 367
 * - Based on 302
 * - Tighter trailing stop (0.04 instead of 0.07)
 */

export interface SRNoTrendTightTrail367Params extends StrategyParams {
...
const defaultParams: SRNoTrendTightTrail367Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendTightTrail367Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_tight_trail_367.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendTightTrail367Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendTightTrail367Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendTightTrail367Strategy implements Strategy {
  params: SRNoTrendTightTrail367Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendTightTrail367Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendTightTrail367Params;
  }

...
function phaseAccumulation(closes: number[], period: number): number | null {
  ...
  for (let i = 1; i < smooth.length; i++) {
    const deltaPhase = Math.atan2(smooth[i] - smooth[i - 1], 1);
    phase += deltaPhase;
  }
...
export interface StratIter139EParams extends StrategyParams { pa_threshold: number; pa_period: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; sr_lookback: number; }
export class StratIter139EStrategy extends BaseIterStrategy<StratIter139EParams> {
  constructor(params: Partial<StratIter139EParams> = {}) { super('strat_iter139_e.params.json', { pa_threshold: -2, pa_period: 20, stop_loss: 0.08, profit_target: 0.18, max_hold_bars: 28, risk_percent: 0.25, sr_lookback: 50 }, params); }
  onNext(ctx: BacktestContext, bar: Bar): void {
    const { series, barNum } = this.nextBar(bar); const pa = phaseAccumulation(series.closes, this.params.pa_period); const sr = priorSupportResistance(series.highs, series.lows, this.params.sr_lookback);
    ...
    if (shouldSkipPrice(bar.close) || !sr || pa === null) return;
    const nearSupport = bar.low <= sr.support * 1.015; const paLow = pa < this.params.pa_threshold;
    if (nearSupport && paLow) { this.open(ctx, bar, barNum, this.params.risk_percent); }
  }
}
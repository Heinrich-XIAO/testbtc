...
export interface PatV15StrategyParams extends StrategyParams {
  consec_bars: number;
  exit_bars: number;
  stop_loss: number;
...
const defaultParams: PatV15StrategyParams = {
  consec_bars: 3,
  exit_bars: 2,
  stop_loss: 0.02,
  trailing_stop: 0.02,
...
function loadSavedParams(): Partial<PatV15StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_pat_v15_188.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<PatV15StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof PatV15StrategyParams] = value;
        }
      }
    }
...
export class PatV15Strategy implements Strategy {
  params: PatV15StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<PatV15StrategyParams> = {}) {

...
export interface ComboV05StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV05StrategyParams = {
  bb_period: 8,
  rsi_period: 5,
  std_mult: 1.5,
  rsi_oversold: 30,
...
function loadSavedParams(): Partial<ComboV05StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v05_138.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV05StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV05StrategyParams] = value;
        }
      }
    }
...
export class ComboV05Strategy implements Strategy {
  params: ComboV05StrategyParams;
  ...
  constructor(params: Partial<ComboV05StrategyParams> = {}) {

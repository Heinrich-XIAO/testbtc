...
export interface PatV07StrategyParams extends StrategyParams {
  consec_bars: number;
  exit_bars: number;
  stop_loss: number;
...
const defaultParams: PatV07StrategyParams = {
  consec_bars: 4,
  exit_bars: 2,
  stop_loss: 0.07,
  trailing_stop: 0.05,
...
function loadSavedParams(): Partial<PatV07StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_pat_v07_180.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<PatV07StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof PatV07StrategyParams] = value;
        }
      }
    }
...
export class PatV07Strategy implements Strategy {
  params: PatV07StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<PatV07StrategyParams> = {}) {

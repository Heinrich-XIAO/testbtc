...
/**
 * SR Bounce Quality 262
 * - Measures bounce quality (cumulative gain over bounce bars)
 * - Higher quality bounces = stronger signal
 */

export interface SRBounceQuality262Params extends StrategyParams {
  ...
  min_bounce_gain: number;
  stop_loss: number;
  trailing_stop: number;
  risk_percent: number;
}

const defaultParams: SRBounceQuality262Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRBounceQuality262Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_bounce_quality_262.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRBounceQuality262Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRBounceQuality262Params] = value;
        }
      }
    }
...
interface TokenData {
  ...
  bounceStartPrice: number;
}

export class SRBounceQuality262Strategy implements Strategy {
  params: SRBounceQuality262Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRBounceQuality262Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRBounceQuality262Params;
  }

...
/**
 * SR Balanced Stoch 292
 * - Balanced stochastic thresholds (oversold=25, overbought=78)
 * - Focus on consistency over extreme filtering
 * - New approach: remove K>D requirement for more entries
 */

export interface SRBalancedStoch292Params extends StrategyParams {
...
const defaultParams: SRBalancedStoch292Params = {
  base_lookback: 20,
  min_lookback: 10,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRBalancedStoch292Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_balanced_stoch_292.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRBalancedStoch292Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRBalancedStoch292Params] = value;
        }
      }
    }
...
export class SRBalancedStoch292Strategy implements Strategy {
  params: SRBalancedStoch292Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRBalancedStoch292Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRBalancedStoch292Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      // NOTE: Removed K > D requirement - just check if K is oversold

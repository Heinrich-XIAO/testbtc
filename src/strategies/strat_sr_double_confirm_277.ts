...
/**
 * SR Double Confirm 277
 * - Requires BOTH RSI and Stochastic confirmation
 * - More selective entries but higher confidence
 */

export interface SRDoubleConfirm277Params extends StrategyParams {
...
const defaultParams: SRDoubleConfirm277Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRDoubleConfirm277Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_double_confirm_277.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRDoubleConfirm277Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRDoubleConfirm277Params] = value;
        }
      }
    }
...
export class SRDoubleConfirm277Strategy implements Strategy {
  params: SRDoubleConfirm277Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRDoubleConfirm277Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRDoubleConfirm277Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        // Exit on double overbought
        const doubleOverbought = stoch.k >= this.params.stoch_overbought && rsi >= this.params.rsi_overbought;
        if ((resistance !== null && bar.close >= resistance) || doubleOverbought) {
          this.closePosition(ctx, bar.tokenId);
        }
      }
      ...
      // Require BOTH stochastic AND RSI oversold
      if (nearSupport && multiBarBounce && stochOversold && rsiOversold && trendOk && momentumOk) {

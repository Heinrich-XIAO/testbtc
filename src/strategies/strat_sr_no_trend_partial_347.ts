...
/**
 * SR No Trend Partial 347
 * - Based on 302
 * - Partial profit taking: close 50% when profit > 5%
 * - Let rest run with full exit rules
 */

export interface SRNoTrendPartial347Params extends StrategyParams {
...
const defaultParams: SRNoTrendPartial347Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendPartial347Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_partial_347.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendPartial347Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendPartial347Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendPartial347Strategy implements Strategy {
  params: SRNoTrendPartial347Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();
  private partialExited: Map<string, boolean> = new Map();

  constructor(params: Partial<SRNoTrendPartial347Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendPartial347Params;
  }
  ...
  private closePosition(ctx: BacktestContext, tokenId: string): void {
    ...
    this.partialExited.delete(tokenId);
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      const hasPartialExited = this.partialExited.get(bar.tokenId) ?? false;
      ...
      if (entry) {
        ...
        const profit = (bar.close - entry) / entry;

        if (!hasPartialExited && profit >= this.params.partial_profit_threshold) {
          const halfSize = position.size / 2;
          ctx.sell(bar.tokenId, halfSize);
          this.partialExited.set(bar.tokenId, true);
        }
      ...
      if (nearSupport && multiBarBounce && stochOversold && momentumOk) {
        ...
        if (size > 0 && cash <= ctx.getCapital()) {
          ...
          if (result.success) {
            ...
            this.partialExited.set(bar.tokenId, false);
          }
        }
      }
    }

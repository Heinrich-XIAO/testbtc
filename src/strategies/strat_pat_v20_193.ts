...
export interface PatV20StrategyParams extends StrategyParams {
  consec_bars: number;
  exit_bars: number;
  stop_loss: number;
...
const defaultParams: PatV20StrategyParams = {
  consec_bars: 5,
  exit_bars: 4,
  stop_loss: 0.1,
  trailing_stop: 0.08,
...
function loadSavedParams(): Partial<PatV20StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_pat_v20_193.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<PatV20StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof PatV20StrategyParams] = value;
        }
      }
    }
...
export class PatV20Strategy implements Strategy {
  params: PatV20StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<PatV20StrategyParams> = {}) {

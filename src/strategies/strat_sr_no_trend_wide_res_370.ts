...
/**
 * SR No Trend Wide Res 370
 * - Based on 302
 * - Exit uses 10% below resistance instead of exact resistance level
 */

export interface SRNoTrendWideRes370Params extends StrategyParams {
  ...
  risk_percent: number;
  resistance_band: number;
}

const defaultParams: SRNoTrendWideRes370Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
  ...
  max_hold_bars: 28,
  risk_percent: 0.30,
  resistance_band: 0.10,
};

function loadSavedParams(): Partial<SRNoTrendWideRes370Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_wide_res_370.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendWideRes370Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendWideRes370Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendWideRes370Strategy implements Strategy {
  params: SRNoTrendWideRes370Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendWideRes370Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendWideRes370Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        const wideResistance = resistance !== null ? resistance * (1 - this.params.resistance_band) : null;
        if ((wideResistance !== null && bar.close >= wideResistance) || stoch.k >= this.params.stoch_overbought) {

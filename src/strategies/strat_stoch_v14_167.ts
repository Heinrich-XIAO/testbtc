...
export interface StochV14StrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochV14StrategyParams = {
  k_period: 9,
  d_period: 4,
  oversold: 18,
  overbought: 82,
...
function loadSavedParams(): Partial<StochV14StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_v14_167.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochV14StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochV14StrategyParams] = value;
        }
      }
    }
...
export class StochV14Strategy implements Strategy {
  params: StochV14StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochV14StrategyParams> = {}) {

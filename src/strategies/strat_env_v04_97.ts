...
export interface EnvV04StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV04StrategyParams = {
  ma_period: 5,
  envelope_pct: 0.04,
  stop_loss: 0.07,
  risk_percent: 0.15,
...
function loadSavedParams(): Partial<EnvV04StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_v04_97.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvV04StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV04StrategyParams] = value;
        }
      }
    }
...
export class EnvV04Strategy implements Strategy {
  params: EnvV04StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV04StrategyParams> = {}) {

...
function bandpassFilter(closes: number[], period: number, bandwidth: number): number | null {
  ...
  const gamma = 1 / Math.cos(4 * Math.PI * bandwidth / period);
  const alpha = gamma - Math.sqrt(gamma * gamma - 1);
  
  let bp = 0;
  for (let i = Math.max(0, closes.length - period - 4); i < closes.length; i++) {
    if (i >= 3) {
      bp = 0.5 * (1 - alpha) * (closes[i] - closes[i - 2]) + beta * (1 + alpha) * bp - alpha * bp;
    }
  }
...
export interface StratIter139CParams extends StrategyParams { bp_threshold: number; bp_period: number; bandwidth: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; sr_lookback: number; }
export class StratIter139CStrategy extends BaseIterStrategy<StratIter139CParams> {
  constructor(params: Partial<StratIter139CParams> = {}) { super('strat_iter139_c.params.json', { bp_threshold: -0.01, bp_period: 20, bandwidth: 0.3, stop_loss: 0.08, profit_target: 0.18, max_hold_bars: 28, risk_percent: 0.25, sr_lookback: 50 }, params); }
  onNext(ctx: BacktestContext, bar: Bar): void {
    const { series, barNum } = this.nextBar(bar); const bp = bandpassFilter(series.closes, this.params.bp_period, this.params.bandwidth); const sr = priorSupportResistance(series.highs, series.lows, this.params.sr_lookback);
    ...
    if (shouldSkipPrice(bar.close) || !sr || bp === null) return;
    const nearSupport = bar.low <= sr.support * 1.015; const bpLow = bp < this.params.bp_threshold;
    if (nearSupport && bpLow) { this.open(ctx, bar, barNum, this.params.risk_percent); }
  }
}
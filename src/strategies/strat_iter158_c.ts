...
export interface StratIter158CParams extends StrategyParams {
  sr_lookback: number;
  vol_period: number;
...
export class StratIter158CStrategy extends BaseIterStrategy<StratIter158CParams> {
  private kVals: Map<string, number[]> = new Map();
  private volHistory: Map<string, number[]> = new Map();
  private spikeBar: Map<string, number> = new Map();
  constructor(params: Partial<StratIter158CParams> = {}) {
    super('strat_iter158_c.params.json', {
      sr_lookback: 50,
      vol_period: 14,
      vol_rank_period: 30,
      spike_threshold: 80,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const vol = atr(series.highs, series.lows, series.closes, this.params.vol_period);
    if (vol !== null) capPush(this.volHistory.get(bar.tokenId)!, vol);
    ...
    if (shouldSkipPrice(bar.close) || !sr || kv.length < 2 || vh.length < this.params.vol_rank_period + 1) return;

    const currentVol = vh[vh.length - 1];
    const volWindow = vh.slice(-this.params.vol_rank_period);
    const volPercentile = percentileRank(volWindow, currentVol);

    const isSpike = volPercentile >= this.params.spike_threshold;
    const wasSpiking = this.spikeBar.has(bar.tokenId);

    if (isSpike && !wasSpiking) {
      this.spikeBar.set(bar.tokenId, barNum);
    }

    if (!isSpike && wasSpiking) {
      this.spikeBar.delete(bar.tokenId);
    }

    const spikeStart = this.spikeBar.get(bar.tokenId);
    const afterSpike = spikeStart !== undefined && barNum - spikeStart >= this.params.settle_bars;
    ...
    if (afterSpike && nearSupport && stochRecover) {
...
export const optimizationConfig = {
  ...
  spike_threshold: { min: 75, max: 90, stepSize: 5 },
  settle_bars: { min: 1, max: 4, stepSize: 1 },
...
export const outputFile = 'strat_iter158_c.params.json';

...
export interface SRNoTrendVolatility316Params extends StrategyParams {
...
const defaultParams: SRNoTrendVolatility316Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendVolatility316Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_volatility_316.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendVolatility316Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendVolatility316Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendVolatility316Strategy implements Strategy {
  params: SRNoTrendVolatility316Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendVolatility316Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendVolatility316Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const currentVolatility = this.calcVolatility(data.prices.slice(-this.params.volatility_period));
    ...
    if (position && position.size > 0) {
      ...
      const volatilityOk = currentVolatility >= this.params.min_volatility_threshold;

      if (nearSupport && multiBarBounce && stochOversold && momentumOk && volatilityOk) {

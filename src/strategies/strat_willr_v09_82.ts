...
export interface WillRV09StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV09StrategyParams = {
  period: 7,
  oversold_level: -95,
  overbought_level: -25,
  stop_loss: 0.06,
...
function loadSavedParams(): Partial<WillRV09StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v09_82.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV09StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV09StrategyParams] = value;
        }
      }
    }
...
export class WillRV09Strategy implements Strategy {
  params: WillRV09StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV09StrategyParams> = {}) {

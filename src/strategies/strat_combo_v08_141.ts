...
export interface ComboV08StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV08StrategyParams = {
  bb_period: 12,
  rsi_period: 7,
  std_mult: 1.5,
  rsi_oversold: 30,
...
function loadSavedParams(): Partial<ComboV08StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v08_141.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV08StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV08StrategyParams] = value;
        }
      }
    }
...
export class ComboV08Strategy implements Strategy {
  params: ComboV08StrategyParams;
  ...
  constructor(params: Partial<ComboV08StrategyParams> = {}) {

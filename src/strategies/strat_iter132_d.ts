...
export interface StratIter132DParams extends StrategyParams {
  di_threshold: number;
  dmi_period: number;
  stop_loss: number;
...
export class StratIter132DStrategy extends BaseIterStrategy<StratIter132DParams> {
  constructor(params: Partial<StratIter132DParams> = {}) {
    super('strat_iter132_d.params.json', {
      di_threshold: 20,
      dmi_period: 14,
      stop_loss: 0.08,
      profit_target: 0.18,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (shouldSkipPrice(bar.close) || !sr || dmiVal === null) return;

    const nearSupport = bar.low <= sr.support * 1.015;
    const minusDominant = dmiVal.minusDI > dmiVal.plusDI && dmiVal.minusDI > this.params.di_threshold;

    if (nearSupport && minusDominant) {

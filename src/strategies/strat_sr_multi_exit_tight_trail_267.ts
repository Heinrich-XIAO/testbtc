...
/**
 * SR Multi Exit Tight Trail 267
 * - Tweak of sr_strict_multi_exit_260 ($4,414 best)
 * - Tighter trailing stop for protecting gains
 */

export interface SRMultiExitTightTrail267Params extends StrategyParams {
...
const defaultParams: SRMultiExitTightTrail267Params = {
  base_lookback: 16,
  min_lookback: 6,
  max_lookback: 35,
  volatility_period: 8,
...
function loadSavedParams(): Partial<SRMultiExitTightTrail267Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_multi_exit_tight_trail_267.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRMultiExitTightTrail267Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRMultiExitTightTrail267Params] = value;
        }
      }
    }
...
export class SRMultiExitTightTrail267Strategy implements Strategy {
  params: SRMultiExitTightTrail267Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRMultiExitTightTrail267Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRMultiExitTightTrail267Params;
  }

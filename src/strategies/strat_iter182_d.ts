...
export interface StratIter182DParams extends StrategyParams { sr_lookback: number; entry_threshold: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; }
export class StratIter182DStrategy extends BaseIterStrategy<StratIter182DParams> {
  constructor(params: Partial<StratIter182DParams> = {}) { super('strat_iter182_d.params.json', { sr_lookback: 10, entry_threshold: 0.02, stop_loss: 0.08, profit_target: 0.15, max_hold_bars: 24, risk_percent: 0.25 }, params); }

...
/**
 * SR No Trend Support Strength 389
 * - Based on 362 but scores support levels by bounce count
 * - Only enters at "strong" support levels with multiple previous bounces
 */

export interface SRNoTrendSupportStrength389Params extends StrategyParams {
  ...
  risk_percent: number;
  min_support_bounces: number;
}

const defaultParams: SRNoTrendSupportStrength389Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
  ...
  max_hold_bars: 28,
  risk_percent: 0.30,
  min_support_bounces: 1,
};

function loadSavedParams(): Partial<SRNoTrendSupportStrength389Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_support_strength_389.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendSupportStrength389Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendSupportStrength389Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
interface TokenData {
  ...
  supportHistory: SupportLevel[];
  lastEntrySupport: number | null;
}

export class SRNoTrendSupportStrength389Strategy implements Strategy {
  params: SRNoTrendSupportStrength389Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendSupportStrength389Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendSupportStrength389Params;
  }
  ...
  private findSupportLevels(data: TokenData, lookback: number): SupportLevel[] {
    ...
    return lowest3.map(price => {
      const existing = data.supportHistory.find(s => Math.abs(s.price - price) / price < this.params.bounce_threshold);
      return existing ? { price, bounces: existing.bounces } : { price, bounces: 0 };
    });
  }
  ...
  private updateSupportBounce(data: TokenData, supportPrice: number): void {
    const existing = data.supportHistory.find(s => Math.abs(s.price - supportPrice) / supportPrice < this.params.bounce_threshold);
    if (existing) {
      existing.bounces++;
    } else {
      data.supportHistory.push({ price: supportPrice, bounces: 1 });
    }
  }

  private closePosition(ctx: BacktestContext, tokenId: string, profitably: boolean = false): void {
    const data = this.tokenData.get(tokenId);
    if (profitably && data && data.lastEntrySupport !== null) {
      this.updateSupportBounce(data, data.lastEntrySupport);
    }
    ctx.close(tokenId);
    this.entryPrice.delete(tokenId);
    this.highestPrice.delete(tokenId);
    this.barsHeld.delete(tokenId);
    if (data) data.lastEntrySupport = null;
  }
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        if (bar.close < entry * (1 - this.params.stop_loss)) {
          this.closePosition(ctx, bar.tokenId, false);
          return;
        }
        ...
        if (bar.close >= entry * (1 + this.params.profit_target)) {
          this.closePosition(ctx, bar.tokenId, true);
          return;
        }
      ...
      const validSupports = supports.filter(s => s.bounces >= this.params.min_support_bounces);
      const nearSupport = validSupports.some(s => Math.abs(bar.close - s.price) / s.price < this.params.bounce_threshold);
      ...
      if (nearSupport && multiBarBounce && stochOversold && momentumOk) {
        ...
        if (size > 0 && cash <= ctx.getCapital()) {
          ...
          if (result.success) {
            ...
            const matchedSupport = validSupports.find(s => Math.abs(bar.close - s.price) / s.price < this.params.bounce_threshold);
            data.lastEntrySupport = matchedSupport ? matchedSupport.price : null;
          }
        }
      }
    }

...
/**
 * SR Momentum Tweak 243
 * - Tweak of sr_adaptive_momentum_235 ($1,939)
 * - Adjusted momentum filter parameters
 * - Wider lookback range
 */

export interface SRMomentumTweak243Params extends StrategyParams {
...
const defaultParams: SRMomentumTweak243Params = {
  base_lookback: 22,
  min_lookback: 6,
  max_lookback: 40,
...
function loadSavedParams(): Partial<SRMomentumTweak243Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_momentum_tweak_243.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRMomentumTweak243Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRMomentumTweak243Params] = value;
        }
      }
    }
...
export class SRMomentumTweak243Strategy implements Strategy {
  params: SRMomentumTweak243Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRMomentumTweak243Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRMomentumTweak243Params;
  }

...
/**
 * SR Moderate Multi Exit 289
 * - Based on sr_wide_multi_exit_281 but with MODERATE stochastic thresholds
 * - Oversold=20, Overbought=82 (less extreme than 281's 16/86)
 * - Goal: Generate more test trades while keeping multi-exit mechanism
 */

export interface SRModerateMultiExit289Params extends StrategyParams {
...
const defaultParams: SRModerateMultiExit289Params = {
  base_lookback: 16,
  min_lookback: 8,
  max_lookback: 40,
  volatility_period: 8,
...
function loadSavedParams(): Partial<SRModerateMultiExit289Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_moderate_multi_exit_289.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRModerateMultiExit289Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRModerateMultiExit289Params] = value;
        }
      }
    }
...
export class SRModerateMultiExit289Strategy implements Strategy {
  params: SRModerateMultiExit289Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRModerateMultiExit289Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRModerateMultiExit289Params;
  }

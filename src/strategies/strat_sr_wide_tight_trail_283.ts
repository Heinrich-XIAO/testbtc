...
/**
 * SR Wide Tight Trail 283
 * - Wide stochastic bands from 274 + tighter trailing stop
 * - Lock in profits more aggressively
 */

export interface SRWideTightTrail283Params extends StrategyParams {
...
const defaultParams: SRWideTightTrail283Params = {
  base_lookback: 18,
  min_lookback: 8,
  max_lookback: 36,
  ...
  min_bounce_bars: 2,
  stop_loss: 0.075,
  trailing_stop: 0.035, // Tighter trailing stop (was 0.055)
  profit_target: 0.14,
  max_hold_bars: 35,
  risk_percent: 0.20,
};

function loadSavedParams(): Partial<SRWideTightTrail283Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_wide_tight_trail_283.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRWideTightTrail283Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRWideTightTrail283Params] = value;
        }
      }
    }
...
export class SRWideTightTrail283Strategy implements Strategy {
  params: SRWideTightTrail283Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRWideTightTrail283Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRWideTightTrail283Params;
  }

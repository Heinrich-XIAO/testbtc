...
export interface PatV08StrategyParams extends StrategyParams {
  consec_bars: number;
  exit_bars: number;
  stop_loss: number;
...
const defaultParams: PatV08StrategyParams = {
  consec_bars: 4,
  exit_bars: 3,
  stop_loss: 0.08,
  trailing_stop: 0.06,
...
function loadSavedParams(): Partial<PatV08StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_pat_v08_181.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<PatV08StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof PatV08StrategyParams] = value;
        }
      }
    }
...
export class PatV08Strategy implements Strategy {
  params: PatV08StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<PatV08StrategyParams> = {}) {

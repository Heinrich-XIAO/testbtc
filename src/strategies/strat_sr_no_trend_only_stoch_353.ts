...
/**
 * SR No Trend Only Stoch 353
 * - Ultra-minimal: ONLY stochastic oversold for entry
 * - No support, no momentum, no bounce
 * - Tests if simple stochastic alone beats complex logic
 */

export interface SRNoTrendOnlyStoch353Params extends StrategyParams {
...
const defaultParams: SRNoTrendOnlyStoch353Params = {
  stoch_k_period: 14,
  stoch_d_period: 4,
  stoch_oversold: 24,
  stoch_overbought: 82,
...
function loadSavedParams(): Partial<SRNoTrendOnlyStoch353Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_only_stoch_353.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendOnlyStoch353Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendOnlyStoch353Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendOnlyStoch353Strategy implements Strategy {
  params: SRNoTrendOnlyStoch353Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendOnlyStoch353Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendOnlyStoch353Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const maxPeriod = this.params.stoch_k_period + 20;

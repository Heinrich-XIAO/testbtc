...
/**
 * SR No Trend Lookup 382
 * - Based on 362 but with max_lookback=48
 */

export interface SRNoTrendLookup382Params extends StrategyParams {
...
const defaultParams: SRNoTrendLookup382Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 48,
...
function loadSavedParams(): Partial<SRNoTrendLookup382Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_lookup_382.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendLookup382Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendLookup382Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendLookup382Strategy implements Strategy {
  params: SRNoTrendLookup382Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendLookup382Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendLookup382Params;
  }

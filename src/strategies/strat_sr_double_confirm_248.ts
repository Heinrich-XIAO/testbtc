...
/**
 * SR Double Confirm 248
 * - Requires both stochastic AND price action confirmation
 * - More conservative entries, aiming for higher quality trades
 */

export interface SRDoubleConfirm248Params extends StrategyParams {
  ...
  bounce_bars: number;  // Must bounce for N bars before entry
  stop_loss: number;
...
const defaultParams: SRDoubleConfirm248Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRDoubleConfirm248Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_double_confirm_248.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRDoubleConfirm248Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRDoubleConfirm248Params] = value;
        }
      }
    }
...
export class SRDoubleConfirm248Strategy implements Strategy {
  params: SRDoubleConfirm248Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRDoubleConfirm248Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRDoubleConfirm248Params;
  }

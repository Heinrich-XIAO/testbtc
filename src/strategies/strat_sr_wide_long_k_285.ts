...
/**
 * SR Wide Long K 285
 * - Wide stochastic bands + longer K period (18)
 * - Smoother, less reactive stochastic indicator
 */

export interface SRWideLongK285Params extends StrategyParams {
...
const defaultParams: SRWideLongK285Params = {
  base_lookback: 20,
  min_lookback: 10,
  max_lookback: 40,
  volatility_period: 10,
  bounce_threshold: 0.026,
  stoch_k_period: 18, // Longer K period (was 14)
  stoch_d_period: 5,
...
function loadSavedParams(): Partial<SRWideLongK285Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_wide_long_k_285.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRWideLongK285Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRWideLongK285Params] = value;
        }
      }
    }
...
export class SRWideLongK285Strategy implements Strategy {
  params: SRWideLongK285Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRWideLongK285Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRWideLongK285Params;
  }

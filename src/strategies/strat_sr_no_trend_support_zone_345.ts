...
/**
 * SR No Trend Support Zone 345
 * - Based on 302 but uses SUPPORT ZONE instead of single support point
 * - Finds multiple support levels and creates a zone
 * - Only enters when price is WITHIN the support zone
 */

export interface SRNoTrendSupportZone345Params extends StrategyParams {
...
const defaultParams: SRNoTrendSupportZone345Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendSupportZone345Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_support_zone_345.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendSupportZone345Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendSupportZone345Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendSupportZone345Strategy implements Strategy {
  params: SRNoTrendSupportZone345Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendSupportZone345Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendSupportZone345Params;
  }
  ...
  private findSupportZone(data: TokenData, lookback: number): SupportZone | null {
    const supports = this.findSupportLevels(data, lookback);
  ...
  private isWithinSupportZone(price: number, zone: SupportZone): boolean {
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const supportZone = this.findSupportZone(data, adaptiveLookback);
    const resistance = this.findResistanceLevel(data, adaptiveLookback);
    
    if (!stoch || !supportZone) return;
    ...
    if (position && position.size > 0) {
      ...
      const inSupportZone = this.isWithinSupportZone(bar.close, supportZone);
      ...
      if (inSupportZone && multiBarBounce && stochOversold && momentumOk) {

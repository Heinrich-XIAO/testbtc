...
/**
 * SR No Trend Clustered Support 393
 * - Based on 362 but with clustered support detection
 * - Finds support zones where multiple lows cluster together
 */

export interface SRNoTrendClusteredSupport393Params extends StrategyParams {
...
const defaultParams: SRNoTrendClusteredSupport393Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
...
function loadSavedParams(): Partial<SRNoTrendClusteredSupport393Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_clustered_support_393.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendClusteredSupport393Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendClusteredSupport393Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
interface ClusteredSupport {
  level: number;
  count: number;
}

export class SRNoTrendClusteredSupport393Strategy implements Strategy {
  params: SRNoTrendClusteredSupport393Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendClusteredSupport393Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendClusteredSupport393Params;
  }
  ...
  private findSupportLevels(data: TokenData, lookback: number): ClusteredSupport[] {
    if (data.lows.length < lookback) return [];
    const recentLows = data.lows.slice(-lookback);
    
    const clusters: ClusteredSupport[] = [];
    
    for (let i = 0; i < recentLows.length; i++) {
      const low = recentLows[i];
      const clusterLows: number[] = [low];
      
      for (let j = 0; j < recentLows.length; j++) {
        if (i !== j) {
          const otherLow = recentLows[j];
          const diff = Math.abs(low - otherLow) / low;
          if (diff <= this.params.cluster_threshold) {
            clusterLows.push(otherLow);
          }
        }
      }
      
      if (clusterLows.length >= this.params.min_cluster_size) {
        const avgLevel = clusterLows.reduce((a, b) => a + b, 0) / clusterLows.length;
        const existingCluster = clusters.find(c => Math.abs(c.level - avgLevel) / avgLevel < this.params.cluster_threshold / 2);
        if (!existingCluster) {
          clusters.push({ level: avgLevel, count: clusterLows.length });
        }
      }
    }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      const nearSupport = supports.some(s => Math.abs(bar.close - s.level) / s.level < this.params.bounce_threshold);

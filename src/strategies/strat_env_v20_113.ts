...
/**
 * Wide period envelope
 */

export interface EnvV20StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV20StrategyParams = {
  ma_period: 18,
  envelope_pct: 0.06,
  stop_loss: 0.1,
  risk_percent: 0.15,
...
function loadSavedParams(): Partial<EnvV20StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_v20_113.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvV20StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV20StrategyParams] = value;
        }
      }
    }
...
export class EnvV20Strategy implements Strategy {
  params: EnvV20StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV20StrategyParams> = {}) {

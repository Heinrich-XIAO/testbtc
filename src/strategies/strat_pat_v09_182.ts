...
export interface PatV09StrategyParams extends StrategyParams {
  consec_bars: number;
  exit_bars: number;
  stop_loss: number;
...
const defaultParams: PatV09StrategyParams = {
  consec_bars: 5,
  exit_bars: 2,
  stop_loss: 0.08,
  trailing_stop: 0.06,
...
function loadSavedParams(): Partial<PatV09StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_pat_v09_182.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<PatV09StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof PatV09StrategyParams] = value;
        }
      }
    }
...
export class PatV09Strategy implements Strategy {
  params: PatV09StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<PatV09StrategyParams> = {}) {

...
/**
 * SR No Trend Min Look 376
 * - Based on 302 but widens min_lookback from 10 to 20
 * - Tests if wider lookback range improves performance
 */

export interface SRNoTrendMinLook376Params extends StrategyParams {
...
const defaultParams: SRNoTrendMinLook376Params = {
  base_lookback: 18,
  min_lookback: 20,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendMinLook376Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_min_look_376.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendMinLook376Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendMinLook376Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendMinLook376Strategy implements Strategy {
  params: SRNoTrendMinLook376Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendMinLook376Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendMinLook376Params;
  }

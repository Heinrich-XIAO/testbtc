...
/**
 * SR No Trend Time Filter 321
 * - Based on 302 but adds time-based filter
 * - Only enters trades in first trade_cutoff_ratio portion of dataset
 * - Skips late-game volatility in final period
 */

export interface SRNoTrendTimeFilter321Params extends StrategyParams {
  ...
  risk_percent: number;
  trade_cutoff_ratio: number;
}

const defaultParams: SRNoTrendTimeFilter321Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
  ...
  max_hold_bars: 28,
  risk_percent: 0.30,
  trade_cutoff_ratio: 0.66,
};

function loadSavedParams(): Partial<SRNoTrendTimeFilter321Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_time_filter_321.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendTimeFilter321Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendTimeFilter321Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendTimeFilter321Strategy implements Strategy {
  params: SRNoTrendTimeFilter321Params;
  ...
  private maxTimestampIndex: number = 600;
  private timestampCutoff: number = 0;
  private allTimestamps: Set<number> = new Set();

  constructor(params: Partial<SRNoTrendTimeFilter321Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendTimeFilter321Params;
  }

  onInit(_ctx: BacktestContext): void {
    this.timestampCutoff = Math.floor(this.maxTimestampIndex * this.params.trade_cutoff_ratio);
  }

  private updateTimestampIndex(bar: Bar): void {
    if (!this.timestampToIndex.has(bar.timestamp)) {
      this.allTimestamps.add(bar.timestamp);
      const sortedTimestamps = Array.from(this.allTimestamps).sort((a, b) => a - b);
      this.timestampToIndex.set(bar.timestamp, sortedTimestamps.indexOf(bar.timestamp));
      this.maxTimestampIndex = sortedTimestamps.length;
      this.timestampCutoff = Math.floor(this.maxTimestampIndex * this.params.trade_cutoff_ratio);
    }
    this.timestampIndex = this.timestampToIndex.get(bar.timestamp)!;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      const beforeCutoff = this.timestampIndex <= this.timestampCutoff;

      if (nearSupport && multiBarBounce && stochOversold && momentumOk && beforeCutoff) {

...
/**
 * SR No Trend Smooth Stoch 364
 * - Based on 302
 * - Wider stochastic D period (8 instead of 4) for smoother signals
 */

export interface SRNoTrendSmoothStoch364Params extends StrategyParams {
...
const defaultParams: SRNoTrendSmoothStoch364Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendSmoothStoch364Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_smooth_stoch_364.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendSmoothStoch364Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendSmoothStoch364Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendSmoothStoch364Strategy implements Strategy {
  params: SRNoTrendSmoothStoch364Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendSmoothStoch364Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendSmoothStoch364Params;
  }

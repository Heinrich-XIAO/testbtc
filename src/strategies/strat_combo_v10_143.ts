...
export interface ComboV10StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV10StrategyParams = {
  bb_period: 15,
  rsi_period: 9,
  std_mult: 2.5,
  rsi_oversold: 20,
...
function loadSavedParams(): Partial<ComboV10StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v10_143.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV10StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV10StrategyParams] = value;
        }
      }
    }
...
export class ComboV10Strategy implements Strategy {
  params: ComboV10StrategyParams;
  ...
  constructor(params: Partial<ComboV10StrategyParams> = {}) {

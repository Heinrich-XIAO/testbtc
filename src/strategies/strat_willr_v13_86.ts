...
export interface WillRV13StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV13StrategyParams = {
  period: 7,
  oversold_level: -82,
  overbought_level: -18,
  stop_loss: 0.05,
...
function loadSavedParams(): Partial<WillRV13StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v13_86.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV13StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV13StrategyParams] = value;
        }
      }
    }
...
export class WillRV13Strategy implements Strategy {
  params: WillRV13StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV13StrategyParams> = {}) {

...
/**
 * SR Adaptive Strict 264
 * - Adaptive thresholds + strict entry conditions
 * - Adjusts strictness based on volatility
 */

export interface SRAdaptiveStrict264Params extends StrategyParams {
...
const defaultParams: SRAdaptiveStrict264Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRAdaptiveStrict264Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_adaptive_strict_264.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRAdaptiveStrict264Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRAdaptiveStrict264Params] = value;
        }
      }
    }
...
export class SRAdaptiveStrict264Strategy implements Strategy {
  params: SRAdaptiveStrict264Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRAdaptiveStrict264Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRAdaptiveStrict264Params;
  }

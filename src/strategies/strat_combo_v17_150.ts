...
export interface ComboV17StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV17StrategyParams = {
  bb_period: 7,
  rsi_period: 5,
  std_mult: 2,
  rsi_oversold: 25,
...
function loadSavedParams(): Partial<ComboV17StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v17_150.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV17StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV17StrategyParams] = value;
        }
      }
    }
...
export class ComboV17Strategy implements Strategy {
  params: ComboV17StrategyParams;
  ...
  constructor(params: Partial<ComboV17StrategyParams> = {}) {

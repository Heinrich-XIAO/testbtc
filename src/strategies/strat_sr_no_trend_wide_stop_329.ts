...
/**
 * SR No Trend Wide Stop 329
 * - Based on 302
 * - Tests wider stop loss (0.15 vs 0.08)
 */

export interface SRNoTrendWideStop329Params extends StrategyParams {
...
const defaultParams: SRNoTrendWideStop329Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendWideStop329Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_wide_stop_329.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendWideStop329Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendWideStop329Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendWideStop329Strategy implements Strategy {
  params: SRNoTrendWideStop329Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendWideStop329Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendWideStop329Params;
  }

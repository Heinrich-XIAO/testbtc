...
/**
 * SR No Trend Stoch Cross Exit 388
 * - Based on 362 but exits on stochastic K/D bearish crossover
 * - Catches reversal earlier than overbought exit
 */

export interface SRNoTrendStochCrossExit388Params extends StrategyParams {
...
const defaultParams: SRNoTrendStochCrossExit388Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
...
function loadSavedParams(): Partial<SRNoTrendStochCrossExit388Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_stoch_cross_exit_388.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendStochCrossExit388Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendStochCrossExit388Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendStochCrossExit388Strategy implements Strategy {
  params: SRNoTrendStochCrossExit388Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendStochCrossExit388Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendStochCrossExit388Params;
  }
  ...
  private getOrCreateData(tokenId: string): TokenData {
    if (!this.tokenData.has(tokenId)) {
      this.tokenData.set(tokenId, { prices: [], highs: [], lows: [], kValues: [], consecutiveBounces: 0, prevK: 0, prevD: 0 });
    }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        const stochCrossExit = stoch.k < stoch.d && data.prevK >= data.prevD;
        if ((resistance !== null && bar.close >= resistance) || stochCrossExit) {

...
export interface PatV11StrategyParams extends StrategyParams {
  consec_bars: number;
  exit_bars: number;
  stop_loss: number;
...
const defaultParams: PatV11StrategyParams = {
  consec_bars: 2,
  exit_bars: 1,
  stop_loss: 0.06,
  trailing_stop: 0.04,
...
function loadSavedParams(): Partial<PatV11StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_pat_v11_184.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<PatV11StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof PatV11StrategyParams] = value;
        }
      }
    }
...
export class PatV11Strategy implements Strategy {
  params: PatV11StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<PatV11StrategyParams> = {}) {

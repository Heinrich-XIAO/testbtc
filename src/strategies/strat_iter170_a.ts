...
export interface StratIter170AParams extends StrategyParams {
  bb_period: number;
  bb_std: number;
  stop_loss: number;
...
export class StratIter170AStrategy implements Strategy {
  params: StratIter170AParams;
  private series: Map<string, TokenSeries> = new Map();
  private bars: Map<string, number> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private entryBar: Map<string, number> = new Map();

  constructor(params: Partial<StratIter170AParams> = {}) {
    const saved = loadSavedParams<StratIter170AParams>('strat_iter170_a.params.json');
    this.params = {
      bb_period: 20,
      bb_std: 2,
      stop_loss: 0.08,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    // Entry: Price touches or goes below lower Bollinger Band

...
export interface StratIter159CParams extends StrategyParams {
  partial_profit: number;
  partial_percent: number;
  stop_loss: number;
  profit_target: number;
  momentum_oversold: number;
...
export class StratIter159CStrategy extends BaseIterStrategy<StratIter159CParams> {
  constructor(params: Partial<StratIter159CParams> = {}) {
    super('strat_iter159_c.params.json', {
      partial_profit: 0.10,
      partial_percent: 0.5,
      stop_loss: 0.08,
      profit_target: 0.18,
      momentum_oversold: -0.02,
      momentum_rise_threshold: 0.005,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const currentMomentum = momentum(series.closes, this.params.momentum_lookback);
    if (currentMomentum === null) return;

    const priorMomentum = momentum(series.closes.slice(0, -1), this.params.momentum_lookback);
    if (priorMomentum === null) return;

    const momentumRising = currentMomentum > priorMomentum + this.params.momentum_rise_threshold;
    const momentumOversold = currentMomentum < this.params.momentum_oversold;

    if (momentumOversold && momentumRising) {

...
export interface ChanV04StrategyParams extends StrategyParams {
...
const defaultParams: ChanV04StrategyParams = {
  channel_period: 5,
  channel_width: 0.7,
  stop_loss: 0.07,
  trailing_stop: 0.05,
...
function loadSavedParams(): Partial<ChanV04StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_v04_117.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanV04StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanV04StrategyParams] = value;
        }
      }
    }
...
export class ChanV04Strategy implements Strategy {
  params: ChanV04StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanV04StrategyParams> = {}) {

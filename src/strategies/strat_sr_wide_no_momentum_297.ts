...
/**
 * SR Wide No Momentum 297
 * - Based on sr_no_momentum_filter_293 (best from ITERATION_13)
 * - Wider stochastic bands: oversold=25, overbought=78
 * - Goal: More trade opportunities with proven no-momentum approach
 */

export interface SRWideNoMomentum297Params extends StrategyParams {
...
const defaultParams: SRWideNoMomentum297Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRWideNoMomentum297Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_wide_no_momentum_297.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRWideNoMomentum297Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRWideNoMomentum297Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRWideNoMomentum297Strategy implements Strategy {
  params: SRWideNoMomentum297Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRWideNoMomentum297Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRWideNoMomentum297Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      // WIDE stoch bands + NO momentum filter

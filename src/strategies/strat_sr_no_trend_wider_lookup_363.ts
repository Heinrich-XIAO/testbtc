...
/**
 * SR No Trend Wider Lookup 363
 * - Based on 302 with wider support lookup (max_lookback 70)
 * - Tests if even wider historical support improves performance
 */

export interface SRNoTrendWiderLookup363Params extends StrategyParams {
...
const defaultParams: SRNoTrendWiderLookup363Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 70,
...
function loadSavedParams(): Partial<SRNoTrendWiderLookup363Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_wider_lookup_363.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendWiderLookup363Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendWiderLookup363Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendWiderLookup363Strategy implements Strategy {
  params: SRNoTrendWiderLookup363Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendWiderLookup363Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendWiderLookup363Params;
  }

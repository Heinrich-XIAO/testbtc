...
/**
 * SR No Trend Price Action 391
 * - Based on 362 with wider support lookup range
 * - Requires bullish price action confirmation: close > previous bar's high
 */

export interface SRNoTrendPriceAction391Params extends StrategyParams {
...
const defaultParams: SRNoTrendPriceAction391Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
...
function loadSavedParams(): Partial<SRNoTrendPriceAction391Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_price_action_391.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendPriceAction391Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number' || typeof value === 'boolean') params[key as keyof SRNoTrendPriceAction391Params] = value as any;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendPriceAction391Strategy implements Strategy {
  params: SRNoTrendPriceAction391Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendPriceAction391Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendPriceAction391Params;
  }
  ...
  private getOrCreateData(tokenId: string): TokenData {
    if (!this.tokenData.has(tokenId)) {
      this.tokenData.set(tokenId, { prices: [], highs: [], lows: [], kValues: [], consecutiveBounces: 0, prevHigh: 0 });
    }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const prevHigh = data.highs.length > 0 ? data.highs[data.highs.length - 1] : bar.high;
    data.prevHigh = prevHigh;
    ...
    if (position && position.size > 0) {
      ...
      const bullishClose = !this.params.require_bullish_close || bar.close > data.prevHigh;

      if (nearSupport && multiBarBounce && stochOversold && momentumOk && bullishClose) {

...
/**
 * SR Asymmetric 246
 * - Asymmetric stop/target ratios
 * - Wider profit targets relative to stop loss (1:2 or better risk/reward)
 */

export interface SRAsymmetric246Params extends StrategyParams {
  ...
  target_multiplier: number;  // profit_target = stop_loss * target_multiplier
  risk_percent: number;
}

const defaultParams: SRAsymmetric246Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRAsymmetric246Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_asymmetric_246.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRAsymmetric246Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRAsymmetric246Params] = value;
        }
      }
    }
...
export class SRAsymmetric246Strategy implements Strategy {
  params: SRAsymmetric246Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRAsymmetric246Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRAsymmetric246Params;
  }

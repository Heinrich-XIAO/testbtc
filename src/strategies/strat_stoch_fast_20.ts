...
export interface StochFastStrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochFastStrategyParams = {
  k_period: 5,
  d_period: 3,
  oversold: 20,
  overbought: 80,
...
function loadSavedParams(): Partial<StochFastStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_fast_20.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochFastStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochFastStrategyParams] = value;
        }
      }
    }
...
export class StochFastStrategy implements Strategy {
  params: StochFastStrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochFastStrategyParams> = {}) {

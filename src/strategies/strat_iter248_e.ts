...
// 60 bar hold, 40% profit, 12% stop, 60% risk, 1.5% drop
...
export interface StratIter248EParams extends StrategyParams { drop_pct: number; stop_loss: number; profit_target: number; max_hold: number; risk: number; }

export class StratIter248EStrategy extends BaseIterStrategy<StratIter248EParams> {
  constructor(params: Partial<StratIter248EParams> = {}) {
    super('strat_iter248_e.params.json', { drop_pct: 0.015, stop_loss: 0.12, profit_target: 0.4, max_hold: 60, risk: 0.6000000000000001 }, params);
  }

...
export interface MRRsiV02StrategyParams extends StrategyParams {
  ma_period: number;
  rsi_period: number;
...
const defaultParams: MRRsiV02StrategyParams = {
  ma_period: 4,
  rsi_period: 4,
...
function loadSavedParams(): Partial<MRRsiV02StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_mr_rsi_v02_55.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<MRRsiV02StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof MRRsiV02StrategyParams] = value;
        }
      }
    }
...
export class MRRsiV02Strategy implements Strategy {
  params: MRRsiV02StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private rsiMap: Map<string, RSI> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<MRRsiV02StrategyParams> = {}) {

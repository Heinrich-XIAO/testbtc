...
function variableMA(closes: number[], period: number): number | null {
  if (closes.length < period) return null;
  
  let sumPV = 0, sumV = 0;
  for (let i = closes.length - period; i < closes.length; i++) {
    const vol = Math.abs(closes[i] - closes[i - 1]);
    sumPV += closes[i] * vol;
    sumV += vol;
  }
...
export interface StratIter138EParams extends StrategyParams { vma_period: number; deviation: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; sr_lookback: number; }
export class StratIter138EStrategy extends BaseIterStrategy<StratIter138EParams> {
  constructor(params: Partial<StratIter138EParams> = {}) { super('strat_iter138_e.params.json', { vma_period: 14, deviation: -0.03, stop_loss: 0.08, profit_target: 0.18, max_hold_bars: 28, risk_percent: 0.25, sr_lookback: 50 }, params); }
  onNext(ctx: BacktestContext, bar: Bar): void {
    const { series, barNum } = this.nextBar(bar); const vma = variableMA(series.closes, this.params.vma_period); const sr = priorSupportResistance(series.highs, series.lows, this.params.sr_lookback);
    ...
    if (shouldSkipPrice(bar.close) || !sr || vma === null) return;
    const nearSupport = bar.low <= sr.support * 1.015; const belowVMA = (bar.close - vma) / vma < this.params.deviation;
    if (nearSupport && belowVMA) { this.open(ctx, bar, barNum, this.params.risk_percent); }
  }
}
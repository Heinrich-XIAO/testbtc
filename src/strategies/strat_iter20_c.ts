...
export interface StratIter20CParams extends StrategyParams {
...
const defaultParams: StratIter20CParams = {
  stoch_k_period: 14,
  stoch_d_period: 3,
  stoch_oversold: 16,
  stoch_overbought: 84,
  support_threshold: 0.008,
  resistance_threshold: 0.975,
  stop_loss: 0.08,
  profit_target: 0.18,
  max_hold_bars: 32,
  risk_percent: 0.25,
...
function loadSavedParams(): Partial<StratIter20CParams> | null {
  const paramsPath = path.join(__dirname, 'strat_iter20_c.params.json');
...
export class StratIter20CStrategy implements Strategy {
  params: StratIter20CParams;
  ...
  constructor(params: Partial<StratIter20CParams> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as StratIter20CParams;
  }

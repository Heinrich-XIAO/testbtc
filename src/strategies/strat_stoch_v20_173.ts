...
export interface StochV20StrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochV20StrategyParams = {
  k_period: 18,
  d_period: 7,
  oversold: 20,
  overbought: 80,
...
function loadSavedParams(): Partial<StochV20StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_v20_173.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochV20StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochV20StrategyParams] = value;
        }
      }
    }
...
export class StochV20Strategy implements Strategy {
  params: StochV20StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochV20StrategyParams> = {}) {

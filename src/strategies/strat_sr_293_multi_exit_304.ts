...
/**
 * SR 293 Multi Exit 304
 * - Uses 293's exact optimized params + 260's multi-exit mechanism
 * - Best parameters from iteration 13 + best exit mechanism from iteration 9
 */

export interface SR293MultiExit304Params extends StrategyParams {
  ...
  partial_profit_target: number;
  partial_exit_pct: number;
  max_hold_bars: number;
  risk_percent: number;
}

// Using 293's optimized params
const defaultParams: SR293MultiExit304Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
  volatility_period: 12,
  bounce_threshold: 0.0266,
  stoch_k_period: 14,
  stoch_d_period: 4,
  stoch_oversold: 22,
  stoch_overbought: 84,
  trend_period: 26,
  trend_threshold: -0.0094,
  min_bounce_bars: 1,
  stop_loss: 0.081,
  trailing_stop: 0.072,
  profit_target: 0.18,
  partial_profit_target: 0.06,
  partial_exit_pct: 0.5,
  max_hold_bars: 28,
  risk_percent: 0.30,
};

function loadSavedParams(): Partial<SR293MultiExit304Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_293_multi_exit_304.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SR293MultiExit304Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SR293MultiExit304Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SR293MultiExit304Strategy implements Strategy {
  params: SR293MultiExit304Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();
  private partialExitDone: Map<string, boolean> = new Map();

  constructor(params: Partial<SR293MultiExit304Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SR293MultiExit304Params;
  }
  ...
  private closePosition(ctx: BacktestContext, tokenId: string): void {
    ...
    this.partialExitDone.delete(tokenId);
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      const partialDone = this.partialExitDone.get(bar.tokenId) ?? false;
      ...
      if (entry) {
        ...
        const gain = (bar.close - entry) / entry;

        // MULTI-EXIT: Partial exit at partial_profit_target
        if (!partialDone && gain >= this.params.partial_profit_target) {
          const sellSize = position.size * this.params.partial_exit_pct;
          ctx.sell(bar.tokenId, sellSize);
          this.partialExitDone.set(bar.tokenId, true);
          return;
        }
      ...
      // Uses 293's entry logic (NO momentum filter)
      ...
      if (nearSupport && multiBarBounce && stochOversold && trendOk) {
        ...
        if (size > 0 && cash <= ctx.getCapital()) {
          ...
          if (result.success) {
            ...
            this.partialExitDone.set(bar.tokenId, false);
          }
        }
      }
    }

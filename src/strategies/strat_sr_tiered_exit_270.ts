 ...
 * - Sell 50% at first target, rest at second
 */

export interface SRTieredExit270Params extends StrategyParams {
  ...
  first_exit_pct: number;
  max_hold_bars: number;
  risk_percent: number;
}

const defaultParams: SRTieredExit270Params = {
  base_lookback: 16,
  min_lookback: 6,
  max_lookback: 35,
  volatility_period: 8,
...
function loadSavedParams(): Partial<SRTieredExit270Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_tiered_exit_270.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRTieredExit270Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRTieredExit270Params] = value;
        }
      }
    }
...
export class SRTieredExit270Strategy implements Strategy {
  params: SRTieredExit270Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();
  private firstTargetHit: Map<string, boolean> = new Map();
  private initialSize: Map<string, number> = new Map();

  constructor(params: Partial<SRTieredExit270Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRTieredExit270Params;
  }

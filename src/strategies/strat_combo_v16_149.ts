...
export interface ComboV16StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV16StrategyParams = {
  bb_period: 6,
  rsi_period: 4,
  std_mult: 2,
  rsi_oversold: 25,
...
function loadSavedParams(): Partial<ComboV16StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v16_149.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV16StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV16StrategyParams] = value;
        }
      }
    }
...
export class ComboV16Strategy implements Strategy {
  params: ComboV16StrategyParams;
  ...
  constructor(params: Partial<ComboV16StrategyParams> = {}) {

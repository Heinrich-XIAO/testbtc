...
/**
 * SR Wide Short K 284
 * - Wide stochastic bands + shorter K period (8)
 * - More responsive stochastic indicator
 */

export interface SRWideShortK284Params extends StrategyParams {
...
const defaultParams: SRWideShortK284Params = {
  base_lookback: 18,
  min_lookback: 8,
  max_lookback: 36,
  volatility_period: 10,
  bounce_threshold: 0.025,
  stoch_k_period: 8, // Shorter K period (was 14)
  stoch_d_period: 3,
...
function loadSavedParams(): Partial<SRWideShortK284Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_wide_short_k_284.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRWideShortK284Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRWideShortK284Params] = value;
        }
      }
    }
...
export class SRWideShortK284Strategy implements Strategy {
  params: SRWideShortK284Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRWideShortK284Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRWideShortK284Params;
  }

...
/**
 * SR No Trend Volume 313 - Fresh Implementation
 * - Based on 302 but adds volume filter using price velocity as proxy
 * - Only enters when current velocity >= average velocity * threshold
 */

export interface SRNoTrendVolume313Params extends StrategyParams {
...
const defaultParams: SRNoTrendVolume313Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendVolume313Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_volume_313.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendVolume313Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendVolume313Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendVolume313Strategy implements Strategy {
  params: SRNoTrendVolume313Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendVolume313Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendVolume313Params;
  }
  ...
  private getAveragePriceChange(data: TokenData, lookback: number): number {
    if (data.priceChanges.length < lookback) return 0;
    const recent = data.priceChanges.slice(-lookback);
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (data.prices.length > 1) {
      ...
      const change = (bar.close - prevPrice) / prevPrice;
      data.priceChanges.push(change);
    }
    ...
    if (position && position.size > 0) {
      ...
      const avgPriceChange = this.getAveragePriceChange(data, adaptiveLookback);
      const currentChange = data.priceChanges.length > 0 ? Math.abs(data.priceChanges[data.priceChanges.length - 1]) : 0;
      const volumeOk = currentChange >= avgPriceChange * this.params.volume_multiplier;

      if (nearSupport && multiBarBounce && stochOversold && momentumOk && volumeOk) {

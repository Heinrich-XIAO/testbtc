...
export interface StratIter149EParams extends StrategyParams {
  announcement_window: number;
  volatility_spike_mult: number;
...
const defaultParams: StratIter149EParams = {
...
function loadSavedParams(): Partial<StratIter149EParams> | null {
  const paramsPath = path.join(__dirname, 'strat_iter149_e.params.json');
...
export class StratIter149EStrategy implements Strategy {
  params: StratIter149EParams;
  ...
  private lastVolatilitySpike: Map<string, number> = new Map();

  constructor(params: Partial<StratIter149EParams> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as StratIter149EParams;
  }
  ...
  private calcLocalVolatility(closes: number[], window: number): number | null {
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    if (!this.closes.has(bar.tokenId)) {
      ...
      this.lastVolatilitySpike.set(bar.tokenId, -999);
    }
    ...
    const currentVol = this.calcLocalVolatility(closes, 5);
    const avgVol = this.calcLocalVolatility(closes.slice(0, -5), 20);
    if (currentVol !== null && avgVol !== null && currentVol > avgVol * this.params.volatility_spike_mult) {
      this.lastVolatilitySpike.set(bar.tokenId, barNum);
    }
    ...
    const lastSpike = this.lastVolatilitySpike.get(bar.tokenId) || -999;
    const withinWindow = barNum - lastSpike <= this.params.announcement_window;

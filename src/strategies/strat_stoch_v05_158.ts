...
export interface StochV05StrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochV05StrategyParams = {
  k_period: 7,
  d_period: 3,
  oversold: 25,
  overbought: 75,
...
function loadSavedParams(): Partial<StochV05StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_v05_158.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochV05StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochV05StrategyParams] = value;
        }
      }
    }
...
export class StochV05Strategy implements Strategy {
  params: StochV05StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochV05StrategyParams> = {}) {

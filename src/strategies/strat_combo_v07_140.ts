...
export interface ComboV07StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV07StrategyParams = {
  bb_period: 10,
  rsi_period: 7,
  std_mult: 2.5,
  rsi_oversold: 20,
...
function loadSavedParams(): Partial<ComboV07StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v07_140.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV07StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV07StrategyParams] = value;
        }
      }
    }
...
export class ComboV07Strategy implements Strategy {
  params: ComboV07StrategyParams;
  ...
  constructor(params: Partial<ComboV07StrategyParams> = {}) {

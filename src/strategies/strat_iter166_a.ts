...
export interface StratIter166AParams extends StrategyParams {
...
const defaultParams: StratIter166AParams = {
  min_hold_bars: 20,
  stoch_oversold: 20,
  sr_lookback_weeks: 8,
  stop_loss: 0.08,
...
function loadSavedParams(): Partial<StratIter166AParams> | null {
  const paramsPath = path.join(__dirname, 'strat_iter166_a.params.json');
...
export class StratIter166AStrategy implements Strategy {
  params: StratIter166AParams;
  ...
  constructor(params: Partial<StratIter166AParams> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as StratIter166AParams;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (entry !== undefined && entryBarNum !== undefined) {
        // KEY DIFFERENCE: Only check exits after min_hold_bars
        const barsHeld = barNum - entryBarNum;
        if (barsHeld < this.params.min_hold_bars) {
          return; // Force hold, cannot exit yet
        }

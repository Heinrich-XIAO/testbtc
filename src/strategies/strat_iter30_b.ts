...
export interface StratIter30BParams extends StrategyParams {
...
const defaultParams: StratIter30BParams = {
  stoch_k_period: 18,
  stoch_d_period: 3,
  stoch_lookback: 50,
  stoch_oversold: 14,
...
function loadSavedParams(): Partial<StratIter30BParams> | null {
  const paramsPath = path.join(__dirname, 'strat_iter30_b.params.json');
...
export class StratIter30BStrategy implements Strategy {
  params: StratIter30BParams;
  ...
  constructor(params: Partial<StratIter30BParams> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as StratIter30BParams;
  }
  ...
  private calculateRSI(history: number[], period: number): number | null {
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const rsi = this.calculateRSI(history, this.params.rsi_period);
    if (rsi !== null) {
      rsiVals.push(rsi);
      if (rsiVals.length > 100) rsiVals.shift();
    }
    ...
    if (position && position.size > 0) {
      ...
      if (kVals.length >= 2 && dVals.length >= 2 && rsiVals.length >= 1) {
        ...
        const rsiFilter = currentRSI > this.params.rsi_filter;

        const priceNearSupport = (bar.close - sr.support) / sr.support <= this.params.support_threshold;

        if (crossedAboveOversold && kAboveD && rsiFilter && priceNearSupport) {

...
/**
 * SR Moderate Looser 290
 * - Based on 281 but with even looser stochastic (oversold=22, overbought=80)
 * - Looser momentum threshold for more trades
 * - Goal: Higher trade count with acceptable win rate
 */

export interface SRModerateLooser290Params extends StrategyParams {
...
const defaultParams: SRModerateLooser290Params = {
  base_lookback: 16,
  min_lookback: 8,
  max_lookback: 40,
  volatility_period: 8,
...
function loadSavedParams(): Partial<SRModerateLooser290Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_moderate_looser_290.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRModerateLooser290Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRModerateLooser290Params] = value;
        }
      }
    }
...
export class SRModerateLooser290Strategy implements Strategy {
  params: SRModerateLooser290Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRModerateLooser290Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRModerateLooser290Params;
  }

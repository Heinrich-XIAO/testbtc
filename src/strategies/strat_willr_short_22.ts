...
/**
 * Short-period Williams %R
 */

export interface WillRShortStrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRShortStrategyParams = {
  period: 7,
  oversold_level: -80,
  overbought_level: -20,
  stop_loss: 0.05,
...
function loadSavedParams(): Partial<WillRShortStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_short_22.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRShortStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRShortStrategyParams] = value;
        }
      }
    }
...
export class WillRShortStrategy implements Strategy {
  params: WillRShortStrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRShortStrategyParams> = {}) {

...
/**
 * SR No Trend Mom Exit 386
 * - Based on 362 but exits when momentum turns negative
 * - Exits earlier on momentum loss instead of waiting for other signals
 */

export interface SRNoTrendMomExit386Params extends StrategyParams {
...
const defaultParams: SRNoTrendMomExit386Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
  ...
  max_hold_bars: 28,
  risk_percent: 0.30,
  momentum_exit_threshold: 0,
};

function loadSavedParams(): Partial<SRNoTrendMomExit386Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_mom_exit_386.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendMomExit386Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendMomExit386Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendMomExit386Strategy implements Strategy {
  params: SRNoTrendMomExit386Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendMomExit386Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendMomExit386Params;
  }
  ...
  private getOrCreateData(tokenId: string): TokenData {
    if (!this.tokenData.has(tokenId)) {
      this.tokenData.set(tokenId, { prices: [], highs: [], lows: [], kValues: [], consecutiveBounces: 0, prevMomentum: 0 });
    }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        if (bar.close < entry * (1 - this.params.stop_loss)) { this.closePosition(ctx, bar.tokenId); data.prevMomentum = momentum; return; }
        if (bar.close < newHighest * (1 - this.params.trailing_stop)) { this.closePosition(ctx, bar.tokenId); data.prevMomentum = momentum; return; }
        if (momentum < this.params.momentum_exit_threshold && data.prevMomentum >= this.params.momentum_exit_threshold) { this.closePosition(ctx, bar.tokenId); data.prevMomentum = momentum; return; }
        if (bar.close >= entry * (1 + this.params.profit_target)) { this.closePosition(ctx, bar.tokenId); data.prevMomentum = momentum; return; }
        if (bars >= this.params.max_hold_bars) { this.closePosition(ctx, bar.tokenId); data.prevMomentum = momentum; return; }

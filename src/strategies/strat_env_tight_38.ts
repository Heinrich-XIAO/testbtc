...
/**
 * Tight MA envelope mean reversion
 */

export interface EnvTightStrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvTightStrategyParams = {
  ma_period: 8,
  envelope_pct: 0.03,
  stop_loss: 0.06,
  risk_percent: 0.1,
...
function loadSavedParams(): Partial<EnvTightStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_tight_38.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvTightStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvTightStrategyParams] = value;
        }
      }
    }
...
export class EnvTightStrategy implements Strategy {
  params: EnvTightStrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvTightStrategyParams> = {}) {

...
/**
 * SR No Trend No Bounce 359
 * - Based on 302
 * - Removes bounce requirement (min_bounce_bars: 0)
 * - Keeps momentum filter
 */

export interface SRNoTrendNoBounce359Params extends StrategyParams {
...
const defaultParams: SRNoTrendNoBounce359Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendNoBounce359Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_no_bounce_359.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendNoBounce359Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendNoBounce359Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendNoBounce359Strategy implements Strategy {
  params: SRNoTrendNoBounce359Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendNoBounce359Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendNoBounce359Params;
  }

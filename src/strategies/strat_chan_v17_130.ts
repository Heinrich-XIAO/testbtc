...
export interface ChanV17StrategyParams extends StrategyParams {
...
const defaultParams: ChanV17StrategyParams = {
  channel_period: 6,
  channel_width: 0.5,
  stop_loss: 0.06,
  trailing_stop: 0.04,
...
function loadSavedParams(): Partial<ChanV17StrategyParams> | null {
  ...
  try {
    ...
    const params: Partial<ChanV17StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanV17StrategyParams] = value;
        }
      }
    }
...
export class ChanV17Strategy implements Strategy {
  params: ChanV17StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanV17StrategyParams> = {}) {

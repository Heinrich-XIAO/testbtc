...
export interface EnvV07StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV07StrategyParams = {
  ma_period: 8,
  envelope_pct: 0.05,
  stop_loss: 0.08,
  risk_percent: 0.18,
...
function loadSavedParams(): Partial<EnvV07StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_v07_100.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvV07StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV07StrategyParams] = value;
        }
      }
    }
...
export class EnvV07Strategy implements Strategy {
  params: EnvV07StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV07StrategyParams> = {}) {

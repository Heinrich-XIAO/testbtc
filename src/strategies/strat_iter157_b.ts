...
export interface StratIter157BParams extends StrategyParams {
  corr_window: number;
  short_horizon: number;
  long_horizon: number;
  corr_regime_window: number;
  min_prior_stability: number;
  corr_breakdown_threshold: number;
  max_current_corr: number;
...
export class StratIter157BStrategy extends BaseIterStrategy<StratIter157BParams> {
  constructor(params: Partial<StratIter157BParams> = {}) {
    super('strat_iter157_b.params.json', {
      corr_window: 20,
      short_horizon: 2,
      long_horizon: 8,
      corr_regime_window: 18,
      min_prior_stability: 1.6,
      corr_breakdown_threshold: 0.22,
      max_current_corr: 0.15,
      stoch_oversold: 18,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const corr = rollingCorrelation(series.shortReturns, series.longReturns, this.params.corr_window);
    if (k === null || corr === null) return;

    capPush(series.corrValues, corr);

    const regimeWindow = Math.max(8, Math.floor(this.params.corr_regime_window));
    if (series.corrValues.length < regimeWindow + 1) return;

    const priorRegime = series.corrValues.slice(-(regimeWindow + 1), -1);
    const priorMean = mean(priorRegime);
    const priorStd = stdDev(priorRegime, priorMean);
    const priorStability = Math.abs(priorMean) / (priorStd + 1e-6);

    const correlationBreakdown = priorMean - corr >= this.params.corr_breakdown_threshold;
    const currentlyDislocated = corr <= this.params.max_current_corr;
    const stablePriorRegime = priorStability >= this.params.min_prior_stability;

...
export interface StratIter169BParams extends StrategyParams {
  macd_fast: number;
  macd_slow: number;
  macd_signal: number;
  stop_loss: number;
...
export class StratIter169BStrategy implements Strategy {
  params: StratIter169BParams;
  ...
  constructor(params: Partial<StratIter169BParams> = {}) {
    const saved = loadSavedParams<StratIter169BParams>('strat_iter169_b.params.json');
    this.params = {
      macd_fast: 12,
      macd_slow: 26,
      macd_signal: 9,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const prevHist = this.prevHistogram.get(bar.tokenId);

    this.prevHistogram.set(bar.tokenId, histogram);

    if (prevHist === undefined) return;

    // Entry: Histogram turns positive (crosses above zero)

...
export interface WillRV20StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV20StrategyParams = {
  period: 18,
  oversold_level: -85,
  overbought_level: -15,
  stop_loss: 0.08,
...
function loadSavedParams(): Partial<WillRV20StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v20_93.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV20StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV20StrategyParams] = value;
        }
      }
    }
...
export class WillRV20Strategy implements Strategy {
  params: WillRV20StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV20StrategyParams> = {}) {

...
/**
 * SR Wide Multi Exit 281
 * - Wide stochastic bands from 274 + multi-exit from 260
 * - Combining best entry with best exit
 */

export interface SRWideMultiExit281Params extends StrategyParams {
...
const defaultParams: SRWideMultiExit281Params = {
  base_lookback: 16,
  min_lookback: 8,
  max_lookback: 40,
  volatility_period: 8,
...
function loadSavedParams(): Partial<SRWideMultiExit281Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_wide_multi_exit_281.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRWideMultiExit281Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRWideMultiExit281Params] = value;
        }
      }
    }
...
export class SRWideMultiExit281Strategy implements Strategy {
  params: SRWideMultiExit281Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRWideMultiExit281Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRWideMultiExit281Params;
  }

...
/**
 * SR Strict Multi Exit 260
 * - Strict entry + multiple exit conditions
 * - Combines best of both approaches
 */

export interface SRStrictMultiExit260Params extends StrategyParams {
...
const defaultParams: SRStrictMultiExit260Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRStrictMultiExit260Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_strict_multi_exit_260.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRStrictMultiExit260Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRStrictMultiExit260Params] = value;
        }
      }
    }
...
export class SRStrictMultiExit260Strategy implements Strategy {
  params: SRStrictMultiExit260Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRStrictMultiExit260Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRStrictMultiExit260Params;
  }

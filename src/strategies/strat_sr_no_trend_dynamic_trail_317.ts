...
/**
 * SR No Trend Dynamic Trail 317
 * - Based on 302 but replaces fixed trailing stop with volatility-based dynamic trailing stop
 * - Dynamic trailing stop = recent_volatility * multiplier
 * - Tests if volatility-adjusted trailing stops improve risk-adjusted returns
 */

export interface SRNoTrendDynamicTrail317Params extends StrategyParams {
...
const defaultParams: SRNoTrendDynamicTrail317Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendDynamicTrail317Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_dynamic_trail_317.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendDynamicTrail317Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendDynamicTrail317Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendDynamicTrail317Strategy implements Strategy {
  params: SRNoTrendDynamicTrail317Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendDynamicTrail317Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendDynamicTrail317Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        const recentVol = this.calcVolatility(data.prices.slice(-this.params.volatility_period));
        const dynamicTrailingStop = Math.max(0.02, Math.min(0.15, recentVol * this.params.trailing_stop_multiplier));

        if (bar.close < entry * (1 - this.params.stop_loss)) { this.closePosition(ctx, bar.tokenId); return; }
        if (bar.close < newHighest * (1 - dynamicTrailingStop)) { this.closePosition(ctx, bar.tokenId); return; }

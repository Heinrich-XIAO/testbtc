...
function tripleScreen(closes: number[], highs: number[], lows: number[], longPeriod: number, shortPeriod: number): { longTrend: number; shortOsc: number } | null {
  if (closes.length < longPeriod) return null;
  
  const longMA = sma(closes, longPeriod);
  const shortStoch = stochK(closes, highs, lows, shortPeriod);
  
  if (longMA === null || shortStoch === null) return null;
  
  const longTrend = closes[closes.length - 1] < longMA ? -1 : 1;
  
  return { longTrend, shortOsc: shortStoch };
}
...
export interface StratIter140AParams extends StrategyParams { stoch_oversold: number; long_period: number; short_period: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; sr_lookback: number; }
export class StratIter140AStrategy extends BaseIterStrategy<StratIter140AParams> {
  constructor(params: Partial<StratIter140AParams> = {}) { super('strat_iter140_a.params.json', { stoch_oversold: 20, long_period: 50, short_period: 14, stop_loss: 0.08, profit_target: 0.18, max_hold_bars: 28, risk_percent: 0.25, sr_lookback: 50 }, params); }
  onNext(ctx: BacktestContext, bar: Bar): void {
    const { series, barNum } = this.nextBar(bar); const ts = tripleScreen(series.closes, series.highs, series.lows, this.params.long_period, this.params.short_period); const sr = priorSupportResistance(series.highs, series.lows, this.params.sr_lookback);
    ...
    if (shouldSkipPrice(bar.close) || !sr || ts === null) return;
    const nearSupport = bar.low <= sr.support * 1.015;
    const buySignal = ts.longTrend > 0 && ts.shortOsc < this.params.stoch_oversold;

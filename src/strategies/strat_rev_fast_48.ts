...
/**
 * Fast reversal detector
 */

export interface RevFastStrategyParams extends StrategyParams {
  lookback: number;
...
const defaultParams: RevFastStrategyParams = {
  lookback: 5,
  drop_threshold: 0.05,
...
function loadSavedParams(): Partial<RevFastStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_rev_fast_48.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<RevFastStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof RevFastStrategyParams] = value;
        }
      }
    }
...
export class RevFastStrategy implements Strategy {
  params: RevFastStrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<RevFastStrategyParams> = {}) {

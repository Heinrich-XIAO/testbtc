...
/**
 * Tight channel mean reversion
 */

export interface ChanTightStrategyParams extends StrategyParams {
...
const defaultParams: ChanTightStrategyParams = {
  channel_period: 8,
  channel_width: 0.5,
  stop_loss: 0.05,
  trailing_stop: 0.04,
...
function loadSavedParams(): Partial<ChanTightStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_tight_50.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanTightStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanTightStrategyParams] = value;
        }
      }
    }
...
export class ChanTightStrategy implements Strategy {
  params: ChanTightStrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanTightStrategyParams> = {}) {

...
export interface WillRV01StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV01StrategyParams = {
  period: 4,
  oversold_level: -85,
  overbought_level: -15,
  stop_loss: 0.03,
...
function loadSavedParams(): Partial<WillRV01StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v01_74.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV01StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV01StrategyParams] = value;
        }
      }
    }
...
export class WillRV01Strategy implements Strategy {
  params: WillRV01StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV01StrategyParams> = {}) {

...
/**
 * SR No Trend Tight Stoch 324
 * - Based on 302
 * - Uses TIGHTER stochastic oversold (14 instead of 24) for earlier entry
 */

export interface SRNoTrendTightStoch324Params extends StrategyParams {
...
const defaultParams: SRNoTrendTightStoch324Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendTightStoch324Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_tight_stoch_324.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendTightStoch324Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendTightStoch324Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendTightStoch324Strategy implements Strategy {
  params: SRNoTrendTightStoch324Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendTightStoch324Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendTightStoch324Params;
  }

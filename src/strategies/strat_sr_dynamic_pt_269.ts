 ...
 * - Higher vol = higher profit target
 */

export interface SRDynamicPT269Params extends StrategyParams {
...
const defaultParams: SRDynamicPT269Params = {
  base_lookback: 16,
  min_lookback: 6,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRDynamicPT269Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_dynamic_pt_269.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRDynamicPT269Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRDynamicPT269Params] = value;
        }
      }
    }
...
export class SRDynamicPT269Strategy implements Strategy {
  params: SRDynamicPT269Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();
  private dynamicPT: Map<string, number> = new Map();

  constructor(params: Partial<SRDynamicPT269Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRDynamicPT269Params;
  }

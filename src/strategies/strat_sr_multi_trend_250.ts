...
/**
 * SR Multi Trend 250
 * - Combines sr_multi_exit_tweak_241's multi-exit with trend strength filter
 * - Best of both worlds: multiple exit conditions + trend confirmation
 */

export interface SRMultiTrend250Params extends StrategyParams {
...
const defaultParams: SRMultiTrend250Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRMultiTrend250Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_multi_trend_250.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRMultiTrend250Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRMultiTrend250Params] = value;
        }
      }
    }
...
export class SRMultiTrend250Strategy implements Strategy {
  params: SRMultiTrend250Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRMultiTrend250Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRMultiTrend250Params;
  }

...
/**
 * SR Stoch Wide 274
 * - Wider stochastic levels (10/90) for more extreme conditions
 * - Only enter at very oversold, exit at very overbought
 */

export interface SRStochWide274Params extends StrategyParams {
  ...
  stoch_oversold: number; // 10-15
  stoch_overbought: number; // 85-90
  trend_period: number;
...
const defaultParams: SRStochWide274Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
  ...
  stoch_k_period: 14,
  stoch_d_period: 4,
  stoch_oversold: 12, // Very wide - only extreme oversold
  stoch_overbought: 88, // Very wide - only extreme overbought
  trend_period: 22,
...
function loadSavedParams(): Partial<SRStochWide274Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_stoch_wide_274.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRStochWide274Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRStochWide274Params] = value;
        }
      }
    }
...
export class SRStochWide274Strategy implements Strategy {
  params: SRStochWide274Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRStochWide274Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRStochWide274Params;
  }

...
/**
 * SR No Trend Base 30 - 365
 * - Based on 302 but increases base_lookback from 18 to 30
 * - Tests if longer lookback for support/resistance improves returns
 */

export interface SRNoTrendBase30_365Params extends StrategyParams {
...
const defaultParams: SRNoTrendBase30_365Params = {
  base_lookback: 30,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendBase30_365Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_base_30_365.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendBase30_365Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendBase30_365Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendBase30_365Strategy implements Strategy {
  params: SRNoTrendBase30_365Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendBase30_365Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendBase30_365Params;
  }

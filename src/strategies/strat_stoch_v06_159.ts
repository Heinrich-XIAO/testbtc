...
export interface StochV06StrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochV06StrategyParams = {
  k_period: 8,
  d_period: 4,
  oversold: 20,
  overbought: 80,
...
function loadSavedParams(): Partial<StochV06StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_v06_159.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochV06StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochV06StrategyParams] = value;
        }
      }
    }
...
export class StochV06Strategy implements Strategy {
  params: StochV06StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochV06StrategyParams> = {}) {

...
// 45 bar hold, 50% profit, 12% stop, 30% risk, 2.0% drop
...
export interface StratIter247BParams extends StrategyParams { drop_pct: number; stop_loss: number; profit_target: number; max_hold: number; risk: number; }

export class StratIter247BStrategy extends BaseIterStrategy<StratIter247BParams> {
  constructor(params: Partial<StratIter247BParams> = {}) {
    super('strat_iter247_b.params.json', { drop_pct: 0.02, stop_loss: 0.12, profit_target: 0.5, max_hold: 45, risk: 0.30000000000000004 }, params);
  }

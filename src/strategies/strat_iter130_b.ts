...
export interface StratIter130BParams extends StrategyParams {
...
export class StratIter130BStrategy extends BaseIterStrategy<StratIter130BParams> {
  private prevHistogram: Map<string, number> = new Map();

  constructor(params: Partial<StratIter130BParams> = {}) {
    super('strat_iter130_b.params.json', {
      stoch_oversold: 16,
      macd_fast: 12,
      macd_slow: 26,
      macd_signal: 9,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (shouldSkipPrice(bar.close) || !sr || k === null || !macdVal) return;
    ...
    if (nearSupport && stochOversold && macdBullish) {

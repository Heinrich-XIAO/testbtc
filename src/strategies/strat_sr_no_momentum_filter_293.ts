...
/**
 * SR No Momentum Filter 293
 * - Removes momentum threshold requirement to increase trade count
 * - Keeps moderate stochastic (oversold=22, overbought=82)
 * - Focus: See if momentum filter was causing 0 trades in ITERATION_12
 */

export interface SRNoMomentumFilter293Params extends StrategyParams {
...
const defaultParams: SRNoMomentumFilter293Params = {
  base_lookback: 18,
  min_lookback: 8,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoMomentumFilter293Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_momentum_filter_293.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoMomentumFilter293Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRNoMomentumFilter293Params] = value;
        }
      }
    }
...
export class SRNoMomentumFilter293Strategy implements Strategy {
  params: SRNoMomentumFilter293Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoMomentumFilter293Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoMomentumFilter293Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      // NO MOMENTUM FILTER - just support/stoch/trend

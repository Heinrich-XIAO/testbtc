...
/**
 * SR No Trend No Time Limit 368
 * - Based on 302 but removes max hold time limit entirely
 * - Tests if time limit was capping returns
 */

export interface SRNoTrendNoTimeLimit368Params extends StrategyParams {
...
const defaultParams: SRNoTrendNoTimeLimit368Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendNoTimeLimit368Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_no_time_limit_368.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendNoTimeLimit368Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendNoTimeLimit368Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendNoTimeLimit368Strategy implements Strategy {
  params: SRNoTrendNoTimeLimit368Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendNoTimeLimit368Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendNoTimeLimit368Params;
  }

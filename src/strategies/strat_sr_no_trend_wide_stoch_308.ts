...
/**
 * SR No Trend Wide Stoch 308
 * - Based on 302 but with wider stochastic range (20/90)
 * - Tests if wider stoch allows better entries
 */

export interface SRNoTrendWideStoch308Params extends StrategyParams {
...
const defaultParams: SRNoTrendWideStoch308Params = {
  base_lookback: 20,
  min_lookback: 14,
  max_lookback: 36,
  ...
  stoch_k_period: 18,
  stoch_d_period: 5,
  stoch_oversold: 28, // Wider than 302's 24
  stoch_overbought: 90, // Wider than 302's 86
  momentum_period: 3,
...
function loadSavedParams(): Partial<SRNoTrendWideStoch308Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_wide_stoch_308.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendWideStoch308Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendWideStoch308Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendWideStoch308Strategy implements Strategy {
  params: SRNoTrendWideStoch308Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendWideStoch308Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendWideStoch308Params;
  }

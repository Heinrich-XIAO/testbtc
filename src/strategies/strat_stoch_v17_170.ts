...
export interface StochV17StrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochV17StrategyParams = {
  k_period: 7,
  d_period: 3,
  oversold: 20,
  overbought: 80,
...
function loadSavedParams(): Partial<StochV17StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_v17_170.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochV17StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochV17StrategyParams] = value;
        }
      }
    }
...
export class StochV17Strategy implements Strategy {
  params: StochV17StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochV17StrategyParams> = {}) {

...
/**
 * SR No Trend Combo2 369
 * - Based on 302
 * - max_lookback = 50 (wider lookup)
 * - bounce_threshold = 0.035 (wider bounce)
 */

export interface SRNoTrendCombo2369Params extends StrategyParams {
...
const defaultParams: SRNoTrendCombo2369Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
...
function loadSavedParams(): Partial<SRNoTrendCombo2369Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_combo2_369.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendCombo2369Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendCombo2369Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendCombo2369Strategy implements Strategy {
  params: SRNoTrendCombo2369Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendCombo2369Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendCombo2369Params;
  }

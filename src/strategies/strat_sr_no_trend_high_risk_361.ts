...
/**
 * SR No Trend High Risk 361
 * - Based on 302 but with higher risk_percent (0.50 instead of 0.30)
 */

export interface SRNoTrendHighRisk361Params extends StrategyParams {
...
const defaultParams: SRNoTrendHighRisk361Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendHighRisk361Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_high_risk_361.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendHighRisk361Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendHighRisk361Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendHighRisk361Strategy implements Strategy {
  params: SRNoTrendHighRisk361Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendHighRisk361Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendHighRisk361Params;
  }

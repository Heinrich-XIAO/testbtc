...
function kaufmanEfficiency(closes: number[], period: number): number | null {
  if (closes.length < period + 1) return null;
  
  let change = Math.abs(closes[closes.length - 1] - closes[closes.length - period - 1]);
  let volatility = 0;
...
export interface StratIter138DParams extends StrategyParams { er_low: number; er_period: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; sr_lookback: number; }
export class StratIter138DStrategy extends BaseIterStrategy<StratIter138DParams> {
  constructor(params: Partial<StratIter138DParams> = {}) { super('strat_iter138_d.params.json', { er_low: 0.3, er_period: 10, stop_loss: 0.08, profit_target: 0.18, max_hold_bars: 28, risk_percent: 0.25, sr_lookback: 50 }, params); }
  onNext(ctx: BacktestContext, bar: Bar): void {
    const { series, barNum } = this.nextBar(bar); const er = kaufmanEfficiency(series.closes, this.params.er_period); const sr = priorSupportResistance(series.highs, series.lows, this.params.sr_lookback);
    ...
    const nearSupport = bar.low <= sr.support * 1.015; const erLow = er < this.params.er_low;
    if (nearSupport && erLow) { this.open(ctx, bar, barNum, this.params.risk_percent); }
  }
}
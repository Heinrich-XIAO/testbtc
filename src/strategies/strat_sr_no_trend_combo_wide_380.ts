...
/**
 * SR No Trend Combo Wide 380
 * - Based on 302
 * - max_lookback = 50 (was 36)
 * - bounce_threshold = 0.040 (was 0.028)
 */

export interface SRNoTrendComboWide380Params extends StrategyParams {
...
const defaultParams: SRNoTrendComboWide380Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
...
function loadSavedParams(): Partial<SRNoTrendComboWide380Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_combo_wide_380.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendComboWide380Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendComboWide380Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendComboWide380Strategy implements Strategy {
  params: SRNoTrendComboWide380Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendComboWide380Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendComboWide380Params;
  }

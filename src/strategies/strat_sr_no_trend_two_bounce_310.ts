...
/**
 * SR No Trend Two Bounce 310
 * - Based on 302 but requires 2 consecutive bounce bars
 * - Tests if stricter bounce confirmation improves quality
 */

export interface SRNoTrendTwoBounce310Params extends StrategyParams {
...
const defaultParams: SRNoTrendTwoBounce310Params = {
  base_lookback: 20,
  min_lookback: 14,
  max_lookback: 36,
  ...
  min_bounce_bars: 2, // Stricter than 302's 1
  stop_loss: 0.065,
  trailing_stop: 0.07,
  profit_target: 0.14,
  max_hold_bars: 32,
...
function loadSavedParams(): Partial<SRNoTrendTwoBounce310Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_two_bounce_310.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendTwoBounce310Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendTwoBounce310Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendTwoBounce310Strategy implements Strategy {
  params: SRNoTrendTwoBounce310Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendTwoBounce310Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendTwoBounce310Params;
  }

...
/**
 * SR No Trend Slow Stoch 360
 * - Based on 302
 * - Wider stochastic K period (28 instead of 14) for smoother signals
 */

export interface SRSlowStoch360Params extends StrategyParams {
...
const defaultParams: SRSlowStoch360Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRSlowStoch360Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_slow_stoch_360.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRSlowStoch360Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRSlowStoch360Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRSlowStoch360Strategy implements Strategy {
  params: SRSlowStoch360Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRSlowStoch360Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRSlowStoch360Params;
  }

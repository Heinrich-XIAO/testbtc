...
function elderRay(closes: number[], highs: number[], lows: number[], period: number): { bullPower: number; bearPower: number } | null {
  if (closes.length < period) return null;
  
  const ema = sma(closes, period);
  if (ema === null) return null;
  
  const bullPower = highs[highs.length - 1] - ema;
  const bearPower = lows[lows.length - 1] - ema;
...
export interface StratIter140BParams extends StrategyParams { bear_threshold: number; er_period: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; sr_lookback: number; }
export class StratIter140BStrategy extends BaseIterStrategy<StratIter140BParams> {
  constructor(params: Partial<StratIter140BParams> = {}) { super('strat_iter140_b.params.json', { bear_threshold: -0.02, er_period: 13, stop_loss: 0.08, profit_target: 0.18, max_hold_bars: 28, risk_percent: 0.25, sr_lookback: 50 }, params); }
  onNext(ctx: BacktestContext, bar: Bar): void {
    const { series, barNum } = this.nextBar(bar); const er = elderRay(series.closes, series.highs, series.lows, this.params.er_period); const sr = priorSupportResistance(series.highs, series.lows, this.params.sr_lookback);
    ...
    if (shouldSkipPrice(bar.close) || !sr || er === null) return;
    const nearSupport = bar.low <= sr.support * 1.015;
    const buySignal = er.bearPower < this.params.bear_threshold;

...
export interface StochV13StrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochV13StrategyParams = {
  k_period: 6,
  d_period: 3,
  oversold: 22,
  overbought: 78,
...
function loadSavedParams(): Partial<StochV13StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_v13_166.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochV13StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochV13StrategyParams] = value;
        }
      }
    }
...
export class StochV13Strategy implements Strategy {
  params: StochV13StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochV13StrategyParams> = {}) {

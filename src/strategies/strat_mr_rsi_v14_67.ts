...
export interface MRRsiV14StrategyParams extends StrategyParams {
  ma_period: number;
  rsi_period: number;
...
const defaultParams: MRRsiV14StrategyParams = {
  ma_period: 7,
  rsi_period: 6,
...
function loadSavedParams(): Partial<MRRsiV14StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_mr_rsi_v14_67.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<MRRsiV14StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof MRRsiV14StrategyParams] = value;
        }
      }
    }
...
export class MRRsiV14Strategy implements Strategy {
  params: MRRsiV14StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private rsiMap: Map<string, RSI> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<MRRsiV14StrategyParams> = {}) {

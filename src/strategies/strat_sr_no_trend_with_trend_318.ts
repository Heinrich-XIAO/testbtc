...
export interface SRNoTrendWithTrend318Params extends StrategyParams {
...
const defaultParams: SRNoTrendWithTrend318Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendWithTrend318Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_with_trend_318.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendWithTrend318Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendWithTrend318Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendWithTrend318Strategy implements Strategy {
  params: SRNoTrendWithTrend318Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendWithTrend318Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendWithTrend318Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const maxPeriod = Math.max(this.params.max_lookback + 20, this.params.trend_ema_period + 5);
    ...
    const trendEMA = this.calcEMA(data.prices, this.params.trend_ema_period);
    
    if (!stoch || supports.length === 0 || trendEMA === null) return;
    ...
    if (position && position.size > 0) {
      ...
      const priceAboveTrend = bar.close > trendEMA;
      ...
      if (nearSupport && multiBarBounce && stochOversold && momentumOk && priceAboveTrend) {

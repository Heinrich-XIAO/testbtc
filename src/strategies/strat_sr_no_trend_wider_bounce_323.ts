...
/**
 * SR No Trend Wider Bounce 323
 * - Based on 302 but requires MORE bounce bars (3 instead of 1)
 * - Filters false signals by requiring sustained upward movement
 */

export interface SRNoTrendWiderBounce323Params extends StrategyParams {
...
const defaultParams: SRNoTrendWiderBounce323Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendWiderBounce323Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_wider_bounce_323.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendWiderBounce323Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendWiderBounce323Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendWiderBounce323Strategy implements Strategy {
  params: SRNoTrendWiderBounce323Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendWiderBounce323Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendWiderBounce323Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      // NO TREND filter - uses loose momentum instead

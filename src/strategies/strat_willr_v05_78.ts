...
export interface WillRV05StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV05StrategyParams = {
  period: 8,
  oversold_level: -85,
  overbought_level: -15,
  stop_loss: 0.07,
...
function loadSavedParams(): Partial<WillRV05StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v05_78.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV05StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV05StrategyParams] = value;
        }
      }
    }
...
export class WillRV05Strategy implements Strategy {
  params: WillRV05StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV05StrategyParams> = {}) {

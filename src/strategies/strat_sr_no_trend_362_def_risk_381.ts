...
/**
 * SR No Trend 362 Def Risk 381
 * - Based on 302
 * - Test 362: max_lookback=50
 * - Uses default risk_percent (0.30, not saved 0.324)
 */

export interface SRNoTrend362DefRisk381Params extends StrategyParams {
...
const defaultParams: SRNoTrend362DefRisk381Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
...
function loadSavedParams(): Partial<SRNoTrend362DefRisk381Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_362_def_risk_381.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrend362DefRisk381Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrend362DefRisk381Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrend362DefRisk381Strategy implements Strategy {
  params: SRNoTrend362DefRisk381Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrend362DefRisk381Params> = {}) {
    const savedParams = loadSavedParams();
    const mergedParams = { ...defaultParams, ...params };
    if (savedParams) {
      delete (mergedParams as any).risk_percent;
      this.params = { ...mergedParams, ...savedParams } as SRNoTrend362DefRisk381Params;
    } else {
      this.params = mergedParams as SRNoTrend362DefRisk381Params;
    }
  }

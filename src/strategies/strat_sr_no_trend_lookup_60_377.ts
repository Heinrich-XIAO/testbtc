...
/**
 * SR No Trend Lookup 60 377
 * - Based on 362 but tests max_lookback 60 (between 50 and 70)
 */

export interface SRNoTrendLookup60377Params extends StrategyParams {
...
const defaultParams: SRNoTrendLookup60377Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 60,
...
function loadSavedParams(): Partial<SRNoTrendLookup60377Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_lookup_60_377.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendLookup60377Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendLookup60377Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendLookup60377Strategy implements Strategy {
  params: SRNoTrendLookup60377Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendLookup60377Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendLookup60377Params;
  }

...
export interface StochV04StrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochV04StrategyParams = {
  k_period: 5,
  d_period: 3,
  oversold: 15,
  overbought: 85,
...
function loadSavedParams(): Partial<StochV04StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_v04_157.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochV04StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochV04StrategyParams] = value;
        }
      }
    }
...
export class StochV04Strategy implements Strategy {
  params: StochV04StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochV04StrategyParams> = {}) {

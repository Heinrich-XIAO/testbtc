...
/**
 * SR Multi Exit Tweak 241
 * - Tweak of sr_adaptive_multi_exit_234 (best: $2,828)
 * - Adjusted profit target and max hold bars
 * - Tighter stop loss
 */

export interface SRMultiExitTweak241Params extends StrategyParams {
...
const defaultParams: SRMultiExitTweak241Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRMultiExitTweak241Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_multi_exit_tweak_241.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRMultiExitTweak241Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRMultiExitTweak241Params] = value;
        }
      }
    }
...
export class SRMultiExitTweak241Strategy implements Strategy {
  params: SRMultiExitTweak241Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRMultiExitTweak241Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRMultiExitTweak241Params;
  }

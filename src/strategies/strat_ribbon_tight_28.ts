...
/**
 * Tight MA ribbon for trend detection
 */

export interface RibbonTightStrategyParams extends StrategyParams {
...
const defaultParams: RibbonTightStrategyParams = {
  shortest_period: 3,
  period_step: 2,
  num_mas: 4,
  stop_loss: 0.05,
...
function loadSavedParams(): Partial<RibbonTightStrategyParams> | null {
  ...
  try {
    ...
    const params: Partial<RibbonTightStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof RibbonTightStrategyParams] = value;
        }
      }
    }
...
export class RibbonTightStrategy implements Strategy {
  params: RibbonTightStrategyParams;
  private maArrays: Map<string, SimpleMovingAverage[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<RibbonTightStrategyParams> = {}) {

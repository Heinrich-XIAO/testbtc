...
export interface WillRV08StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV08StrategyParams = {
  period: 12,
  oversold_level: -90,
  overbought_level: -10,
  stop_loss: 0.08,
...
function loadSavedParams(): Partial<WillRV08StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v08_81.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV08StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV08StrategyParams] = value;
        }
      }
    }
...
export class WillRV08Strategy implements Strategy {
  params: WillRV08StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV08StrategyParams> = {}) {

...
/**
 * SR No Trend VWAP 338
 * - Based on 302
 * - Adds VWAP filter: only enter when price is below VWAP (undervalued)
 */

export interface SRNoTrendVwap338Params extends StrategyParams {
...
const defaultParams: SRNoTrendVwap338Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendVwap338Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_vwap_338.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendVwap338Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendVwap338Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendVwap338Strategy implements Strategy {
  params: SRNoTrendVwap338Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendVwap338Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendVwap338Params;
  }
  ...
  private getOrCreateData(tokenId: string): TokenData {
    if (!this.tokenData.has(tokenId)) {
      this.tokenData.set(tokenId, { prices: [], highs: [], lows: [], kValues: [], consecutiveBounces: 0, typicalPrices: [] });
    }
  ...
  private getVwap(data: TokenData): number | null {
    if (data.typicalPrices.length < this.params.vwap_period) return null;
    const recent = data.typicalPrices.slice(-this.params.vwap_period);
    let sumTpv = 0;
    let sumWeight = 0;
    for (let i = 0; i < recent.length; i++) {
      const weight = i + 1;
      sumTpv += recent[i] * weight;
      sumWeight += weight;
    }
    return sumTpv / sumWeight;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    data.typicalPrices.push(typicalPrice);
    ...
    if (position && position.size > 0) {
      ...
      const belowVwap = vwap === null || bar.close < vwap;

      if (nearSupport && multiBarBounce && stochOversold && momentumOk && belowVwap) {

...
export interface ChanV15StrategyParams extends StrategyParams {
...
const defaultParams: ChanV15StrategyParams = {
  channel_period: 7,
  channel_width: 0.5,
  stop_loss: 0.02,
  trailing_stop: 0.02,
...
function loadSavedParams(): Partial<ChanV15StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_v15_128.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanV15StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanV15StrategyParams] = value;
        }
      }
    }
...
export class ChanV15Strategy implements Strategy {
  params: ChanV15StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanV15StrategyParams> = {}) {

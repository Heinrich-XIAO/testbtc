...
export interface ChanV08StrategyParams extends StrategyParams {
...
const defaultParams: ChanV08StrategyParams = {
  channel_period: 12,
  channel_width: 0.4,
  stop_loss: 0.05,
  trailing_stop: 0.04,
...
function loadSavedParams(): Partial<ChanV08StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_v08_121.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanV08StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanV08StrategyParams] = value;
        }
      }
    }
...
export class ChanV08Strategy implements Strategy {
  params: ChanV08StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanV08StrategyParams> = {}) {

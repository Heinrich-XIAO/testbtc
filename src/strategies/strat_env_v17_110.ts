...
export interface EnvV17StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV17StrategyParams = {
  ma_period: 7,
  envelope_pct: 0.03,
  stop_loss: 0.06,
  risk_percent: 0.3,
...
function loadSavedParams(): Partial<EnvV17StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_v17_110.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvV17StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV17StrategyParams] = value;
        }
      }
    }
...
export class EnvV17Strategy implements Strategy {
  params: EnvV17StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV17StrategyParams> = {}) {

...
/**
 * Medium EMA crossover with balanced risk
 */

export interface EMAMedCrossStrategyParams extends StrategyParams {
  fast_period: number;
  slow_period: number;
  stop_loss: number;
...
const defaultParams: EMAMedCrossStrategyParams = {
  fast_period: 5,
  slow_period: 15,
  stop_loss: 0.05,
  trailing_stop: 0.04,
...
function loadSavedParams(): Partial<EMAMedCrossStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_ema_med_12.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EMAMedCrossStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EMAMedCrossStrategyParams] = value;
        }
      }
    }
...
export class EMAMedCrossStrategy implements Strategy {
  params: EMAMedCrossStrategyParams;
  ...
  constructor(params: Partial<EMAMedCrossStrategyParams> = {}) {

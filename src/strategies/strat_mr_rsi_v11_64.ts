...
export interface MRRsiV11StrategyParams extends StrategyParams {
  ma_period: number;
  rsi_period: number;
...
const defaultParams: MRRsiV11StrategyParams = {
  ma_period: 3,
  rsi_period: 3,
...
function loadSavedParams(): Partial<MRRsiV11StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_mr_rsi_v11_64.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<MRRsiV11StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof MRRsiV11StrategyParams] = value;
        }
      }
    }
...
export class MRRsiV11Strategy implements Strategy {
  params: MRRsiV11StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private rsiMap: Map<string, RSI> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<MRRsiV11StrategyParams> = {}) {

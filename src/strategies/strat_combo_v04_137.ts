...
export interface ComboV04StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV04StrategyParams = {
  bb_period: 6,
  rsi_period: 5,
  std_mult: 2.5,
  rsi_oversold: 20,
...
function loadSavedParams(): Partial<ComboV04StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v04_137.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV04StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV04StrategyParams] = value;
        }
      }
    }
...
export class ComboV04Strategy implements Strategy {
  params: ComboV04StrategyParams;
  ...
  constructor(params: Partial<ComboV04StrategyParams> = {}) {

...
/**
 * SR No Trend Tight Stop 328
 * - Based on 302
 * - Tighter stop loss: 0.04 instead of 0.08
 */

export interface SRNoTrendTightStop328Params extends StrategyParams {
...
const defaultParams: SRNoTrendTightStop328Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendTightStop328Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_tight_stop_328.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendTightStop328Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendTightStop328Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendTightStop328Strategy implements Strategy {
  params: SRNoTrendTightStop328Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendTightStop328Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendTightStop328Params;
  }

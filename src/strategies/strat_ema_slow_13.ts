...
/**
 * Slow EMA crossover for longer trends
 */

export interface EMASlowCrossStrategyParams extends StrategyParams {
  fast_period: number;
  slow_period: number;
  stop_loss: number;
...
const defaultParams: EMASlowCrossStrategyParams = {
  fast_period: 8,
  slow_period: 20,
  stop_loss: 0.06,
  trailing_stop: 0.05,
...
function loadSavedParams(): Partial<EMASlowCrossStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_ema_slow_13.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EMASlowCrossStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EMASlowCrossStrategyParams] = value;
        }
      }
    }
...
export class EMASlowCrossStrategy implements Strategy {
  params: EMASlowCrossStrategyParams;
  ...
  constructor(params: Partial<EMASlowCrossStrategyParams> = {}) {

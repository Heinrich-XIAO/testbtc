...
/**
 * SR Minimal Filters 298
 * - Based on 293 but REMOVES trend filter too
 * - Only requires: support bounce + stochastic oversold
 * - Goal: Maximum trade count while keeping core logic
 */

export interface SRMinimalFilters298Params extends StrategyParams {
...
const defaultParams: SRMinimalFilters298Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRMinimalFilters298Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_minimal_filters_298.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRMinimalFilters298Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRMinimalFilters298Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRMinimalFilters298Strategy implements Strategy {
  params: SRMinimalFilters298Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRMinimalFilters298Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRMinimalFilters298Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      // MINIMAL FILTERS: Only support + stoch oversold + bounce
      // NO momentum, NO trend filter

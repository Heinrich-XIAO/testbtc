...
export interface WillRV12StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV12StrategyParams = {
  period: 14,
  oversold_level: -90,
  overbought_level: -10,
  stop_loss: 0.04,
...
function loadSavedParams(): Partial<WillRV12StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v12_85.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV12StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV12StrategyParams] = value;
        }
      }
    }
...
export class WillRV12Strategy implements Strategy {
  params: WillRV12StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV12StrategyParams> = {}) {

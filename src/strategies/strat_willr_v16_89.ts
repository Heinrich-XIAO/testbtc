...
export interface WillRV16StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV16StrategyParams = {
  period: 7,
  oversold_level: -80,
  overbought_level: -20,
  stop_loss: 0.12,
...
function loadSavedParams(): Partial<WillRV16StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v16_89.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV16StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV16StrategyParams] = value;
        }
      }
    }
...
export class WillRV16Strategy implements Strategy {
  params: WillRV16StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV16StrategyParams> = {}) {

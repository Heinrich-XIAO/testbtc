...
/**
 * SR Multi Exit Short Hold 266
 * - Tweak of sr_strict_multi_exit_260 ($4,414 best)
 * - Shorter max hold bars for faster turnover
 */

export interface SRMultiExitShortHold266Params extends StrategyParams {
...
const defaultParams: SRMultiExitShortHold266Params = {
  base_lookback: 16,
  min_lookback: 6,
  max_lookback: 35,
  volatility_period: 8,
...
function loadSavedParams(): Partial<SRMultiExitShortHold266Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_multi_exit_short_hold_266.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRMultiExitShortHold266Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRMultiExitShortHold266Params] = value;
        }
      }
    }
...
export class SRMultiExitShortHold266Strategy implements Strategy {
  params: SRMultiExitShortHold266Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRMultiExitShortHold266Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRMultiExitShortHold266Params;
  }

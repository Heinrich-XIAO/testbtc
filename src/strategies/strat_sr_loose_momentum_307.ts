...
/**
 * SR Loose Momentum 307
 * - Based on 302 but with looser momentum threshold
 * - Tests if more permissive momentum allows more trades
 */

export interface SRLooseMomentum307Params extends StrategyParams {
...
const defaultParams: SRLooseMomentum307Params = {
  base_lookback: 20,
  min_lookback: 14,
  max_lookback: 36,
  ...
  stoch_overbought: 86,
  momentum_period: 3,
  momentum_threshold: 0.002, // Looser than 302's 0.006
  min_bounce_bars: 1,
  stop_loss: 0.065,
  trailing_stop: 0.07,
  profit_target: 0.14,
...
function loadSavedParams(): Partial<SRLooseMomentum307Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_loose_momentum_307.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRLooseMomentum307Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRLooseMomentum307Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRLooseMomentum307Strategy implements Strategy {
  params: SRLooseMomentum307Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRLooseMomentum307Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRLooseMomentum307Params;
  }

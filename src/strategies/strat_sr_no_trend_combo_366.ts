...
/**
 * SR No Trend Combo 366
 * - Based on 302 (no trend filter)
 * - Wider lookup (max_lookback = 50) like 362
 * - No momentum filter (like 315)
 */

export interface SRNoTrendCombo366Params extends StrategyParams {
...
const defaultParams: SRNoTrendCombo366Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
...
function loadSavedParams(): Partial<SRNoTrendCombo366Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_combo_366.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendCombo366Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendCombo366Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendCombo366Strategy implements Strategy {
  params: SRNoTrendCombo366Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendCombo366Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendCombo366Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      // NO momentum filter (315 approach) - only stoch oversold and bounce

...
/**
 * SR No Trend Tight Stoch 309
 * - Based on 302 but with tighter stochastic range (18/80)
 * - Tests if tighter stoch improves entry quality
 */

export interface SRNoTrendTightStoch309Params extends StrategyParams {
...
const defaultParams: SRNoTrendTightStoch309Params = {
  base_lookback: 20,
  min_lookback: 14,
  max_lookback: 36,
  ...
  stoch_k_period: 18,
  stoch_d_period: 5,
  stoch_oversold: 18, // Tighter than 302's 24
  stoch_overbought: 80, // Tighter than 302's 86
  momentum_period: 3,
...
function loadSavedParams(): Partial<SRNoTrendTightStoch309Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_tight_stoch_309.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendTightStoch309Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendTightStoch309Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendTightStoch309Strategy implements Strategy {
  params: SRNoTrendTightStoch309Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendTightStoch309Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendTightStoch309Params;
  }

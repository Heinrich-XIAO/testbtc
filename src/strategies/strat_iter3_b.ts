...
export interface StratIter3BParams extends StrategyParams {
  ...
  retest_lookback: number;
  retest_tolerance: number;
}

const defaultParams: StratIter3BParams = {
  stoch_k_period: 14,
  stoch_oversold: 14,
  stoch_overbought: 82,
...
function loadSavedParams(): Partial<StratIter3BParams> | null {
  const paramsPath = path.join(__dirname, 'strat_iter3_b.params.json');
...
export class StratIter3BStrategy implements Strategy {
  params: StratIter3BParams;
  ...
  constructor(params: Partial<StratIter3BParams> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as StratIter3BParams;
  }
  ...
  private hasRetestedSupport(history: number[], lows: number[], support: number, tolerance: number): boolean {
    const lookback = Math.min(this.params.retest_lookback, lows.length - 1);
      ...
      if (kVals.length >= 2) {
        ...
        if (prevK < this.params.stoch_oversold && currK >= this.params.stoch_oversold) {
          ...
          if (momentum !== null && momentum > this.params.momentum_threshold) {
            ...
            if (nearSupport) {
              const retested = this.hasRetestedSupport(history, lows, sr.support, this.params.retest_tolerance);

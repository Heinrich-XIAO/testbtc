...
/**
 * SR Moderate Tighter 291
 * - Based on 281 but with moderate-tight stochastic (oversold=18, overbought=84)
 * - Tighter trailing stop for faster profit taking
 * - Goal: Better balance between trade frequency and quality
 */

export interface SRModerateTighter291Params extends StrategyParams {
...
const defaultParams: SRModerateTighter291Params = {
  base_lookback: 18,
  min_lookback: 8,
  max_lookback: 38,
  volatility_period: 10,
  bounce_threshold: 0.026,
  stoch_k_period: 14,
  stoch_d_period: 3,
  stoch_oversold: 18,
  stoch_overbought: 84,
...
function loadSavedParams(): Partial<SRModerateTighter291Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_moderate_tighter_291.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRModerateTighter291Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRModerateTighter291Params] = value;
        }
      }
    }
...
export class SRModerateTighter291Strategy implements Strategy {
  params: SRModerateTighter291Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRModerateTighter291Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRModerateTighter291Params;
  }

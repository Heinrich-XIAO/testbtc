...
/**
 * SR No Trend Momentum Only 352
 * - Ultra-simple: ONLY stochastic oversold + momentum positive
 * - NO support level check
 * - NO multi-bar bounce requirement
 * - Tests if support/bounce requirements were limiting returns
 */

export interface SRNoTrendMomentumOnly352Params extends StrategyParams {
...
const defaultParams: SRNoTrendMomentumOnly352Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendMomentumOnly352Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_momentum_only_352.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendMomentumOnly352Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendMomentumOnly352Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendMomentumOnly352Strategy implements Strategy {
  params: SRNoTrendMomentumOnly352Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendMomentumOnly352Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendMomentumOnly352Params;
  }
  ...
  private getOrCreateData(tokenId: string): TokenData {
    if (!this.tokenData.has(tokenId)) {
      this.tokenData.set(tokenId, { prices: [], highs: [], lows: [], kValues: [] });
    }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const resistance = this.findResistanceLevel(data, this.params.base_lookback);

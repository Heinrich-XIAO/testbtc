...
/**
 * SR No Trend Tight Bounce 354
 * - Based on 302
 * - TIGHTER bounce threshold (0.015 vs 0.028) - only enter when VERY close to support
 */

export interface SRNoTrendTightBounce354Params extends StrategyParams {
...
const defaultParams: SRNoTrendTightBounce354Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendTightBounce354Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_tight_bounce_354.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendTightBounce354Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendTightBounce354Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendTightBounce354Strategy implements Strategy {
  params: SRNoTrendTightBounce354Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendTightBounce354Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendTightBounce354Params;
  }

...
/**
 * SR No Trend Multi Exit 333
 * - Based on 302
 * - Adds multiple exit conditions (already present)
 * - Adds time-based exit: exit after max_hold_bars even if not profitable
 * - Adds BE exit: if bars held > (max_hold_bars / 2), exit at break-even
 */

export interface SRNoTrendMultiExit333Params extends StrategyParams {
...
const defaultParams: SRNoTrendMultiExit333Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendMultiExit333Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_multi_exit_333.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendMultiExit333Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendMultiExit333Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendMultiExit333Strategy implements Strategy {
  params: SRNoTrendMultiExit333Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendMultiExit333Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendMultiExit333Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        const stopLossExit = bar.close < entry * (1 - this.params.stop_loss);
        const trailingStopExit = bar.close < newHighest * (1 - this.params.trailing_stop);
        const profitTargetExit = bar.close >= entry * (1 + this.params.profit_target);
        const maxBarsExit = bars >= this.params.max_hold_bars;
        const resistanceExit = resistance !== null && bar.close >= resistance;
        const stochOverboughtExit = stoch.k >= this.params.stoch_overbought;
        const beExit = bars > Math.floor(this.params.max_hold_bars / 2) && bar.close >= entry;

        if (stopLossExit || trailingStopExit || profitTargetExit || maxBarsExit || resistanceExit || stochOverboughtExit || beExit) {

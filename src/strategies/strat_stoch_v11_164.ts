...
export interface StochV11StrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochV11StrategyParams = {
  k_period: 3,
  d_period: 2,
  oversold: 25,
  overbought: 75,
...
function loadSavedParams(): Partial<StochV11StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_v11_164.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochV11StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochV11StrategyParams] = value;
        }
      }
    }
...
export class StochV11Strategy implements Strategy {
  params: StochV11StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochV11StrategyParams> = {}) {

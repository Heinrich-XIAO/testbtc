...
export interface StochV07StrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochV07StrategyParams = {
  k_period: 10,
  d_period: 4,
  oversold: 15,
  overbought: 85,
...
function loadSavedParams(): Partial<StochV07StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_v07_160.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochV07StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochV07StrategyParams] = value;
        }
      }
    }
...
export class StochV07Strategy implements Strategy {
  params: StochV07StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochV07StrategyParams> = {}) {

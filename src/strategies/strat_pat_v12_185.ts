...
export interface PatV12StrategyParams extends StrategyParams {
  consec_bars: number;
  exit_bars: number;
  stop_loss: number;
...
const defaultParams: PatV12StrategyParams = {
  consec_bars: 4,
  exit_bars: 3,
  stop_loss: 0.04,
  trailing_stop: 0.03,
...
function loadSavedParams(): Partial<PatV12StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_pat_v12_185.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<PatV12StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof PatV12StrategyParams] = value;
        }
      }
    }
...
export class PatV12Strategy implements Strategy {
  params: PatV12StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<PatV12StrategyParams> = {}) {

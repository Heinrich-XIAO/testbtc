...
export interface EnvV18StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV18StrategyParams = {
  ma_period: 7,
  envelope_pct: 0.03,
  stop_loss: 0.06,
  risk_percent: 0.05,
...
function loadSavedParams(): Partial<EnvV18StrategyParams> | null {
  ...
  try {
    ...
    const params: Partial<EnvV18StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV18StrategyParams] = value;
        }
      }
    }
...
export class EnvV18Strategy implements Strategy {
  params: EnvV18StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV18StrategyParams> = {}) {

...
export interface ChanV03StrategyParams extends StrategyParams {
...
const defaultParams: ChanV03StrategyParams = {
  channel_period: 5,
  channel_width: 0.5,
  stop_loss: 0.05,
  trailing_stop: 0.04,
...
function loadSavedParams(): Partial<ChanV03StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_v03_116.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanV03StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanV03StrategyParams] = value;
        }
      }
    }
...
export class ChanV03Strategy implements Strategy {
  params: ChanV03StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanV03StrategyParams> = {}) {

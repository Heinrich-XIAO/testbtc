...
export interface MRRsiV01StrategyParams extends StrategyParams {
  ma_period: number;
  rsi_period: number;
...
const defaultParams: MRRsiV01StrategyParams = {
  ma_period: 3,
  rsi_period: 3,
...
function loadSavedParams(): Partial<MRRsiV01StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_mr_rsi_v01_54.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<MRRsiV01StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof MRRsiV01StrategyParams] = value;
        }
      }
    }
...
export class MRRsiV01Strategy implements Strategy {
  params: MRRsiV01StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private rsiMap: Map<string, RSI> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<MRRsiV01StrategyParams> = {}) {

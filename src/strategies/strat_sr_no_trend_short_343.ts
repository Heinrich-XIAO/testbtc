...
/**
 * SR No Trend Short 343
 * - CONTRARIAN: Buy at resistance (opposite of 302 which buys at support)
 * - Enter LONG when price bounces DOWN from resistance
 * - This is a mean-reversion play expecting reversal from resistance
 */

export interface SRNoTrendShort343Params extends StrategyParams {
...
const defaultParams: SRNoTrendShort343Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendShort343Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_short_343.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendShort343Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendShort343Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendShort343Strategy implements Strategy {
  params: SRNoTrendShort343Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendShort343Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendShort343Params;
  }
  ...
  private getOrCreateData(tokenId: string): TokenData {
    if (!this.tokenData.has(tokenId)) {
      this.tokenData.set(tokenId, { prices: [], highs: [], lows: [], kValues: [], consecutiveDrops: 0 });
    }
  ...
  private findResistanceLevels(data: TokenData, lookback: number): number[] {
    if (data.highs.length < lookback) return [];
    const recentHighs = data.highs.slice(-lookback);
    const sorted = [...recentHighs].sort((a, b) => b - a);
    return sorted.slice(0, 3);
  }

  private findSupportLevel(data: TokenData, lookback: number): number | null {
    if (data.lows.length < lookback) return null;
    return Math.min(...data.lows.slice(-lookback));
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (bar.close < prevPrice) data.consecutiveDrops++;
    else data.consecutiveDrops = 0;
    ...
    const resistances = this.findResistanceLevels(data, adaptiveLookback);
    const support = this.findSupportLevel(data, adaptiveLookback);
    
    if (!stoch || resistances.length === 0) return;
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        if ((support !== null && bar.close <= support) || stoch.k <= this.params.stoch_oversold) {
          this.closePosition(ctx, bar.tokenId);
        }
      }
    } else if (bar.close > 0.05 && bar.close < 0.95) {
      const nearResistance = resistances.some(r => Math.abs(bar.close - r) / r < this.params.bounce_threshold);
      const momentumOk = momentum >= this.params.momentum_threshold;
      const multiBarDrop = data.consecutiveDrops >= this.params.min_bounce_bars;

      if (nearResistance && multiBarDrop && momentumOk) {

...
/**
 * SR No Trend Multi Exit 305
 * - Combines 302's no-trend-filter with multi-exit mechanism
 * - Uses partial profit taking like 260
 */

export interface SRNoTrendMultiExit305Params extends StrategyParams {
...
const defaultParams: SRNoTrendMultiExit305Params = {
  base_lookback: 20,
  min_lookback: 14,
  max_lookback: 36,
  ...
  trailing_stop: 0.07,
  profit_target: 0.14,
  partial_profit_target: 0.07,
  partial_exit_pct: 0.4,
  max_hold_bars: 32,
  risk_percent: 0.32,
};

function loadSavedParams(): Partial<SRNoTrendMultiExit305Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_multi_exit_305.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendMultiExit305Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendMultiExit305Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendMultiExit305Strategy implements Strategy {
  params: SRNoTrendMultiExit305Params;
  ...
  constructor(params: Partial<SRNoTrendMultiExit305Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendMultiExit305Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        // Partial exit at first profit target
        if (!this.partialExitDone.get(bar.tokenId) && bar.close >= entry * (1 + this.params.partial_profit_target)) {

...
/**
 * SR No Mom Multi Exit 299
 * - Combines 293's no-momentum approach with 260's multi-exit mechanism
 * - Best of both: trade frequency + multiple exit paths
 */

export interface SRNoMomMultiExit299Params extends StrategyParams {
...
const defaultParams: SRNoMomMultiExit299Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
  volatility_period: 12,
  bounce_threshold: 0.027,
  stoch_k_period: 14,
  stoch_d_period: 4,
  stoch_oversold: 22,
  stoch_overbought: 84,
  trend_period: 26,
  trend_threshold: -0.009,
  min_bounce_bars: 1,
  stop_loss: 0.08,
  trailing_stop: 0.07,
  profit_target: 0.18,
  partial_profit_target: 0.08,
  partial_exit_pct: 0.5,
  max_hold_bars: 30,
  risk_percent: 0.30,
};

function loadSavedParams(): Partial<SRNoMomMultiExit299Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_mom_multi_exit_299.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoMomMultiExit299Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoMomMultiExit299Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoMomMultiExit299Strategy implements Strategy {
  params: SRNoMomMultiExit299Params;
  ...
  constructor(params: Partial<SRNoMomMultiExit299Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoMomMultiExit299Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        // Partial exit at partial_profit_target

...
/**
 * Mean reversion + RSI with wide thresholds
 */

export interface MRRSIWideStrategyParams extends StrategyParams {
  ma_period: number;
  rsi_period: number;
...
const defaultParams: MRRSIWideStrategyParams = {
  ma_period: 10,
  rsi_period: 7,
...
function loadSavedParams(): Partial<MRRSIWideStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_mr_rsi_wide_33.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<MRRSIWideStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof MRRSIWideStrategyParams] = value;
        }
      }
    }
...
export class MRRSIWideStrategy implements Strategy {
  params: MRRSIWideStrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private rsiMap: Map<string, RSI> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<MRRSIWideStrategyParams> = {}) {

...
/**
 * SR Tight Momentum 306
 * - Based on 302 but with tighter momentum threshold
 * - Tests if stricter momentum improves test returns
 */

export interface SRTightMomentum306Params extends StrategyParams {
...
const defaultParams: SRTightMomentum306Params = {
  base_lookback: 20,
  min_lookback: 14,
  max_lookback: 36,
  ...
  stoch_overbought: 86,
  momentum_period: 3,
  momentum_threshold: 0.012, // Tighter than 302's 0.006
  min_bounce_bars: 1,
  stop_loss: 0.065,
  trailing_stop: 0.07,
  profit_target: 0.14,
...
function loadSavedParams(): Partial<SRTightMomentum306Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_tight_momentum_306.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRTightMomentum306Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRTightMomentum306Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRTightMomentum306Strategy implements Strategy {
  params: SRTightMomentum306Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRTightMomentum306Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRTightMomentum306Params;
  }

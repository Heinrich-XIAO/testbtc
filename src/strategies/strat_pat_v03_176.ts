...
export interface PatV03StrategyParams extends StrategyParams {
  consec_bars: number;
  exit_bars: number;
  stop_loss: number;
...
const defaultParams: PatV03StrategyParams = {
  consec_bars: 3,
  exit_bars: 1,
  stop_loss: 0.04,
  trailing_stop: 0.03,
...
function loadSavedParams(): Partial<PatV03StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_pat_v03_176.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<PatV03StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof PatV03StrategyParams] = value;
        }
      }
    }
...
export class PatV03Strategy implements Strategy {
  params: PatV03StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<PatV03StrategyParams> = {}) {

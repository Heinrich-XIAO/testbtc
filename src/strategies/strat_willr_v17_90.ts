...
export interface WillRV17StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV17StrategyParams = {
  period: 6,
  oversold_level: -85,
  overbought_level: -15,
  stop_loss: 0.06,
...
function loadSavedParams(): Partial<WillRV17StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v17_90.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV17StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV17StrategyParams] = value;
        }
      }
    }
...
export class WillRV17Strategy implements Strategy {
  params: WillRV17StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV17StrategyParams> = {}) {

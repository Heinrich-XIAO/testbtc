...
/**
 * Buy on consecutive dips (reversal)
 */

export interface PatDipStrategyParams extends StrategyParams {
  consec_bars: number;
  buy_on_dip: number;
  exit_bars: number;
  stop_loss: number;
...
const defaultParams: PatDipStrategyParams = {
  consec_bars: 3,
  buy_on_dip: 1,
  exit_bars: 2,
  stop_loss: 0.06,
...
function loadSavedParams(): Partial<PatDipStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_pat_dip_40.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<PatDipStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof PatDipStrategyParams] = value;
        }
      }
    }
...
export class PatDipStrategy implements Strategy {
  params: PatDipStrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<PatDipStrategyParams> = {}) {

...
/**
 * SR Trend Strict 249
 * - Tweak of sr_trend_strength_247 (best: $3,274)
 * - Stricter trend filter (positive trend only)
 * - Adjusted trailing stop
 */

export interface SRTrendStrict249Params extends StrategyParams {
...
const defaultParams: SRTrendStrict249Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
  ...
  stoch_overbought: 70,
  trend_period: 25,
  trend_threshold: 0.01,  // Require positive trend
  stop_loss: 0.08,
  trailing_stop: 0.05,
  risk_percent: 0.15,
};

function loadSavedParams(): Partial<SRTrendStrict249Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_trend_strict_249.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRTrendStrict249Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRTrendStrict249Params] = value;
        }
      }
    }
...
export class SRTrendStrict249Strategy implements Strategy {
  params: SRTrendStrict249Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRTrendStrict249Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRTrendStrict249Params;
  }

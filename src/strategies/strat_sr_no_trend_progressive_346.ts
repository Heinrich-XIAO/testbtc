...
/**
 * SR No Trend Progressive 346
 * - Based on 302
 * - Progressive stop loss: move to breakeven after reaching 50% of profit target
 */

export interface SRNoTrendProgressive346Params extends StrategyParams {
...
const defaultParams: SRNoTrendProgressive346Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendProgressive346Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_progressive_346.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendProgressive346Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendProgressive346Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendProgressive346Strategy implements Strategy {
  params: SRNoTrendProgressive346Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();
  private movedToBreakeven: Map<string, boolean> = new Map();

  constructor(params: Partial<SRNoTrendProgressive346Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendProgressive346Params;
  }
  ...
  private closePosition(ctx: BacktestContext, tokenId: string): void {
    ...
    this.movedToBreakeven.delete(tokenId);
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      const movedToBE = this.movedToBreakeven.get(bar.tokenId) ?? false;
      ...
      if (entry) {
        ...
        const profitThreshold = entry * (1 + this.params.profit_target * 0.5);
        const profitTarget = entry * (1 + this.params.profit_target);

        if (!movedToBE && bar.close >= profitThreshold) {
          this.movedToBreakeven.set(bar.tokenId, true);
        }

        const currentStop = movedToBE ? entry : entry * (1 - this.params.stop_loss);

        if (bar.close < currentStop) { this.closePosition(ctx, bar.tokenId); return; }
        if (bar.close < newHighest * (1 - this.params.trailing_stop)) { this.closePosition(ctx, bar.tokenId); return; }
        if (bar.close >= profitTarget) { this.closePosition(ctx, bar.tokenId); return; }
      ...
      if (nearSupport && multiBarBounce && stochOversold && momentumOk) {
        ...
        if (size > 0 && cash <= ctx.getCapital()) {
          ...
          if (result.success) {
            ...
            this.movedToBreakeven.set(bar.tokenId, false);
          }
        }
      }
    }

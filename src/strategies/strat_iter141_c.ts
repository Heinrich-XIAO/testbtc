...
export interface StratIter141CParams extends StrategyParams {
  cci_period: number;
  cci_oversold: number;
  atr_period: number;
...
export class StratIter141CStrategy extends BaseIterStrategy<StratIter141CParams> {
  constructor(params: Partial<StratIter141CParams> = {}) {
    super('strat_iter141_c.params.json', {
      cci_period: 20,
      cci_oversold: -100,
      atr_period: 14,
      atr_multiplier: 1.5,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (shouldSkipPrice(bar.close) || !sr || cciVal === null || atrVal === null || k === null) return;

    const nearSupport = bar.low <= sr.support * 1.015;
    const cciOversold = cciVal < this.params.cci_oversold;
    const lowVolatility = bar.close <= sr.support + atrVal * this.params.atr_multiplier;
    const stochOversold = k < this.params.stoch_oversold;

    if (nearSupport && cciOversold && lowVolatility && stochOversold) {

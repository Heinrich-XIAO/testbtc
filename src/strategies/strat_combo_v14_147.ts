...
export interface ComboV14StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV14StrategyParams = {
  bb_period: 9,
  rsi_period: 6,
  std_mult: 2.2,
  rsi_oversold: 26,
...
function loadSavedParams(): Partial<ComboV14StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v14_147.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV14StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV14StrategyParams] = value;
        }
      }
    }
...
export class ComboV14Strategy implements Strategy {
  params: ComboV14StrategyParams;
  ...
  constructor(params: Partial<ComboV14StrategyParams> = {}) {

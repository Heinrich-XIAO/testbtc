...
/**
 * SR No Trend Strong Momentum 348
 * - Based on 302
 * - Requires HIGHER momentum (0.01 instead of 0.004) for stronger bounce quality
 */

export interface SRNoTrendStrongMomentum348Params extends StrategyParams {
...
const defaultParams: SRNoTrendStrongMomentum348Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendStrongMomentum348Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_strong_momentum_348.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendStrongMomentum348Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendStrongMomentum348Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendStrongMomentum348Strategy implements Strategy {
  params: SRNoTrendStrongMomentum348Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendStrongMomentum348Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendStrongMomentum348Params;
  }

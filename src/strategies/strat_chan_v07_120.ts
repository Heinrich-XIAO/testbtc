...
export interface ChanV07StrategyParams extends StrategyParams {
...
const defaultParams: ChanV07StrategyParams = {
  channel_period: 8,
  channel_width: 0.8,
  stop_loss: 0.08,
  trailing_stop: 0.06,
...
function loadSavedParams(): Partial<ChanV07StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_v07_120.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanV07StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanV07StrategyParams] = value;
        }
      }
    }
...
export class ChanV07Strategy implements Strategy {
  params: ChanV07StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanV07StrategyParams> = {}) {

...
export interface StochSlowStrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochSlowStrategyParams = {
  k_period: 10,
  d_period: 5,
  oversold: 25,
  overbought: 75,
...
function loadSavedParams(): Partial<StochSlowStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_slow_21.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochSlowStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochSlowStrategyParams] = value;
        }
      }
    }
...
export class StochSlowStrategy implements Strategy {
  params: StochSlowStrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochSlowStrategyParams> = {}) {

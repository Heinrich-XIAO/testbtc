...
export interface ComboV09StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV09StrategyParams = {
  bb_period: 12,
  rsi_period: 8,
  std_mult: 2,
  rsi_oversold: 25,
...
function loadSavedParams(): Partial<ComboV09StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v09_142.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV09StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV09StrategyParams] = value;
        }
      }
    }
...
export class ComboV09Strategy implements Strategy {
  params: ComboV09StrategyParams;
  ...
  constructor(params: Partial<ComboV09StrategyParams> = {}) {

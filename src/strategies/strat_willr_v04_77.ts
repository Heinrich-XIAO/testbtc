...
export interface WillRV04StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV04StrategyParams = {
  period: 8,
  oversold_level: -80,
  overbought_level: -20,
  stop_loss: 0.05,
...
function loadSavedParams(): Partial<WillRV04StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v04_77.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV04StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV04StrategyParams] = value;
        }
      }
    }
...
export class WillRV04Strategy implements Strategy {
  params: WillRV04StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV04StrategyParams> = {}) {

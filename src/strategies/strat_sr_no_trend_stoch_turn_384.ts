...
/**
 * SR No Trend Stoch Turn 384
 * - Based on 362 but requires K to be TURNING UP from oversold
 * - Catches actual reversal moment, not just oversold condition
 */

export interface SRNoTrendStochTurn384Params extends StrategyParams {
...
const defaultParams: SRNoTrendStochTurn384Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
...
function loadSavedParams(): Partial<SRNoTrendStochTurn384Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_stoch_turn_384.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendStochTurn384Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendStochTurn384Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendStochTurn384Strategy implements Strategy {
  params: SRNoTrendStochTurn384Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendStochTurn384Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendStochTurn384Params;
  }
  ...
  private getOrCreateData(tokenId: string): TokenData {
    if (!this.tokenData.has(tokenId)) {
      this.tokenData.set(tokenId, { prices: [], highs: [], lows: [], kValues: [], prevKValue: 50, consecutiveBounces: 0 });
    }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (!stoch || supports.length === 0) {
      if (stoch) data.prevKValue = stoch.k;
      return;
    }
    ...
    if (position && position.size > 0) {
      ...
      const stochOversold = stoch.k <= this.params.stoch_oversold && stoch.k > stoch.d && stoch.k > data.prevKValue;

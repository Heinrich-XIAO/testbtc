...
export interface EnvV02StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV02StrategyParams = {
  ma_period: 5,
  envelope_pct: 0.015,
  stop_loss: 0.04,
  risk_percent: 0.1,
...
function loadSavedParams(): Partial<EnvV02StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_v02_95.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvV02StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV02StrategyParams] = value;
        }
      }
    }
...
export class EnvV02Strategy implements Strategy {
  params: EnvV02StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV02StrategyParams> = {}) {

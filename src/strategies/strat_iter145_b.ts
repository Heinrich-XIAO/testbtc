...
export interface StratIter145BParams extends StrategyParams {
...
export class StratIter145BStrategy extends BaseIterStrategy<StratIter145BParams> {
  constructor(params: Partial<StratIter145BParams> = {}) {
    super('strat_iter145_b.params.json', {
      swing_lookback: 3,
      stoch_oversold: 16,
      stop_loss: 0.08,
      profit_target: 0.18,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (shouldSkipPrice(bar.close) || k === null || series.swingLows.length < 2) return;

    const stochOversold = k < this.params.stoch_oversold;
    
    const recentLows = series.swingLows.slice(-2);
    const higherLow = recentLows.length === 2 && recentLows[1].price > recentLows[0].price;

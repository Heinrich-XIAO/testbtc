...
export interface StratIter202CParams extends StrategyParams { drop_pct: number; stop_loss: number; profit_target: number; max_hold: number; risk: number; }
export class StratIter202CStrategy extends BaseIterStrategy<StratIter202CParams> {
  constructor(params: Partial<StratIter202CParams> = {}) {
    super('strat_iter202_c.params.json', { drop_pct: 0.02, stop_loss: 0.08, profit_target: 0.18, max_hold: 15, risk: 0.25 }, params);
  }
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (pos && pos.size > 0) {
      ...
      if (bar.low <= e * (1 - this.params.stop_loss) || bar.high >= e * (1 + this.params.profit_target) || barNum - eb >= this.params.max_hold) this.close(ctx, bar.tokenId);
      return;
    }
    const curr = series.closes[series.closes.length - 1]; const prev = series.closes[series.closes.length - 2];
    const drop = prev - curr; if (drop > 0 && drop / prev > this.params.drop_pct) this.open(ctx, bar, barNum, this.params.risk);
  }
}

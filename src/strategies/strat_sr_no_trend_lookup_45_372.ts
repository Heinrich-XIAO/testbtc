...
/**
 * SR No Trend Lookup 45 - 372
 * - Based on 302 (max_lookback 36)
 * - Tests max_lookback 45 (between 36 and 50)
 */

export interface SRNoTrendLookup45Params extends StrategyParams {
...
const defaultParams: SRNoTrendLookup45Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 45,
...
function loadSavedParams(): Partial<SRNoTrendLookup45Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_lookup_45_372.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendLookup45Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendLookup45Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendLookup45Strategy implements Strategy {
  params: SRNoTrendLookup45Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendLookup45Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendLookup45Params;
  }

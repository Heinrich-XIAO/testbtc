...
function tradingSystem1(closes: number[], maPeriod: number, rsiPeriod: number): { trend: number; momentum: number } | null {
  if (closes.length < Math.max(maPeriod, rsiPeriod) + 1) return null;
  
  const ma = sma(closes, maPeriod);
  const rsiVal = rsi(closes, rsiPeriod);
  
  if (ma === null || rsiVal === null) return null;
...
export interface StratIter140CParams extends StrategyParams { rsi_oversold: number; ma_period: number; rsi_period: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; sr_lookback: number; }
export class StratIter140CStrategy extends BaseIterStrategy<StratIter140CParams> {
  constructor(params: Partial<StratIter140CParams> = {}) { super('strat_iter140_c.params.json', { rsi_oversold: 30, ma_period: 20, rsi_period: 14, stop_loss: 0.08, profit_target: 0.18, max_hold_bars: 28, risk_percent: 0.25, sr_lookback: 50 }, params); }
  onNext(ctx: BacktestContext, bar: Bar): void {
    const { series, barNum } = this.nextBar(bar); const ts1 = tradingSystem1(series.closes, this.params.ma_period, this.params.rsi_period); const sr = priorSupportResistance(series.highs, series.lows, this.params.sr_lookback);
    ...
    if (shouldSkipPrice(bar.close) || !sr || ts1 === null) return;
    const nearSupport = bar.low <= sr.support * 1.015;
    const buySignal = ts1.trend > 0 && ts1.momentum < this.params.rsi_oversold;

...
/**
 * Slow price-MA crossover
 */

export interface MCrossSlowStrategyParams extends StrategyParams {
  ma_period: number;
  stop_loss: number;
...
const defaultParams: MCrossSlowStrategyParams = {
  ma_period: 12,
  stop_loss: 0.07,
  trailing_stop: 0.05,
  risk_percent: 0.15,
...
function loadSavedParams(): Partial<MCrossSlowStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_mcross_slow_53.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<MCrossSlowStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof MCrossSlowStrategyParams] = value;
        }
      }
    }
...
export class MCrossSlowStrategy implements Strategy {
  params: MCrossSlowStrategyParams;
  ...
  constructor(params: Partial<MCrossSlowStrategyParams> = {}) {

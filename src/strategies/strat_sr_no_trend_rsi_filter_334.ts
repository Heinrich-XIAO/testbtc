...
/**
 * SR No Trend RSI Filter 334
 * - Based on 302 but adds RSI as additional filter
 * - Only enter when RSI is ALSO oversold (RSI < 40)
 * - Dual confirmation: stochastic oversold + RSI oversold
 */

export interface SRNoTrendRSIFilter334Params extends StrategyParams {
...
const defaultParams: SRNoTrendRSIFilter334Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendRSIFilter334Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_rsi_filter_334.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendRSIFilter334Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendRSIFilter334Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendRSIFilter334Strategy implements Strategy {
  params: SRNoTrendRSIFilter334Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendRSIFilter334Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendRSIFilter334Params;
  }
  ...
  private getOrCreateData(tokenId: string): TokenData {
    if (!this.tokenData.has(tokenId)) {
      this.tokenData.set(tokenId, { prices: [], highs: [], lows: [], kValues: [], consecutiveBounces: 0, rsiValues: [] });
    }
  ...
  private getRSI(data: TokenData): number | null {
    ...
    const period = this.params.rsi_period;
    let gains = 0;
    let losses = 0;
    
    for (let i = data.prices.length - period; i < data.prices.length; i++) {
      const change = data.prices[i] - data.prices[i - 1];
    ...
    if (data.rsiValues.length > period) data.rsiValues.shift();
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (!stoch || supports.length === 0 || rsi === null) return;
    ...
    if (position && position.size > 0) {
      ...
      if (nearSupport && multiBarBounce && stochOversold && momentumOk && rsiOversold) {

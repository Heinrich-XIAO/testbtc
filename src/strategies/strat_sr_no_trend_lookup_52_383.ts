...
/**
 * SR No Trend Lookup 383
 * - Based on 382 but with max_lookback=52
 */

export interface SRNoTrendLookup383Params extends StrategyParams {
...
const defaultParams: SRNoTrendLookup383Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 52,
...
function loadSavedParams(): Partial<SRNoTrendLookup383Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_lookup_383.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendLookup383Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendLookup383Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendLookup383Strategy implements Strategy {
  params: SRNoTrendLookup383Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendLookup383Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendLookup383Params;
  }

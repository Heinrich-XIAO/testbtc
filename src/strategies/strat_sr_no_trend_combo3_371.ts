...
/**
 * SR No Trend Combo3 371
 * - Based on 302
 * - Combines wider lookup (50) with stoch_k_period=18 (insight from ITERATION_19)
 */

export interface SRSRNoTrendCombo3371Params extends StrategyParams {
...
const defaultParams: SRSRNoTrendCombo3371Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
...
function loadSavedParams(): Partial<SRSRNoTrendCombo3371Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_combo3_371.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRSRNoTrendCombo3371Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRSRNoTrendCombo3371Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRSRNoTrendCombo3371Strategy implements Strategy {
  params: SRSRNoTrendCombo3371Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRSRNoTrendCombo3371Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRSRNoTrendCombo3371Params;
  }

...
export interface ChanV19StrategyParams extends StrategyParams {
...
const defaultParams: ChanV19StrategyParams = {
  channel_period: 3,
  channel_width: 0.2,
  stop_loss: 0.03,
  trailing_stop: 0.02,
...
function loadSavedParams(): Partial<ChanV19StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_v19_132.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanV19StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanV19StrategyParams] = value;
        }
      }
    }
...
export class ChanV19Strategy implements Strategy {
  params: ChanV19StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanV19StrategyParams> = {}) {

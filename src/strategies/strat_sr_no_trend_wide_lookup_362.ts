...
/**
 * SR No Trend Wide Lookup 362
 * - Based on 302 but with wider support lookup range
 * - Uses max_lookback 50 instead of 36
 */

export interface SRNoTrendWideLookup362Params extends StrategyParams {
...
const defaultParams: SRNoTrendWideLookup362Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
...
function loadSavedParams(): Partial<SRNoTrendWideLookup362Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_wide_lookup_362.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendWideLookup362Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendWideLookup362Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendWideLookup362Strategy implements Strategy {
  params: SRNoTrendWideLookup362Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendWideLookup362Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendWideLookup362Params;
  }

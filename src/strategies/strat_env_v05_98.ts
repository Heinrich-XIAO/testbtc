...
export interface EnvV05StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV05StrategyParams = {
  ma_period: 8,
  envelope_pct: 0.02,
  stop_loss: 0.04,
  risk_percent: 0.1,
...
function loadSavedParams(): Partial<EnvV05StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_v05_98.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvV05StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV05StrategyParams] = value;
        }
      }
    }
...
export class EnvV05Strategy implements Strategy {
  params: EnvV05StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV05StrategyParams> = {}) {

...
export interface EnvV14StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV14StrategyParams = {
  ma_period: 9,
  envelope_pct: 0.035,
  stop_loss: 0.06,
  risk_percent: 0.14,
...
function loadSavedParams(): Partial<EnvV14StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_v14_107.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvV14StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV14StrategyParams] = value;
        }
      }
    }
...
export class EnvV14Strategy implements Strategy {
  params: EnvV14StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV14StrategyParams> = {}) {

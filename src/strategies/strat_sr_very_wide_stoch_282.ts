...
/**
 * SR Very Wide Stoch 282
 * - Even wider stochastic bands (10/92)
 */

export interface SRVeryWideStoch282Params extends StrategyParams {
  base_lookback: number; min_lookback: number; max_lookback: number; volatility_period: number;
  bounce_threshold: number; stoch_k_period: number; stoch_d_period: number;
  stoch_oversold: number; stoch_overbought: number; trend_period: number; trend_threshold: number;
  momentum_period: number; momentum_threshold: number; min_bounce_bars: number;
  stop_loss: number; trailing_stop: number; profit_target: number; max_hold_bars: number; risk_percent: number;
}

const defaultParams: SRVeryWideStoch282Params = {
  base_lookback: 16, min_lookback: 8, max_lookback: 40, volatility_period: 8,
  bounce_threshold: 0.028, stoch_k_period: 12, stoch_d_period: 3,
  stoch_oversold: 10, stoch_overbought: 92, trend_period: 24, trend_threshold: -0.007,
  momentum_period: 2, momentum_threshold: 0.0075, min_bounce_bars: 1,
  stop_loss: 0.075, trailing_stop: 0.045, profit_target: 0.14, max_hold_bars: 45, risk_percent: 0.26,
};

function loadSavedParams(): Partial<SRVeryWideStoch282Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_very_wide_stoch_282.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRVeryWideStoch282Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams && typeof value === 'number') {
        params[key as keyof SRVeryWideStoch282Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
interface TokenData { prices: number[]; highs: number[]; lows: number[]; kValues: number[]; consecutiveBounces: number; }

export class SRVeryWideStoch282Strategy implements Strategy {
  params: SRVeryWideStoch282Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRVeryWideStoch282Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRVeryWideStoch282Params;
  }
  ...
  private getAdaptiveLookback(data: TokenData): number {
    ...
    return Math.max(this.params.min_lookback, Math.min(this.params.max_lookback, Math.round(this.params.base_lookback / (recentVol / historicalVol))));
  }
  ...
  private getMomentum(prices: number[]): number {
    ...
    return (prices[prices.length - 1] - prices[prices.length - 1 - this.params.momentum_period]) / prices[prices.length - 1 - this.params.momentum_period];
  }

  private findSupportLevels(data: TokenData, lookback: number): number[] {
    if (data.lows.length < lookback) return [];
    return [...data.lows.slice(-lookback)].sort((a, b) => a - b).slice(0, 3);
  }
  ...
  private getStochastic(data: TokenData): { k: number; d: number } | null {
    ...
    return { k, d: data.kValues.reduce((a, b) => a + b, 0) / data.kValues.length };
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    data.prices.push(bar.close); data.highs.push(bar.high); data.lows.push(bar.low);
    const maxPeriod = Math.max(this.params.max_lookback, this.params.trend_period) + 20;
    if (data.prices.length > maxPeriod) { data.prices.shift(); data.highs.shift(); data.lows.shift(); }
    const prevPrice = data.prices.length > 1 ? data.prices[data.prices.length - 2] : bar.close;
    if (bar.close > prevPrice) data.consecutiveBounces++; else data.consecutiveBounces = 0;
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        ...
        if ((resistance !== null && bar.close >= resistance) || stoch.k >= this.params.stoch_overbought) this.closePosition(ctx, bar.tokenId);
      }
      ...
      const trendOk = this.getTrendStrength(data.prices) >= this.params.trend_threshold;
      const momentumOk = this.getMomentum(data.prices) >= this.params.momentum_threshold;
      ...
      if (nearSupport && multiBarBounce && stochOversold && trendOk && momentumOk) {
        ...
        if (size > 0 && cash <= ctx.getCapital()) {
          ...
          if (result.success) { this.entryPrice.set(bar.tokenId, bar.close); this.highestPrice.set(bar.tokenId, bar.close); this.barsHeld.set(bar.tokenId, 0); }
        }
      }
    }
  }

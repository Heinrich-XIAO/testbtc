...
export interface ComboV06StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV06StrategyParams = {
  bb_period: 8,
  rsi_period: 6,
  std_mult: 2,
  rsi_oversold: 25,
...
function loadSavedParams(): Partial<ComboV06StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v06_139.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV06StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV06StrategyParams] = value;
        }
      }
    }
...
export class ComboV06Strategy implements Strategy {
  params: ComboV06StrategyParams;
  ...
  constructor(params: Partial<ComboV06StrategyParams> = {}) {

...
/**
 * SR No Trend ATR Filter 337
 * - Based on 302 but adds ATR filter: only enter when ATR < average ATR * 0.95
 * - Result: ATR filter HURTS performance - fewer trades, lower return
 */

export interface SRNoTrendATRFilter337Params extends StrategyParams {
  ...
  risk_percent: number;
  atr_period: number;
  atr_filter_mult: number;
}

const defaultParams: SRNoTrendATRFilter337Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
  ...
  risk_percent: 0.30,
  atr_period: 14,
  atr_filter_mult: 0.95,
};

function loadSavedParams(): Partial<SRNoTrendATRFilter337Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_atr_filter_337.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendATRFilter337Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendATRFilter337Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendATRFilter337Strategy implements Strategy {
  params: SRNoTrendATRFilter337Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendATRFilter337Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendATRFilter337Params;
  }
  ...
  private getOrCreateData(tokenId: string): TokenData {
    if (!this.tokenData.has(tokenId)) {
      this.tokenData.set(tokenId, { prices: [], highs: [], lows: [], kValues: [], consecutiveBounces: 0, trueRanges: [] });
    }
  ...
  private getATR(data: TokenData): { current: number; average: number } | null {
    if (data.trueRanges.length < this.params.atr_period) return null;
    const recentTR = data.trueRanges.slice(-this.params.atr_period);
    const currentATR = recentTR[recentTR.length - 1];
    const averageATR = recentTR.reduce((a, b) => a + b, 0) / recentTR.length;
    return { current: currentATR, average: averageATR };
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (data.prices.length > 1) {
      const prevClose = data.prices[data.prices.length - 2];
      const tr = Math.max(
        bar.high - bar.low,
      ...
      if (data.trueRanges.length > this.params.atr_period + 10) data.trueRanges.shift();
    }
    ...
    if (position && position.size > 0) {
      ...
      const atrFilterOk = !atr || atr.current < atr.average * this.params.atr_filter_mult;

      if (nearSupport && multiBarBounce && stochOversold && momentumOk && atrFilterOk) {

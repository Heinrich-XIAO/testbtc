...
export interface StratIter130DParams extends StrategyParams {
  ma_fast: number;
  ma_slow: number;
  macd_fast: number;
  macd_slow: number;
...
export class StratIter130DStrategy extends BaseIterStrategy<StratIter130DParams> {
  private prevHistogram: Map<string, number> = new Map();

  constructor(params: Partial<StratIter130DParams> = {}) {
    super('strat_iter130_d.params.json', {
      ma_fast: 10,
      ma_slow: 30,
      macd_fast: 12,
      macd_slow: 26,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const fastMa = sma(series.closes, this.params.ma_fast);
    const slowMa = sma(series.closes, this.params.ma_slow);
    const macdVal = macd(series.closes, this.params.macd_fast, this.params.macd_slow, this.params.macd_signal);
    const sr = priorSupportResistance(series.highs, series.lows, this.params.sr_lookback);

    if (macdVal) {
      this.prevHistogram.set(bar.tokenId, macdVal.histogram);
    }
    ...
    if (shouldSkipPrice(bar.close) || !sr || !fastMa || !slowMa || !macdVal) return;

    const nearSupport = bar.low <= sr.support * 1.015;
    const maBullish = fastMa > slowMa;
    const prevH = this.prevHistogram.get(bar.tokenId);
    const macdBullish = macdVal.histogram > 0 || (prevH !== undefined && prevH < 0 && macdVal.histogram > prevH);

    if (nearSupport && maBullish && macdBullish) {

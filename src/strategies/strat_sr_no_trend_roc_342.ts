 ...
 * - Adds ROC(5) filter: only enter when ROC < 0 (price declining = better entry)
 */

export interface SRNoTrendRoc342Params extends StrategyParams {
...
const defaultParams: SRNoTrendRoc342Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendRoc342Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_roc_342.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendRoc342Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendRoc342Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendRoc342Strategy implements Strategy {
  params: SRNoTrendRoc342Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendRoc342Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendRoc342Params;
  }
  ...
  private getRoc(prices: number[]): number {
    if (prices.length < this.params.roc_period + 1) return 0;
    const current = prices[prices.length - 1];
    const past = prices[prices.length - 1 - this.params.roc_period];
    return (current - past) / past;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const roc = this.getRoc(data.prices);
    ...
    if (position && position.size > 0) {
      ...
      const rocNegative = roc < 0;

      if (nearSupport && multiBarBounce && stochOversold && momentumOk && rocNegative) {

...
export interface StratGridParams extends StrategyParams {
  stoch_k_period: number;
  stoch_d_period: number;
  stoch_lookback: number;
...
const defaultParams: StratGridParams = {
  stoch_k_period: 18,
  stoch_d_period: 3,
  stoch_lookback: 51,
  stoch_oversold: 14,
  ...
  risk_percent: 0.25,
  sr_lookback: 51,
  resistance_exit: true,
};

export class StratGridStrategy implements Strategy {
  params: StratGridParams;
  ...
  constructor(params: Partial<StratGridParams> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as StratGridParams;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (entry !== undefined && entryBarNum !== undefined) {
        ...
        if (this.params.resistance_exit && bar.close >= sr.resistance * this.params.resistance_threshold) {
          ctx.close(bar.tokenId);
          this.entryPrice.delete(bar.tokenId);
          this.entryBar.delete(bar.tokenId);
          return;
...
function loadSavedParams(): Partial<StratGridParams> | null {
  const paramsPath = path.join(__dirname, 'strat_grid.params.json');

...
export interface ComboV20StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV20StrategyParams = {
  bb_period: 18,
  rsi_period: 10,
  std_mult: 3,
  rsi_oversold: 15,
...
function loadSavedParams(): Partial<ComboV20StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v20_153.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV20StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV20StrategyParams] = value;
        }
      }
    }
...
export class ComboV20Strategy implements Strategy {
  params: ComboV20StrategyParams;
  ...
  constructor(params: Partial<ComboV20StrategyParams> = {}) {

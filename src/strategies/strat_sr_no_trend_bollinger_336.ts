...
/**
 * SR No Trend Bollinger 336
 * - Based on 302 but adds Bollinger Bands filter
 * - Only enter when price is near lower band (oversold)
 */

export interface SRNoTrendBollinger336Params extends StrategyParams {
  ...
  bollinger_near_mult: number;
}

const defaultParams: SRNoTrendBollinger336Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
  ...
  bollinger_period: 20,
  bollinger_std: 2.0,
  bollinger_near_mult: 1.05,
};

function loadSavedParams(): Partial<SRNoTrendBollinger336Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_bollinger_336.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendBollinger336Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendBollinger336Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendBollinger336Strategy implements Strategy {
  params: SRNoTrendBollinger336Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendBollinger336Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendBollinger336Params;
  }
  ...
  private getBollingerBands(prices: number[]): { upper: number; middle: number; lower: number } | null {
    if (prices.length < this.params.bollinger_period) return null;
    const slice = prices.slice(-this.params.bollinger_period);
    const mean = slice.reduce((a, b) => a + b, 0) / slice.length;
    const sqDiffs = slice.reduce((sum, p) => sum + (p - mean) * (p - mean), 0);
    const std = Math.sqrt(sqDiffs / slice.length);
    const upper = mean + (std * this.params.bollinger_std);
    const middle = mean;
    const lower = mean - (std * this.params.bollinger_std);
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const bollinger = this.getBollingerBands(data.prices);
    
    if (!stoch || supports.length === 0 || !bollinger) return;
    ...
    if (position && position.size > 0) {
      ...
      const nearLowerBollinger = bar.close < bollinger.lower * this.params.bollinger_near_mult;

      if (nearSupport && multiBarBounce && stochOversold && momentumOk && nearLowerBollinger) {

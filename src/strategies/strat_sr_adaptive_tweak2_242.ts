...
/**
 * SR Adaptive Tweak2 242
 * - Tweak of sr_adaptive_tweak_233 ($2,763)
 * - Adjusted volatility scaling
 * - Modified bounce threshold range
 */

export interface SRAdaptiveTweak2_242Params extends StrategyParams {
...
const defaultParams: SRAdaptiveTweak2_242Params = {
  base_lookback: 20,
  min_lookback: 6,
  max_lookback: 40,
...
function loadSavedParams(): Partial<SRAdaptiveTweak2_242Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_adaptive_tweak2_242.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRAdaptiveTweak2_242Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRAdaptiveTweak2_242Params] = value;
        }
      }
    }
...
export class SRAdaptiveTweak2_242Strategy implements Strategy {
  params: SRAdaptiveTweak2_242Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRAdaptiveTweak2_242Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRAdaptiveTweak2_242Params;
  }

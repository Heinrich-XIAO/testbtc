...
export interface StochV01StrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochV01StrategyParams = {
  k_period: 3,
  d_period: 2,
  oversold: 15,
  overbought: 85,
...
function loadSavedParams(): Partial<StochV01StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_v01_154.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochV01StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochV01StrategyParams] = value;
        }
      }
    }
...
export class StochV01Strategy implements Strategy {
  params: StochV01StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochV01StrategyParams> = {}) {

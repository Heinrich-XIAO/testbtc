...
// 1.5% drop, 8% trail, 8% stop, 20 hold, 25% risk
...
export interface StratIter240AParams extends StrategyParams { drop_pct: number; stop_loss: number; trailing_stop: number; max_hold: number; risk: number; }

export class StratIter240AStrategy extends BaseIterStrategy<StratIter240AParams> {
  constructor(params: Partial<StratIter240AParams> = {}) {
    super('strat_iter240_a.params.json', { drop_pct: 0.015, stop_loss: 0.08, trailing_stop: 0.08, max_hold: 20, risk: 0.25 }, params);
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (pos && pos.size > 0) {
      ...
      if (stopHit || trailingHit || holdExpired) {

...
/**
 * SR Adaptive Tweak 233
 * - Based on sr_adaptive_225 (best full return: $2,696)
 * - Wider volatility range for adaptation
 * - Dynamic bounce threshold based on volatility
 */

export interface SRAdaptiveTweak233Params extends StrategyParams {
...
const defaultParams: SRAdaptiveTweak233Params = {
  base_lookback: 20,
  min_lookback: 6,
  max_lookback: 40,
...
function loadSavedParams(): Partial<SRAdaptiveTweak233Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_adaptive_tweak_233.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRAdaptiveTweak233Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRAdaptiveTweak233Params] = value;
        }
      }
    }
...
export class SRAdaptiveTweak233Strategy implements Strategy {
  params: SRAdaptiveTweak233Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRAdaptiveTweak233Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRAdaptiveTweak233Params;
  }
  ...
  private getAdaptiveBounceThreshold(data: TokenData): number {
    ...
    const recentVol = this.calcVolatility(data.prices.slice(-this.params.volatility_period));
    // Higher volatility = wider bounce threshold

...
/**
 * SR No Trend Filter 302
 * - Based on 293 but removes trend filter (keeps momentum)
 * - Tests if trend filter was the real blocker in ITERATION_12
 * - Keeps loose momentum as quality filter
 */

export interface SRNoTrendFilter302Params extends StrategyParams {
...
const defaultParams: SRNoTrendFilter302Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendFilter302Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_filter_302.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendFilter302Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendFilter302Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendFilter302Strategy implements Strategy {
  params: SRNoTrendFilter302Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendFilter302Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendFilter302Params;
  }

...
export interface PatV17StrategyParams extends StrategyParams {
  consec_bars: number;
  exit_bars: number;
  stop_loss: number;
...
const defaultParams: PatV17StrategyParams = {
  consec_bars: 2,
  exit_bars: 1,
  stop_loss: 0.05,
  trailing_stop: 0.04,
...
function loadSavedParams(): Partial<PatV17StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_pat_v17_190.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<PatV17StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof PatV17StrategyParams] = value;
        }
      }
    }
...
export class PatV17Strategy implements Strategy {
  params: PatV17StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<PatV17StrategyParams> = {}) {

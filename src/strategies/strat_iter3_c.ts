...
export interface StratIter3CParams extends StrategyParams {
...
const defaultParams: StratIter3CParams = {
  stoch_k_period: 14,
  stoch_d_period: 3,
  stoch_oversold: 16,
  stoch_overbought: 84,
...
function loadSavedParams(): Partial<StratIter3CParams> | null {
  const paramsPath = path.join(__dirname, 'strat_iter3_c.params.json');
...
export class StratIter3CStrategy implements Strategy {
  params: StratIter3CParams;
  ...
  constructor(params: Partial<StratIter3CParams> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as StratIter3CParams;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (kVals.length >= 3 && dVals.length >= 3) {
        ...
        const kCrossesAboveD = prevK < prevD && currK >= currD;
        const kCrossesAboveOversold = prevK < this.params.stoch_oversold && currK >= this.params.stoch_oversold;

        if (kCrossesAboveD && kCrossesAboveOversold) {

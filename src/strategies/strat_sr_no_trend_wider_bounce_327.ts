...
/**
 * SR No Trend Wider Bounce 327
 * - Based on 302 but widens bounce_threshold from 0.028 to 0.05
 * - More lenient on what's "near support"
 */

export interface SRNoTrendWiderBounce327Params extends StrategyParams {
...
const defaultParams: SRNoTrendWiderBounce327Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendWiderBounce327Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_wider_bounce_327.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendWiderBounce327Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendWiderBounce327Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendWiderBounce327Strategy implements Strategy {
  params: SRNoTrendWiderBounce327Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendWiderBounce327Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendWiderBounce327Params;
  }

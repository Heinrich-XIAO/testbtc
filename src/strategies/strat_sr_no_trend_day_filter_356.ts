...
/**
 * SR No Trend Day Filter 356
 * - Based on 302 but skips trading on Monday (1) and Friday (5)
 * - Avoids weekend gap risk
 */

export interface SRNoTrendDayFilter356Params extends StrategyParams {
...
const defaultParams: SRNoTrendDayFilter356Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendDayFilter356Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_day_filter_356.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendDayFilter356Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendDayFilter356Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendDayFilter356Strategy implements Strategy {
  params: SRNoTrendDayFilter356Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendDayFilter356Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendDayFilter356Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    const dayOfWeek = new Date(bar.timestamp).getDay();

...
export interface StratIter176DParams extends StrategyParams { sr_lookback: number; entry_threshold: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; }
export class StratIter176DStrategy extends BaseIterStrategy<StratIter176DParams> {
  constructor(params: Partial<StratIter176DParams> = {}) { super('strat_iter176_d.params.json', { sr_lookback: 50, entry_threshold: 0.02, stop_loss: 0.08, profit_target: 0.15, max_hold_bars: 24, risk_percent: 0.25 }, params); }
  onNext(ctx: BacktestContext, bar: Bar): void {
    const { series, barNum } = this.nextBar(bar);
    const sr = supportResistance(series.highs, series.lows, this.params.sr_lookback);
    const pos = ctx.getPosition(bar.tokenId);
    if (pos && pos.size > 0) { const e = this.entryPrice.get(bar.tokenId)!; const eb = this.entryBar.get(bar.tokenId)!; if (bar.low <= e * (1 - this.params.stop_loss) || bar.high >= e * (1 + this.params.profit_target) || (sr && bar.high >= sr.resistance * 0.99) || barNum - eb >= this.params.max_hold_bars) { this.close(ctx, bar.tokenId); } return; }
    if (shouldSkipPrice(bar.close) || sr === null) return;
    if (bar.low <= sr.support * (1 + this.params.entry_threshold)) { this.open(ctx, bar, barNum, this.params.risk_percent); }
  }
}

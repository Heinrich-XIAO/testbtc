...
/**
 * SR Multi Momentum 244
 * - Combines multi-exit with momentum filter
 * - Best of both worlds: profit targets + momentum confirmation
 */

export interface SRMultiMomentum244Params extends StrategyParams {
...
const defaultParams: SRMultiMomentum244Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRMultiMomentum244Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_multi_momentum_244.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRMultiMomentum244Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRMultiMomentum244Params] = value;
        }
      }
    }
...
export class SRMultiMomentum244Strategy implements Strategy {
  params: SRMultiMomentum244Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRMultiMomentum244Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRMultiMomentum244Params;
  }

...
export interface ComboV18StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV18StrategyParams = {
  bb_period: 7,
  rsi_period: 5,
  std_mult: 2,
  rsi_oversold: 25,
...
function loadSavedParams(): Partial<ComboV18StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v18_151.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV18StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV18StrategyParams] = value;
        }
      }
    }
...
export class ComboV18Strategy implements Strategy {
  params: ComboV18StrategyParams;
  ...
  constructor(params: Partial<ComboV18StrategyParams> = {}) {

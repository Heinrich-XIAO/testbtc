...
export interface StratIter196DParams extends StrategyParams { sr_lookback: number; entry_threshold: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; }
export class StratIter196DStrategy extends BaseIterStrategy<StratIter196DParams> {
  constructor(params: Partial<StratIter196DParams> = {}) { super('strat_iter196_d.params.json', { sr_lookback: 50, entry_threshold: 0.02, stop_loss: 0.08, profit_target: 0.15, max_hold_bars: 24, risk_percent: 0.25 }, params); }

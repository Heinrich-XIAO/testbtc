...
/**
 * RSI + BB combo with tight thresholds
 */

export interface ComboTightStrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboTightStrategyParams = {
  bb_period: 8,
  rsi_period: 5,
  std_mult: 1.8,
  rsi_oversold: 30,
...
function loadSavedParams(): Partial<ComboTightStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_tight_42.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboTightStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboTightStrategyParams] = value;
        }
      }
    }
...
export class ComboTightStrategy implements Strategy {
  params: ComboTightStrategyParams;
  ...
  constructor(params: Partial<ComboTightStrategyParams> = {}) {

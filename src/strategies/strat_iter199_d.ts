...
export interface StratIter199DParams extends StrategyParams { sr_lookback: number; entry_threshold: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; }
export class StratIter199DStrategy extends BaseIterStrategy<StratIter199DParams> {
  constructor(params: Partial<StratIter199DParams> = {}) { super('strat_iter199_d.params.json', { sr_lookback: 30, entry_threshold: 0.02, stop_loss: 0.08, profit_target: 0.15, max_hold_bars: 24, risk_percent: 0.25 }, params); }

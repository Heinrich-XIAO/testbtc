...
export interface PatV05StrategyParams extends StrategyParams {
  consec_bars: number;
  exit_bars: number;
  stop_loss: number;
...
const defaultParams: PatV05StrategyParams = {
  consec_bars: 3,
  exit_bars: 3,
  stop_loss: 0.07,
  trailing_stop: 0.05,
...
function loadSavedParams(): Partial<PatV05StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_pat_v05_178.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<PatV05StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof PatV05StrategyParams] = value;
        }
      }
    }
...
export class PatV05Strategy implements Strategy {
  params: PatV05StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<PatV05StrategyParams> = {}) {

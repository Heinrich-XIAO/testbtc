...
export interface StratIter168DParams extends StrategyParams {
  rsi_period: number;
  rsi_oversold: number;
  rsi_recover: number;
  stop_loss: number;
...
export class StratIter168DStrategy implements Strategy {
  params: StratIter168DParams;
  private series: Map<string, TokenSeries> = new Map();
  private bars: Map<string, number> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private entryBar: Map<string, number> = new Map();

  constructor(params: Partial<StratIter168DParams> = {}) {
    const saved = loadSavedParams<StratIter168DParams>('strat_iter168_d.params.json');
    this.params = {
      rsi_period: 14,
      rsi_oversold: 15,
      rsi_recover: 40,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    // Entry: RSI was below oversold (15), now recovered above rsi_recover

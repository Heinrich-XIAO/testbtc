...
/**
 * SR Momentum Trend 252
 * - New strategy combining momentum and trend strength
 * - Uses price rate of change as momentum indicator
 * - Requires both momentum and trend to be positive
 */

export interface SRMomentumTrend252Params extends StrategyParams {
...
const defaultParams: SRMomentumTrend252Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRMomentumTrend252Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_momentum_trend_252.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRMomentumTrend252Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRMomentumTrend252Params] = value;
        }
      }
    }
...
export class SRMomentumTrend252Strategy implements Strategy {
  params: SRMomentumTrend252Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRMomentumTrend252Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRMomentumTrend252Params;
  }

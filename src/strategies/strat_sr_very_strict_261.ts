...
/**
 * SR Very Strict 261
 * - Even more strict confirmations
 * - Requires 3+ consecutive bounces
 * - Very tight stoch oversold
 */

export interface SRVeryStrict261Params extends StrategyParams {
...
const defaultParams: SRVeryStrict261Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRVeryStrict261Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_very_strict_261.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRVeryStrict261Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRVeryStrict261Params] = value;
        }
      }
    }
...
export class SRVeryStrict261Strategy implements Strategy {
  params: SRVeryStrict261Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRVeryStrict261Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRVeryStrict261Params;
  }

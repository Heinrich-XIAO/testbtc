...
/**
 * Long swing trading at lows
 */

export interface SwingLongStrategyParams extends StrategyParams {
  swing_window: number;
  stop_loss: number;
...
const defaultParams: SwingLongStrategyParams = {
  swing_window: 5,
  stop_loss: 0.08,
  trailing_stop: 0.06,
  take_profit: 0.12,
...
function loadSavedParams(): Partial<SwingLongStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_swing_long_47.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SwingLongStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SwingLongStrategyParams] = value;
        }
      }
    }
...
export class SwingLongStrategy implements Strategy {
  params: SwingLongStrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SwingLongStrategyParams> = {}) {

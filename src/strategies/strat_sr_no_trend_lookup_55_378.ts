...
/**
 * SR No Trend Lookup 55 - 378
 * - Based on 302 with max_lookback=55
 * - Fine-tuning adaptive lookback range
 */

export interface SRNoTrendLookup55378Params extends StrategyParams {
...
const defaultParams: SRNoTrendLookup55378Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 55,
...
function loadSavedParams(): Partial<SRNoTrendLookup55378Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_lookup_55_378.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendLookup55378Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendLookup55378Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendLookup55378Strategy implements Strategy {
  params: SRNoTrendLookup55378Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendLookup55378Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendLookup55378Params;
  }

...
/**
 * SR No Trend Combo Tight 379
 * - Based on 302
 * - max_lookback=50 for longer-term S/R detection
 * - bounce_threshold=0.020 tighter proximity to support
 */

export interface SRNoTrendComboTight379Params extends StrategyParams {
...
const defaultParams: SRNoTrendComboTight379Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
...
function loadSavedParams(): Partial<SRNoTrendComboTight379Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_combo_tight_379.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendComboTight379Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendComboTight379Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendComboTight379Strategy implements Strategy {
  params: SRNoTrendComboTight379Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendComboTight379Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendComboTight379Params;
  }

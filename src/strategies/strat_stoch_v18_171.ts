...
export interface StochV18StrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochV18StrategyParams = {
  k_period: 7,
  d_period: 3,
  oversold: 20,
  overbought: 80,
...
function loadSavedParams(): Partial<StochV18StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_v18_171.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochV18StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochV18StrategyParams] = value;
        }
      }
    }
...
export class StochV18Strategy implements Strategy {
  params: StochV18StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochV18StrategyParams> = {}) {

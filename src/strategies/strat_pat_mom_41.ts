...
/**
 * Buy on consecutive ups (momentum)
 */

export interface PatMomStrategyParams extends StrategyParams {
  consec_bars: number;
  buy_on_dip: number;
  exit_bars: number;
  stop_loss: number;
...
const defaultParams: PatMomStrategyParams = {
  consec_bars: 3,
  buy_on_dip: 0,
  exit_bars: 2,
  stop_loss: 0.05,
...
function loadSavedParams(): Partial<PatMomStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_pat_mom_41.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<PatMomStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof PatMomStrategyParams] = value;
        }
      }
    }
...
export class PatMomStrategy implements Strategy {
  params: PatMomStrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<PatMomStrategyParams> = {}) {

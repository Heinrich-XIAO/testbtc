...
/**
 * SR Wide Higher PT 287
 * - Wide stochastic bands + higher profit target (0.20)
 * - Go for bigger wins
 */

export interface SRWideHigherPT287Params extends StrategyParams {
...
const defaultParams: SRWideHigherPT287Params = {
  base_lookback: 18,
  min_lookback: 8,
  max_lookback: 36,
  ...
  stop_loss: 0.075,
  trailing_stop: 0.06,
  profit_target: 0.20, // Higher profit target (was 0.15)
  max_hold_bars: 40,
  risk_percent: 0.20,
};

function loadSavedParams(): Partial<SRWideHigherPT287Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_wide_higher_pt_287.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRWideHigherPT287Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRWideHigherPT287Params] = value;
        }
      }
    }
...
export class SRWideHigherPT287Strategy implements Strategy {
  params: SRWideHigherPT287Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRWideHigherPT287Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRWideHigherPT287Params;
  }

...
export interface StratIter162AParams extends StrategyParams {
  sr_lookback: number;
  macd_fast: number;
  macd_slow: number;
  macd_signal: number;
...
export class StratIter162AStrategy extends BaseIterStrategy<StratIter162AParams> {
  ...
  private signalHist: Map<string, number[]> = new Map();
  private kVals: Map<string, number[]> = new Map();

  constructor(params: Partial<StratIter162AParams> = {}) {
    super('strat_iter162_a.params.json', {
      sr_lookback: 50,
      macd_fast: 12,
      macd_slow: 26,
      macd_signal: 9,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (!this.signalHist.has(bar.tokenId)) this.signalHist.set(bar.tokenId, []);
    ...
    const signalLine = emaNext(this.emaSignal.get(bar.tokenId), macdLine, this.params.macd_signal);
    this.emaSignal.set(bar.tokenId, signalLine);

    capPush(this.macdHist.get(bar.tokenId)!, macdLine);
    capPush(this.signalHist.get(bar.tokenId)!, signalLine);
    ...
    const sh = this.signalHist.get(bar.tokenId)!;
    ...
    if (!sr || shouldSkipPrice(bar.close) || mh.length < 3 || sh.length < 3 || kv.length < 2) return;

    const prevMacd = mh[mh.length - 2];
    const currMacd = mh[mh.length - 1];
    const prevSignal = sh[sh.length - 2];
    const currSignal = sh[sh.length - 1];
    const macdCrossAbove = prevMacd <= prevSignal && currMacd > currSignal;

    const nearSupport = bar.low <= sr.support * 1.015;
    const stochRecover = kv[kv.length - 2] < this.params.stoch_oversold && kv[kv.length - 1] >= this.params.stoch_oversold;

    if (macdCrossAbove && nearSupport && stochRecover) {

...
/**
 * Conservative RSI div
 */

export interface RsiDV06StrategyParams extends StrategyParams {
  rsi_period: number;
...
const defaultParams: RsiDV06StrategyParams = {
  rsi_period: 10,
...
function loadSavedParams(): Partial<RsiDV06StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_rsi_d_v06_199.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<RsiDV06StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof RsiDV06StrategyParams] = value;
        }
      }
    }
...
export class RsiDV06Strategy implements Strategy {
  params: RsiDV06StrategyParams;
  private rsiMap: Map<string, RSI> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private rsiHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<RsiDV06StrategyParams> = {}) {

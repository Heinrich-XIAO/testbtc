...
export interface EnvV16StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV16StrategyParams = {
  ma_period: 6,
  envelope_pct: 0.02,
  stop_loss: 0.12,
  risk_percent: 0.1,
...
function loadSavedParams(): Partial<EnvV16StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_v16_109.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvV16StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV16StrategyParams] = value;
        }
      }
    }
...
export class EnvV16Strategy implements Strategy {
  params: EnvV16StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV16StrategyParams> = {}) {

...
/**
 * SR Trend Strength 247
 * - Adds trend strength filter before entering
 * - Only enters when recent trend is positive
 */

export interface SRTrendStrength247Params extends StrategyParams {
...
const defaultParams: SRTrendStrength247Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
  ...
  stoch_overbought: 70,
  trend_period: 20,
  trend_threshold: -0.02,  // Allow slight negative trend (buying dips)
  stop_loss: 0.09,
  trailing_stop: 0.055,
  risk_percent: 0.16,
};

function loadSavedParams(): Partial<SRTrendStrength247Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_trend_strength_247.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRTrendStrength247Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRTrendStrength247Params] = value;
        }
      }
    }
...
export class SRTrendStrength247Strategy implements Strategy {
  params: SRTrendStrength247Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRTrendStrength247Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRTrendStrength247Params;
  }

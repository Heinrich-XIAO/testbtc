...
export interface WillRV10StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV10StrategyParams = {
  period: 6,
  oversold_level: -70,
  overbought_level: -30,
  stop_loss: 0.04,
...
function loadSavedParams(): Partial<WillRV10StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v10_83.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV10StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV10StrategyParams] = value;
        }
      }
    }
...
export class WillRV10Strategy implements Strategy {
  params: WillRV10StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV10StrategyParams> = {}) {

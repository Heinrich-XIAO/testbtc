...
export interface ComboV12StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV12StrategyParams = {
  bb_period: 14,
  rsi_period: 8,
  std_mult: 2.8,
  rsi_oversold: 18,
...
function loadSavedParams(): Partial<ComboV12StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v12_145.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV12StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV12StrategyParams] = value;
        }
      }
    }
...
export class ComboV12Strategy implements Strategy {
  params: ComboV12StrategyParams;
  ...
  constructor(params: Partial<ComboV12StrategyParams> = {}) {

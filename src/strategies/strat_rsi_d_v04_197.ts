...
export interface RsiDV04StrategyParams extends StrategyParams {
  rsi_period: number;
...
const defaultParams: RsiDV04StrategyParams = {
  rsi_period: 8,
...
function loadSavedParams(): Partial<RsiDV04StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_rsi_d_v04_197.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<RsiDV04StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof RsiDV04StrategyParams] = value;
        }
      }
    }
...
export class RsiDV04Strategy implements Strategy {
  params: RsiDV04StrategyParams;
  private rsiMap: Map<string, RSI> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private rsiHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<RsiDV04StrategyParams> = {}) {

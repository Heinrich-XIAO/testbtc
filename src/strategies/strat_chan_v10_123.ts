...
export interface ChanV10StrategyParams extends StrategyParams {
...
const defaultParams: ChanV10StrategyParams = {
  channel_period: 12,
  channel_width: 0.9,
  stop_loss: 0.1,
  trailing_stop: 0.07,
...
function loadSavedParams(): Partial<ChanV10StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_v10_123.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanV10StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanV10StrategyParams] = value;
        }
      }
    }
...
export class ChanV10Strategy implements Strategy {
  params: ChanV10StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanV10StrategyParams> = {}) {

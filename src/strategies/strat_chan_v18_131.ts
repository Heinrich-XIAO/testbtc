...
export interface ChanV18StrategyParams extends StrategyParams {
...
const defaultParams: ChanV18StrategyParams = {
  channel_period: 6,
  channel_width: 0.5,
  stop_loss: 0.06,
  trailing_stop: 0.04,
...
function loadSavedParams(): Partial<ChanV18StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_v18_131.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanV18StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanV18StrategyParams] = value;
        }
      }
    }
...
export class ChanV18Strategy implements Strategy {
  params: ChanV18StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanV18StrategyParams> = {}) {

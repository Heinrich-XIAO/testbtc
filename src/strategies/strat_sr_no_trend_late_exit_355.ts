...
/**
 * SR No Trend Late Exit 355
 * - Based on 302
 * - Wider stochastic overbought exit (95 instead of 82) to stay in longer before exiting
 */

export interface SRNoTrendLateExit355Params extends StrategyParams {
...
const defaultParams: SRNoTrendLateExit355Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendLateExit355Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_late_exit_355.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendLateExit355Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendLateExit355Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendLateExit355Strategy implements Strategy {
  params: SRNoTrendLateExit355Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendLateExit355Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendLateExit355Params;
  }

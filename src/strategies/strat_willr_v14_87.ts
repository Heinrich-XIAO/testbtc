...
export interface WillRV14StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV14StrategyParams = {
  period: 9,
  oversold_level: -78,
  overbought_level: -22,
  stop_loss: 0.06,
...
function loadSavedParams(): Partial<WillRV14StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v14_87.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV14StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV14StrategyParams] = value;
        }
      }
    }
...
export class WillRV14Strategy implements Strategy {
  params: WillRV14StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV14StrategyParams> = {}) {

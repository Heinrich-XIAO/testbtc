...
/**
 * SR No Trend WillR 340
 * - Based on 302 but adds Williams %R filter
 * - Only enter when %R is oversold (< -80)
 */

export interface SRNoTrendWillR340Params extends StrategyParams {
...
const defaultParams: SRNoTrendWillR340Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendWillR340Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_willr_340.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendWillR340Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendWillR340Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendWillR340Strategy implements Strategy {
  params: SRNoTrendWillR340Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendWillR340Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendWillR340Params;
  }
  ...
  private getWilliamsR(data: TokenData): number | null {
    if (data.prices.length < this.params.willr_period) return null;
    const period = Math.min(this.params.willr_period, data.prices.length);
    const highs = data.highs.slice(-period);
    const lows = data.lows.slice(-period);
    const highestHigh = Math.max(...highs);
    const lowestLow = Math.min(...lows);
    const currentClose = data.prices[data.prices.length - 1];
    if (highestHigh === lowestLow) return -50;
    const willr = ((highestHigh - currentClose) / (highestHigh - lowestLow)) * -100;
    return willr;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const willr = this.getWilliamsR(data);
    ...
    if (!stoch || supports.length === 0 || willr === null) return;
    ...
    if (position && position.size > 0) {
      ...
      const willrOversold = willr < this.params.willr_oversold;

      if (nearSupport && multiBarBounce && stochOversold && momentumOk && willrOversold) {

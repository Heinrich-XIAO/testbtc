...
/**
 * SR No Trend Wide Stoch 325
 * - Based on 302 but uses WIDER stochastic oversold (34 instead of 24)
 * - Later entry = more confirmation = potentially fewer false signals
 */

export interface SRNoTrendWideStoch325Params extends StrategyParams {
...
const defaultParams: SRNoTrendWideStoch325Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendWideStoch325Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_wide_stoch_325.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendWideStoch325Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendWideStoch325Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendWideStoch325Strategy implements Strategy {
  params: SRNoTrendWideStoch325Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendWideStoch325Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendWideStoch325Params;
  }

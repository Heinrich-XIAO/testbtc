...
export interface MRRsiV18StrategyParams extends StrategyParams {
  ma_period: number;
  rsi_period: number;
...
const defaultParams: MRRsiV18StrategyParams = {
  ma_period: 6,
  rsi_period: 5,
...
function loadSavedParams(): Partial<MRRsiV18StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_mr_rsi_v18_71.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<MRRsiV18StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof MRRsiV18StrategyParams] = value;
        }
      }
    }
...
export class MRRsiV18Strategy implements Strategy {
  params: MRRsiV18StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private rsiMap: Map<string, RSI> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<MRRsiV18StrategyParams> = {}) {

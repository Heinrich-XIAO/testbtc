...
export interface PatV04StrategyParams extends StrategyParams {
  consec_bars: number;
  exit_bars: number;
  stop_loss: number;
...
const defaultParams: PatV04StrategyParams = {
  consec_bars: 3,
  exit_bars: 2,
  stop_loss: 0.06,
  trailing_stop: 0.05,
...
function loadSavedParams(): Partial<PatV04StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_pat_v04_177.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<PatV04StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof PatV04StrategyParams] = value;
        }
      }
    }
...
export class PatV04Strategy implements Strategy {
  params: PatV04StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<PatV04StrategyParams> = {}) {

...
/**
 * SR No Trend Short Hold 357
 * - Based on 302
 * - SHORTER max hold (15 instead of 28) - exit faster if no progress
 */

export interface SRNoTrendShortHold357Params extends StrategyParams {
...
const defaultParams: SRNoTrendShortHold357Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendShortHold357Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_short_hold_357.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendShortHold357Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendShortHold357Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendShortHold357Strategy implements Strategy {
  params: SRNoTrendShortHold357Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendShortHold357Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendShortHold357Params;
  }

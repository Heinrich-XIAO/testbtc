...
export interface MRRsiV06StrategyParams extends StrategyParams {
  ma_period: number;
  rsi_period: number;
...
const defaultParams: MRRsiV06StrategyParams = {
  ma_period: 8,
  rsi_period: 7,
...
function loadSavedParams(): Partial<MRRsiV06StrategyParams> | null {
  ...
  try {
    ...
    const params: Partial<MRRsiV06StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof MRRsiV06StrategyParams] = value;
        }
      }
    }
...
export class MRRsiV06Strategy implements Strategy {
  params: MRRsiV06StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private rsiMap: Map<string, RSI> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<MRRsiV06StrategyParams> = {}) {

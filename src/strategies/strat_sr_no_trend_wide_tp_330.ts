...
/**
 * SR No Trend Wide TP 330
 * - Based on 302
 * - Wider profit target (0.25 instead of 0.12)
 */

export interface SRNoTrendWideTP330Params extends StrategyParams {
...
const defaultParams: SRNoTrendWideTP330Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendWideTP330Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_wide_tp_330.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendWideTP330Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendWideTP330Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendWideTP330Strategy implements Strategy {
  params: SRNoTrendWideTP330Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendWideTP330Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendWideTP330Params;
  }

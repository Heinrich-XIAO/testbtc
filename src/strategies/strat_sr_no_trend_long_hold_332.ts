...
/**
 * SR No Trend Long Hold 332
 * - Based on 302
 * - Longer max hold bars (50 instead of 28) to let trades run longer
 */

export interface SRNoTrendLongHold332Params extends StrategyParams {
...
const defaultParams: SRNoTrendLongHold332Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendLongHold332Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_long_hold_332.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendLongHold332Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendLongHold332Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendLongHold332Strategy implements Strategy {
  params: SRNoTrendLongHold332Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendLongHold332Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendLongHold332Params;
  }

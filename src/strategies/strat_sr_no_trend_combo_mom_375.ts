...
/**
 * SR No Trend Combo Mom 375
 * - Based on 302 but uses max_lookback=50 (from 362) with higher momentum threshold
 */

export interface SRNoTrendComboMom375Params extends StrategyParams {
...
const defaultParams: SRNoTrendComboMom375Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
...
function loadSavedParams(): Partial<SRNoTrendComboMom375Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_combo_mom_375.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendComboMom375Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendComboMom375Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendComboMom375Strategy implements Strategy {
  params: SRNoTrendComboMom375Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendComboMom375Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendComboMom375Params;
  }

...
function capPushStr(values: string[], value: string, max = 500): void {
  values.push(value);
  if (values.length > max) values.shift();
}
...
export interface StratIter158EParams extends StrategyParams {
  sr_lookback: number;
...
export class StratIter158EStrategy implements Strategy {
  params: StratIter158EParams;
  ...
  private regimeHistory: Map<string, ('low' | 'normal' | 'high')[]> = new Map();
  private prevVol: Map<string, number | null> = new Map();

  constructor(params: Partial<StratIter158EParams> = {}) {
    const saved = loadSavedParams<StratIter158EParams>('strat_iter158_e.params.json');
    this.params = {
      sr_lookback: 50,
      stoch_k_period: 14,
      stoch_oversold: 16,
  ...
  private nextBar(bar: Bar): { series: TokenSeries; barNum: number } {
    if (!this.series.has(bar.tokenId)) {
      ...
      this.regimeHistory.set(bar.tokenId, []);
      this.prevVol.set(bar.tokenId, null);
    }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (!this.regimeHistory.has(bar.tokenId)) this.regimeHistory.set(bar.tokenId, []);
    ...
    if (vol !== null) {
      ...
      if (vh.length >= this.params.vol_rank_period) {
        ...
        capPushStr(this.regimeHistory.get(bar.tokenId)!, newRegime);
        const rh = this.regimeHistory.get(bar.tokenId)!;

        if (rh.length >= this.params.cluster_length + 1) {
          const prevCluster = rh.slice(-(this.params.cluster_length + 1), -1);
          const currentReg = rh[rh.length - 1];
          const allSame = prevCluster.every(r => r === prevCluster[0]);
          const clusterChanged = !allSame || (allSame && currentReg !== prevCluster[0]);
          if (clusterChanged) {
            const prevVolVal = this.prevVol.get(bar.tokenId);
            if (prevVolVal !== null && prevVolVal !== undefined) {
              const volChange = Math.abs((vol - prevVolVal) / prevVolVal) * 100;
              if (volChange >= this.params.vol_change_threshold) {
                clusterBroken = true;
              }
            }
          }
        }
        this.prevVol.set(bar.tokenId, vol);
      }
    }
    ...
    if (clusterBroken && nearSupport && stochRecover) {

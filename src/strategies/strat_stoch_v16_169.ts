...
export interface StochV16StrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochV16StrategyParams = {
  k_period: 6,
  d_period: 3,
  oversold: 20,
  overbought: 80,
...
function loadSavedParams(): Partial<StochV16StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_v16_169.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochV16StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochV16StrategyParams] = value;
        }
      }
    }
...
export class StochV16Strategy implements Strategy {
  params: StochV16StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochV16StrategyParams> = {}) {

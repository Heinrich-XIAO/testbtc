...
/**
 * SR No Trend Vol Sized 390
 * - Based on 362 but with volatility-scaled position sizing
 * - Smaller positions when volatility is high, larger when low
 */

export interface SRNoTrendVolSized390Params extends StrategyParams {
...
const defaultParams: SRNoTrendVolSized390Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
...
function loadSavedParams(): Partial<SRNoTrendVolSized390Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_vol_sized_390.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendVolSized390Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendVolSized390Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendVolSized390Strategy implements Strategy {
  params: SRNoTrendVolSized390Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendVolSized390Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendVolSized390Params;
  }
  ...
  private getVolatilityRatio(data: TokenData): number {
    if (data.prices.length < this.params.volatility_period) return 1;
    const recentVol = this.calcVolatility(data.prices.slice(-this.params.volatility_period));
    const historicalVol = this.calcVolatility(data.prices.slice(-this.params.max_lookback));
  ...
  private getAdjustedRisk(data: TokenData): number {
    const volRatio = this.getVolatilityRatio(data);
    const adjustedRisk = this.params.risk_percent / volRatio;
    return Math.max(this.params.min_risk_percent, Math.min(this.params.max_risk_percent, adjustedRisk));
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (nearSupport && multiBarBounce && stochOversold && momentumOk) {
        const adjustedRisk = this.getAdjustedRisk(data);
        const cash = ctx.getCapital() * adjustedRisk * 0.995;

...
/**
 * SR Wide Long Hold 286
 * - Wide stochastic bands + longer max_hold (55 bars)
 * - Let winning trades run longer
 */

export interface SRWideLongHold286Params extends StrategyParams {
...
const defaultParams: SRWideLongHold286Params = {
  base_lookback: 18,
  min_lookback: 8,
  max_lookback: 36,
  ...
  trailing_stop: 0.055,
  profit_target: 0.18,
  max_hold_bars: 55, // Longer hold (was 35)
  risk_percent: 0.20,
};

function loadSavedParams(): Partial<SRWideLongHold286Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_wide_long_hold_286.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRWideLongHold286Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRWideLongHold286Params] = value;
        }
      }
    }
...
export class SRWideLongHold286Strategy implements Strategy {
  params: SRWideLongHold286Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRWideLongHold286Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRWideLongHold286Params;
  }

...
/**
 * Mean reversion + RSI with tight thresholds
 */

export interface MRRSITightStrategyParams extends StrategyParams {
  ma_period: number;
  rsi_period: number;
...
const defaultParams: MRRSITightStrategyParams = {
  ma_period: 6,
  rsi_period: 5,
...
function loadSavedParams(): Partial<MRRSITightStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_mr_rsi_tight_32.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<MRRSITightStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof MRRSITightStrategyParams] = value;
        }
      }
    }
...
export class MRRSITightStrategy implements Strategy {
  params: MRRSITightStrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private rsiMap: Map<string, RSI> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<MRRSITightStrategyParams> = {}) {

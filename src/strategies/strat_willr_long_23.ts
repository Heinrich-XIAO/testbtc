...
/**
 * Long-period Williams %R
 */

export interface WillRLongStrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRLongStrategyParams = {
  period: 14,
  oversold_level: -85,
  overbought_level: -15,
  stop_loss: 0.08,
...
function loadSavedParams(): Partial<WillRLongStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_long_23.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRLongStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRLongStrategyParams] = value;
        }
      }
    }
...
export class WillRLongStrategy implements Strategy {
  params: WillRLongStrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRLongStrategyParams> = {}) {

...
export interface StratIter170CParams extends StrategyParams {
  bb_period: number;
  bb_std: number;
  stop_loss: number;
...
export class StratIter170CStrategy implements Strategy {
  params: StratIter170CParams;
  ...
  constructor(params: Partial<StratIter170CParams> = {}) {
    const saved = loadSavedParams<StratIter170CParams>('strat_iter170_c.params.json');
    this.params = {
      bb_period: 20,
      bb_std: 2,
      stop_loss: 0.08,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (shouldSkipPrice(bar.close) || s.closes.length < this.params.bb_period + 1) return;
    ...
    // Entry: Price crosses above lower band (bounce off lower band)
    if (prev <= lowerBand && bar.close > lowerBand) {

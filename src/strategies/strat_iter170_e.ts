...
export interface StratIter170EParams extends StrategyParams {
  bb_period: number;
  bb_std: number;
  percent_b_threshold: number;
  stop_loss: number;
...
export class StratIter170EStrategy implements Strategy {
  params: StratIter170EParams;
  private series: Map<string, TokenSeries> = new Map();
  private bars: Map<string, number> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private entryBar: Map<string, number> = new Map();

  constructor(params: Partial<StratIter170EParams> = {}) {
    const saved = loadSavedParams<StratIter170EParams>('strat_iter170_e.params.json');
    this.params = {
      bb_period: 20,
      bb_std: 2,
      percent_b_threshold: 0.2,
      stop_loss: 0.08,
      profit_target: 0.16,
      max_hold_bars: 24,
      risk_percent: 0.25,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (middle === null || sd === null || sd === 0) return;
    ...
    const percentB = (bar.close - lowerBand) / (upperBand - lowerBand);

    // Entry: %B below threshold (oversold, near lower band)
    if (percentB < this.params.percent_b_threshold) {

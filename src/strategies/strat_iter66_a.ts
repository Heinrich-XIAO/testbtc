...
function lempelZivComplexity(series: number[], threshold: number): number {
  if (series.length < 10) return 0;
  
  const binary = series.map(v => v > threshold ? 1 : 0);
  let complexity = 1;
  let subsequence = '';
  
  for (let i = 0; i < binary.length; i++) {
    const current = binary.slice(0, i + 1).join('');
    if (!subsequence.includes(current)) {
      complexity++;
      subsequence = current;
    }
  }
  
  return complexity / (binary.length + 1);
}

function normalizeLZComplexity(closes: number[], window: number, threshold: number): number {
  if (closes.length < window) return 0.5;
  
  const subset = closes.slice(-window);
  const mean = subset.reduce((a, b) => a + b, 0) / subset.length;
  const normalized = subset.map(v => (v - mean) / (mean + 0.0001));
  
  return lempelZivComplexity(normalized, 0);
}
...
export interface StratIter66AParams extends StrategyParams {
  lz_window: number;
  lz_normalize_window: number;
  lz_threshold: number;
  lz_low: number;
  lz_high: number;
...
export class StratIter66AStrategy implements Strategy {
  params: StratIter66AParams;

  private state: Map<string, TokenState> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private entryBar: Map<string, number> = new Map();

  constructor(params: Partial<StratIter66AParams> = {}) {
    const saved = loadSavedParams<StratIter66AParams>('strat_iter66_a.params.json');
    this.params = {
      lz_window: 30,
      lz_normalize_window: 50,
      lz_threshold: 0,
      lz_low: 0.15,
      lz_high: 0.45,
      stoch_k_period: 14,
  ...
  private computeLZIndicators(s: TokenState): void {
    const lw = Math.floor(this.params.lz_window);
    const nlw = Math.floor(this.params.lz_normalize_window);
    const th = this.params.lz_threshold;
    
    if (s.closes.length >= lw) {
      const lz = lempelZivComplexity(s.closes.slice(-lw), th);
      capPush(s.lzComplexity, lz);
    }
    
    if (s.closes.length >= nlw) {
      const norm = normalizeLZComplexity(s.closes, nlw, th);
      capPush(s.lzNormalized, norm);
    }
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (k === null || !s.stoch || s.stoch.length < 2 || s.lzNormalized.length < 3) return;
    ...
    const lzVal = s.lzNormalized[s.lzNormalized.length - 1];
    const lzAvg = s.lzNormalized.slice(-3).reduce((a, b) => a + b, 0) / 3;
    const lzValid = lzVal >= this.params.lz_low || lzAvg >= this.params.lz_low || s.lzNormalized.length < 5;

    if (nearSupport && supportReclaim && stochRebound && lzValid) {

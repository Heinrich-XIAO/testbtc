...
export interface ComboV02StrategyParams extends StrategyParams {
  bb_period: number;
  rsi_period: number;
  std_mult: number;
  rsi_oversold: number;
...
const defaultParams: ComboV02StrategyParams = {
  bb_period: 5,
  rsi_period: 3,
  std_mult: 1.8,
  rsi_oversold: 30,
...
function loadSavedParams(): Partial<ComboV02StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_combo_v02_135.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ComboV02StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ComboV02StrategyParams] = value;
        }
      }
    }
...
export class ComboV02Strategy implements Strategy {
  params: ComboV02StrategyParams;
  ...
  constructor(params: Partial<ComboV02StrategyParams> = {}) {

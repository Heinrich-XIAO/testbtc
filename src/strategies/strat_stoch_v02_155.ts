...
export interface StochV02StrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochV02StrategyParams = {
  k_period: 4,
  d_period: 2,
  oversold: 20,
  overbought: 80,
...
function loadSavedParams(): Partial<StochV02StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_v02_155.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochV02StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochV02StrategyParams] = value;
        }
      }
    }
...
export class StochV02Strategy implements Strategy {
  params: StochV02StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochV02StrategyParams> = {}) {

...
export interface StochV09StrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochV09StrategyParams = {
  k_period: 12,
  d_period: 5,
  oversold: 20,
  overbought: 80,
...
function loadSavedParams(): Partial<StochV09StrategyParams> | null {
  ...
  try {
    ...
    const params: Partial<StochV09StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochV09StrategyParams] = value;
        }
      }
    }
...
export class StochV09Strategy implements Strategy {
  params: StochV09StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochV09StrategyParams> = {}) {

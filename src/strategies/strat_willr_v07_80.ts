...
export interface WillRV07StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV07StrategyParams = {
  period: 12,
  oversold_level: -80,
  overbought_level: -20,
  stop_loss: 0.05,
...
function loadSavedParams(): Partial<WillRV07StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v07_80.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV07StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV07StrategyParams] = value;
        }
      }
    }
...
export class WillRV07Strategy implements Strategy {
  params: WillRV07StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV07StrategyParams> = {}) {

...
export interface StratIter147EParams extends StrategyParams {
  hf_threshold: number;
  quick_profit: number;
  tight_stop: number;
...
const defaultParams: StratIter147EParams = {
  hf_threshold: 0.008,
  quick_profit: 0.04,
  tight_stop: 0.025,
  stoch_oversold: 20,
...
function loadSavedParams(): Partial<StratIter147EParams> | null {
  const paramsPath = path.join(__dirname, 'strat_iter147_e.params.json');
...
export class StratIter147EStrategy implements Strategy {
  params: StratIter147EParams;
  ...
  constructor(params: Partial<StratIter147EParams> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as StratIter147EParams;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const volatility = this.calcVolatility(closes, 10);
    const nearSupport = bar.low <= sr.support * 1.01;
    const stochOversold = k !== null && k < this.params.stoch_oversold;
    const highFreqEnv = volatility !== null && volatility >= this.params.hf_threshold;

    if (nearSupport && stochOversold && highFreqEnv) {

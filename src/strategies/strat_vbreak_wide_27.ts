...
/**
 * Volatility breakout with wide parameters
 */

export interface VolBreakWideStrategyParams extends StrategyParams {
  vol_period: number;
  lookback: number;
...
const defaultParams: VolBreakWideStrategyParams = {
  vol_period: 12,
  lookback: 18,
...
function loadSavedParams(): Partial<VolBreakWideStrategyParams> | null {
  ...
  try {
    ...
    const params: Partial<VolBreakWideStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof VolBreakWideStrategyParams] = value;
        }
      }
    }
...
export class VolBreakWideStrategy implements Strategy {
  params: VolBreakWideStrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private volHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<VolBreakWideStrategyParams> = {}) {

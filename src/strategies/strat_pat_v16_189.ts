...
export interface PatV16StrategyParams extends StrategyParams {
  consec_bars: number;
  exit_bars: number;
  stop_loss: number;
...
const defaultParams: PatV16StrategyParams = {
  consec_bars: 3,
  exit_bars: 2,
  stop_loss: 0.12,
  trailing_stop: 0.08,
...
function loadSavedParams(): Partial<PatV16StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_pat_v16_189.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<PatV16StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof PatV16StrategyParams] = value;
        }
      }
    }
...
export class PatV16Strategy implements Strategy {
  params: PatV16StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<PatV16StrategyParams> = {}) {

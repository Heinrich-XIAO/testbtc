...
/**
 * SR Range Vol 254
 * - Uses price range (high-low) as volume proxy
 * - Wider range = more activity = stronger signals
 * - Filters entries based on range expansion
 */

export interface SRRangeVol254Params extends StrategyParams {
...
const defaultParams: SRRangeVol254Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRRangeVol254Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_range_vol_254.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRRangeVol254Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRRangeVol254Params] = value;
        }
      }
    }
...
export class SRRangeVol254Strategy implements Strategy {
  params: SRRangeVol254Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRRangeVol254Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRRangeVol254Params;
  }
  ...
  private isRangeExpansion(data: TokenData, currentRange: number): boolean {
    if (data.ranges.length < this.params.range_period) return true;
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      // Require range expansion for stronger signal

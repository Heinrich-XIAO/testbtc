...
/**
 * SR Tight Strict Combo 273
 * - Combines 260's strict entry conditions with 267's tighter trailing stop
 * - Best of both worlds approach
 */

export interface SRTightStrictCombo273Params extends StrategyParams {
  ...
  stop_loss: number;
  trailing_stop: number; // Tighter (3-5%)
  profit_target: number;
  max_hold_bars: number;
  risk_percent: number;
}

const defaultParams: SRTightStrictCombo273Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
  ...
  min_bounce_bars: 2,
  stop_loss: 0.075,
  trailing_stop: 0.04, // Tighter than 260's 0.055
  profit_target: 0.15,
  max_hold_bars: 35,
  risk_percent: 0.20,
};

function loadSavedParams(): Partial<SRTightStrictCombo273Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_tight_strict_combo_273.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRTightStrictCombo273Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRTightStrictCombo273Params] = value;
        }
      }
    }
...
export class SRTightStrictCombo273Strategy implements Strategy {
  params: SRTightStrictCombo273Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRTightStrictCombo273Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRTightStrictCombo273Params;
  }

...
/**
 * SR Adaptive Momentum 235
 * - Combines sr_adaptive_225 with momentum filter from sr_momentum_filter_227
 * - Adaptive lookback + momentum confirmation
 */

export interface SRAdaptiveMomentum235Params extends StrategyParams {
...
const defaultParams: SRAdaptiveMomentum235Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRAdaptiveMomentum235Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_adaptive_momentum_235.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRAdaptiveMomentum235Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRAdaptiveMomentum235Params] = value;
        }
      }
    }
...
export class SRAdaptiveMomentum235Strategy implements Strategy {
  params: SRAdaptiveMomentum235Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRAdaptiveMomentum235Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRAdaptiveMomentum235Params;
  }

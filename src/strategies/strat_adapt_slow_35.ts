...
/**
 * Slow adaptive MA crossover
 */

export interface AdaptSlowStrategyParams extends StrategyParams {
  min_period: number;
  max_period: number;
...
const defaultParams: AdaptSlowStrategyParams = {
  min_period: 5,
  max_period: 25,
  vol_sensitivity: 30,
  stop_loss: 0.08,
...
function loadSavedParams(): Partial<AdaptSlowStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_adapt_slow_35.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<AdaptSlowStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof AdaptSlowStrategyParams] = value;
        }
      }
    }
...
export class AdaptSlowStrategy implements Strategy {
  params: AdaptSlowStrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private adaptiveMA: Map<string, number> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<AdaptSlowStrategyParams> = {}) {

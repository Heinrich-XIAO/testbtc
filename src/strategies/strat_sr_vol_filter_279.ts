 ...
 * - Filter out high volatility = more predictable moves
 */

export interface SRVolFilter279Params extends StrategyParams {
  base_lookback: number;
  min_lookback: number;
  max_lookback: number;
  volatility_period: number;
  max_volatility: number; // Only trade when vol is below this
...
const defaultParams: SRVolFilter279Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
  volatility_period: 10,
  max_volatility: 0.08, // Only trade when volatility is below 8%
...
function loadSavedParams(): Partial<SRVolFilter279Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_vol_filter_279.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRVolFilter279Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRVolFilter279Params] = value;
        }
      }
    }
...
export class SRVolFilter279Strategy implements Strategy {
  params: SRVolFilter279Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRVolFilter279Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRVolFilter279Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      // VOLATILITY FILTER: Only enter during low volatility
      if (currentVol > this.params.max_volatility) return;

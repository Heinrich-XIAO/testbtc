...
export interface MRRsiV15StrategyParams extends StrategyParams {
  ma_period: number;
  rsi_period: number;
...
const defaultParams: MRRsiV15StrategyParams = {
  ma_period: 5,
  rsi_period: 4,
...
function loadSavedParams(): Partial<MRRsiV15StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_mr_rsi_v15_68.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<MRRsiV15StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof MRRsiV15StrategyParams] = value;
        }
      }
    }
...
export class MRRsiV15Strategy implements Strategy {
  params: MRRsiV15StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private rsiMap: Map<string, RSI> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<MRRsiV15StrategyParams> = {}) {

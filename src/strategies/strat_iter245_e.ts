...
// 35 bar hold, 50% profit, 10% stop, 60% risk, 1.5% drop
...
export interface StratIter245EParams extends StrategyParams { drop_pct: number; stop_loss: number; profit_target: number; max_hold: number; risk: number; }

export class StratIter245EStrategy extends BaseIterStrategy<StratIter245EParams> {
  constructor(params: Partial<StratIter245EParams> = {}) {
    super('strat_iter245_e.params.json', { drop_pct: 0.015, stop_loss: 0.1, profit_target: 0.5, max_hold: 35, risk: 0.6000000000000001 }, params);
  }

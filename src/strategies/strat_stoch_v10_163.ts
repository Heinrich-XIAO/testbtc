...
export interface StochV10StrategyParams extends StrategyParams {
  k_period: number;
  d_period: number;
  oversold: number;
  overbought: number;
...
const defaultParams: StochV10StrategyParams = {
  k_period: 14,
  d_period: 6,
  oversold: 15,
  overbought: 85,
...
function loadSavedParams(): Partial<StochV10StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_stoch_v10_163.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<StochV10StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof StochV10StrategyParams] = value;
        }
      }
    }
...
export class StochV10Strategy implements Strategy {
  params: StochV10StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private kValues: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<StochV10StrategyParams> = {}) {

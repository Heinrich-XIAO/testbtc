...
export interface ChanV12StrategyParams extends StrategyParams {
...
const defaultParams: ChanV12StrategyParams = {
  channel_period: 15,
  channel_width: 0.7,
  stop_loss: 0.04,
  trailing_stop: 0.03,
...
function loadSavedParams(): Partial<ChanV12StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_v12_125.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanV12StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanV12StrategyParams] = value;
        }
      }
    }
...
export class ChanV12Strategy implements Strategy {
  params: ChanV12StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanV12StrategyParams> = {}) {

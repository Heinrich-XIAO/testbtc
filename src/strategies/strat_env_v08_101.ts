...
export interface EnvV08StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV08StrategyParams = {
  ma_period: 12,
  envelope_pct: 0.02,
  stop_loss: 0.05,
  risk_percent: 0.1,
...
function loadSavedParams(): Partial<EnvV08StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_v08_101.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvV08StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV08StrategyParams] = value;
        }
      }
    }
...
export class EnvV08Strategy implements Strategy {
  params: EnvV08StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV08StrategyParams> = {}) {

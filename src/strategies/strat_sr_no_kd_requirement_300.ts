...
/**
 * SR No K>D Requirement 300
 * - Based on 293 but removes K>D requirement (stoch.k > stoch.d)
 * - No momentum filter + No K>D = more trade opportunities
 * - Goal: Test if K>D was preventing entries
 */

export interface SRNoKDRequirement300Params extends StrategyParams {
...
const defaultParams: SRNoKDRequirement300Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoKDRequirement300Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_kd_requirement_300.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoKDRequirement300Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoKDRequirement300Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoKDRequirement300Strategy implements Strategy {
  params: SRNoKDRequirement300Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoKDRequirement300Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoKDRequirement300Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      // NO K>D requirement - just stoch oversold level
      // NO momentum filter either
      const stochOversold = stoch.k <= this.params.stoch_oversold;  // REMOVED: && stoch.k > stoch.d

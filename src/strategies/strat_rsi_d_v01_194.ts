...
export interface RsiDV01StrategyParams extends StrategyParams {
  rsi_period: number;
...
const defaultParams: RsiDV01StrategyParams = {
  rsi_period: 3,
...
function loadSavedParams(): Partial<RsiDV01StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_rsi_d_v01_194.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<RsiDV01StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof RsiDV01StrategyParams] = value;
        }
      }
    }
...
export class RsiDV01Strategy implements Strategy {
  params: RsiDV01StrategyParams;
  private rsiMap: Map<string, RSI> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private rsiHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<RsiDV01StrategyParams> = {}) {

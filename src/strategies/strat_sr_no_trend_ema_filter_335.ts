...
/**
 * SR No Trend EMA Filter 335
 * - Based on 302 but adds EMA(20) filter for bullish bias
 * - Only enter when close > EMA(20)
 */

export interface SRNoTrendEMAFilter335Params extends StrategyParams {
...
const defaultParams: SRNoTrendEMAFilter335Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendEMAFilter335Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_ema_filter_335.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendEMAFilter335Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendEMAFilter335Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendEMAFilter335Strategy implements Strategy {
  params: SRNoTrendEMAFilter335Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendEMAFilter335Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendEMAFilter335Params;
  }
  ...
  private getOrCreateData(tokenId: string): TokenData {
    if (!this.tokenData.has(tokenId)) {
      this.tokenData.set(tokenId, { prices: [], highs: [], lows: [], kValues: [], consecutiveBounces: 0, emaValue: null, emaInitialized: false });
    }
  ...
  private updateEMA(data: TokenData): void {
    const period = this.params.ema_period;
    if (data.prices.length < period) {
      data.emaValue = null;
      data.emaInitialized = false;
      return;
    }
    
    const k = 2 / (period + 1);
    
    if (!data.emaInitialized) {
      let sum = 0;
      for (let i = 0; i < period; i++) {
        sum += data.prices[data.prices.length - period + i];
      }
      data.emaValue = sum / period;
      data.emaInitialized = true;
    } else if (data.emaValue !== null) {
      const newPrice = data.prices[data.prices.length - 1];
      data.emaValue = newPrice * k + data.emaValue * (1 - k);
    }
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      const emaFilter = data.emaValue !== null && bar.close >= data.emaValue * 0.97;

      if (nearSupport && multiBarBounce && stochOversold && momentumOk && emaFilter) {

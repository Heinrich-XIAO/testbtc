...
function homodyneDiscriminator(closes: number[], period: number): number | null {
  ...
  if (last >= 3) {
    i1 = smooth[last] - smooth[last - 2];
    q1 = smooth[last - 1] - smooth[last - 3];
  }
  
  return Math.atan2(q1, i1);
}
...
export interface StratIter139DParams extends StrategyParams { hd_threshold: number; hd_period: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; sr_lookback: number; }
export class StratIter139DStrategy extends BaseIterStrategy<StratIter139DParams> {
  constructor(params: Partial<StratIter139DParams> = {}) { super('strat_iter139_d.params.json', { hd_threshold: -0.5, hd_period: 20, stop_loss: 0.08, profit_target: 0.18, max_hold_bars: 28, risk_percent: 0.25, sr_lookback: 50 }, params); }
  onNext(ctx: BacktestContext, bar: Bar): void {
    const { series, barNum } = this.nextBar(bar); const hd = homodyneDiscriminator(series.closes, this.params.hd_period); const sr = priorSupportResistance(series.highs, series.lows, this.params.sr_lookback);
    ...
    if (shouldSkipPrice(bar.close) || !sr || hd === null) return;
    const nearSupport = bar.low <= sr.support * 1.015; const hdLow = hd < this.params.hd_threshold;
    if (nearSupport && hdLow) { this.open(ctx, bar, barNum, this.params.risk_percent); }
  }
}
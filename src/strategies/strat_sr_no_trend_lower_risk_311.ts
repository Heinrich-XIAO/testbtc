...
/**
 * SR No Trend Lower Risk 311
 * - Based on 302 but with lower risk per trade (20%)
 * - Tests if smaller position sizes improve consistency
 */

export interface SRNoTrendLowerRisk311Params extends StrategyParams {
...
const defaultParams: SRNoTrendLowerRisk311Params = {
  base_lookback: 20,
  min_lookback: 14,
  max_lookback: 36,
  ...
  profit_target: 0.14,
  max_hold_bars: 32,
  risk_percent: 0.20, // Lower than 302's 0.32
};

function loadSavedParams(): Partial<SRNoTrendLowerRisk311Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_lower_risk_311.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendLowerRisk311Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendLowerRisk311Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendLowerRisk311Strategy implements Strategy {
  params: SRNoTrendLowerRisk311Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendLowerRisk311Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendLowerRisk311Params;
  }

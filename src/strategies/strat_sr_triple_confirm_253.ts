...
/**
 * SR Triple Confirm 253
 * - Triple confirmation: support + stoch + trend + momentum
 * - Very strict entry requiring all conditions
 * - Should result in fewer but higher quality trades
 */

export interface SRTripleConfirm253Params extends StrategyParams {
...
const defaultParams: SRTripleConfirm253Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRTripleConfirm253Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_triple_confirm_253.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRTripleConfirm253Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRTripleConfirm253Params] = value;
        }
      }
    }
...
export class SRTripleConfirm253Strategy implements Strategy {
  params: SRTripleConfirm253Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRTripleConfirm253Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRTripleConfirm253Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      // Triple confirmation: all conditions must be met

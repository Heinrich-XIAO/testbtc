...
export interface MRRsiV16StrategyParams extends StrategyParams {
  ma_period: number;
  rsi_period: number;
...
const defaultParams: MRRsiV16StrategyParams = {
  ma_period: 5,
  rsi_period: 4,
...
function loadSavedParams(): Partial<MRRsiV16StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_mr_rsi_v16_69.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<MRRsiV16StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof MRRsiV16StrategyParams] = value;
        }
      }
    }
...
export class MRRsiV16Strategy implements Strategy {
  params: MRRsiV16StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private rsiMap: Map<string, RSI> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<MRRsiV16StrategyParams> = {}) {

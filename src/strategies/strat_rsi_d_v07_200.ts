...
export interface RsiDV07StrategyParams extends StrategyParams {
  rsi_period: number;
...
const defaultParams: RsiDV07StrategyParams = {
  rsi_period: 6,
...
function loadSavedParams(): Partial<RsiDV07StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_rsi_d_v07_200.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<RsiDV07StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof RsiDV07StrategyParams] = value;
        }
      }
    }
...
export class RsiDV07Strategy implements Strategy {
  params: RsiDV07StrategyParams;
  private rsiMap: Map<string, RSI> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private rsiHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<RsiDV07StrategyParams> = {}) {

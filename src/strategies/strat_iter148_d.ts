...
export interface StratIter148DParams extends StrategyParams {
  rsi_period: number;
  rsi_oversold: number;
...
const defaultParams: StratIter148DParams = {
  rsi_period: 14,
  rsi_oversold: 30,
  stoch_oversold: 16,
  stop_loss: 0.06,
...
function loadSavedParams(): Partial<StratIter148DParams> | null {
  const paramsPath = path.join(__dirname, 'strat_iter148_d.params.json');
...
export class StratIter148DStrategy implements Strategy {
  params: StratIter148DParams;
  ...
  constructor(params: Partial<StratIter148DParams> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as StratIter148DParams;
  }

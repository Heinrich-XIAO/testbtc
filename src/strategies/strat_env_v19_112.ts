...
export interface EnvV19StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV19StrategyParams = {
  ma_period: 3,
  envelope_pct: 0.008,
  stop_loss: 0.03,
  risk_percent: 0.08,
...
function loadSavedParams(): Partial<EnvV19StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_v19_112.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvV19StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV19StrategyParams] = value;
        }
      }
    }
...
export class EnvV19Strategy implements Strategy {
  params: EnvV19StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV19StrategyParams> = {}) {

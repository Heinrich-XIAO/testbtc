...
export interface StratIter147CParams extends StrategyParams {
  small_target: number;
  tight_stop: number;
  min_r_ratio: number;
...
const defaultParams: StratIter147CParams = {
  small_target: 0.05,
  tight_stop: 0.03,
  min_r_ratio: 1.5,
  stoch_oversold: 18,
...
function loadSavedParams(): Partial<StratIter147CParams> | null {
  const paramsPath = path.join(__dirname, 'strat_iter147_c.params.json');
...
export class StratIter147CStrategy implements Strategy {
  params: StratIter147CParams;
  ...
  constructor(params: Partial<StratIter147CParams> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as StratIter147CParams;
  }
  ...
  private calcRiskReward(support: number, resistance: number, price: number): { reward: number; risk: number; ratio: number } | null {
    const reward = (resistance - price) / price;
    const risk = (price - support) / price;
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (entry !== undefined && entryBarNum !== undefined) {
        ...
        if (bar.high >= entry * (1 + this.params.small_target)) {
          ctx.close(bar.tokenId);
          this.entryPrice.delete(bar.tokenId);
          this.entryBar.delete(bar.tokenId);
          return;
    ...
    const rr = this.calcRiskReward(sr.support, sr.resistance, bar.close);
    const nearSupport = bar.low <= sr.support * 1.01;
    const stochOversold = k !== null && k < this.params.stoch_oversold;
    const goodRR = rr !== null && rr.ratio >= this.params.min_r_ratio;

    if (nearSupport && stochOversold && goodRR) {

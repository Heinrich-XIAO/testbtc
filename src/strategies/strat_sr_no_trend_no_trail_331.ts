...
/**
 * SR No Trend No Trail 331
 * - Based on 302 but removes trailing stop (set to 0.99 - essentially disabled)
 * - Tests if trailing stop was limiting gains
 */

export interface SRNoTrendNoTrail331Params extends StrategyParams {
...
const defaultParams: SRNoTrendNoTrail331Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendNoTrail331Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_no_trail_331.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendNoTrail331Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendNoTrail331Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendNoTrail331Strategy implements Strategy {
  params: SRNoTrendNoTrail331Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendNoTrail331Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendNoTrail331Params;
  }

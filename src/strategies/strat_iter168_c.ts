...
export interface StratIter168CParams extends StrategyParams {
  rsi_period: number;
  rsi_oversold: number;
  rsi_recover: number;
  stop_loss: number;
...
export class StratIter168CStrategy implements Strategy {
  params: StratIter168CParams;
  private series: Map<string, TokenSeries> = new Map();
  private bars: Map<string, number> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private entryBar: Map<string, number> = new Map();

  constructor(params: Partial<StratIter168CParams> = {}) {
    const saved = loadSavedParams<StratIter168CParams>('strat_iter168_c.params.json');
    this.params = {
      rsi_period: 14,
      rsi_oversold: 20,
      rsi_recover: 40,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    // Entry: RSI was below oversold (20), now recovered above rsi_recover

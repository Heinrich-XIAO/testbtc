...
export interface StratIter168EParams extends StrategyParams {
  rsi_period: number;
  rsi_oversold: number;
  rsi_recover: number;
  stop_loss: number;
...
export class StratIter168EStrategy implements Strategy {
  params: StratIter168EParams;
  private series: Map<string, TokenSeries> = new Map();
  private bars: Map<string, number> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private entryBar: Map<string, number> = new Map();

  constructor(params: Partial<StratIter168EParams> = {}) {
    const saved = loadSavedParams<StratIter168EParams>('strat_iter168_e.params.json');
    this.params = {
      rsi_period: 14,
      rsi_oversold: 10,
      rsi_recover: 40,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    // Entry: RSI was below oversold (10), now recovered above rsi_recover

...
/**
 * SR Adaptive Multi Exit 234
 * - Combines sr_adaptive_225 with multi-exit from sr_multi_exit_226
 * - Adaptive lookback + profit target + time-based exit
 */

export interface SRAdaptiveMultiExit234Params extends StrategyParams {
...
const defaultParams: SRAdaptiveMultiExit234Params = {
  base_lookback: 20,
  min_lookback: 8,
  max_lookback: 35,
...
function loadSavedParams(): Partial<SRAdaptiveMultiExit234Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_adaptive_multi_exit_234.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRAdaptiveMultiExit234Params> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof SRAdaptiveMultiExit234Params] = value;
        }
      }
    }
...
export class SRAdaptiveMultiExit234Strategy implements Strategy {
  params: SRAdaptiveMultiExit234Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRAdaptiveMultiExit234Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRAdaptiveMultiExit234Params;
  }

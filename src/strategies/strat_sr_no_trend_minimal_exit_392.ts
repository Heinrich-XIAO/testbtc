...
/**
 * SR No Trend Minimal Exit 392
 * - Based on 362 but with minimal exit conditions
 * - Only exits on stop loss or resistance touch
 * - Removes profit_target, trailing_stop, max_hold_bars, and stoch overbought exits
 * - Lets winners run longer
 */

export interface SRNoTrendMinimalExit392Params extends StrategyParams {
...
const defaultParams: SRNoTrendMinimalExit392Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
...
function loadSavedParams(): Partial<SRNoTrendMinimalExit392Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_minimal_exit_392.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendMinimalExit392Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendMinimalExit392Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendMinimalExit392Strategy implements Strategy {
  params: SRNoTrendMinimalExit392Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendMinimalExit392Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendMinimalExit392Params;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      if (entry) {
        // Minimal exit: only stop loss or resistance touch

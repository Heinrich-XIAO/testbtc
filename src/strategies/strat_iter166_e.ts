...
export interface StratIter166EParams extends StrategyParams {
...
const defaultParams: StratIter166EParams = {
  max_consecutive_losses: 3,
  pause_after_losses: 75,
  stoch_oversold: 20,
  stoch_overbought: 80,
  sr_lookback_weeks: 8,
  stop_loss: 0.08,
...
function loadSavedParams(): Partial<StratIter166EParams> | null {
  const paramsPath = path.join(__dirname, 'strat_iter166_e.params.json');
...
export class StratIter166EStrategy implements Strategy {
  params: StratIter166EParams;
  ...
  constructor(params: Partial<StratIter166EParams> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as StratIter166EParams;
  }
  ...
  private getMarketState(tokenId: string): MarketState {
    if (!this.marketState.has(tokenId)) {
      this.marketState.set(tokenId, {
        consecutiveLosses: 0,
        pauseUntilBar: 0,
        totalTrades: 0,
        winningTrades: 0,
    ...
    return this.marketState.get(tokenId)!;
  }

  private canTrade(tokenId: string, currentBar: number): boolean {
    const state = this.getMarketState(tokenId);
    return currentBar >= state.pauseUntilBar;
  }

  private recordTradeResult(tokenId: string, isWin: boolean, currentBar: number): void {
    const state = this.getMarketState(tokenId);
    state.totalTrades++;
    ...
    if (isWin) {
      ...
      if (state.consecutiveLosses >= this.params.max_consecutive_losses) {
        state.pauseUntilBar = currentBar + this.params.pause_after_losses;
      }
    }
  }
    ...
    if (position && position.size > 0) {
      ...
      if (entry !== undefined && entryBarNum !== undefined) {
        ...
        if (bar.low <= entry * (1 - this.params.stop_loss)) {
          exited = true;
          isWin = false;
        } else if (bar.high >= entry * (1 + this.params.profit_target)) {
          exited = true;
          isWin = true;
        } else if (barNum - entryBarNum >= this.params.max_hold_bars) {
          exited = true;
          isWin = bar.close > entry;
        } else if (sr && bar.high >= sr.resistance) {
          exited = true;
          isWin = bar.close > entry;
        } else {
        ...
        if (exited) {
          ctx.close(tokenId);
          this.recordTradeResult(tokenId, isWin, barNum);
      ...
      if (!this.canTrade(tokenId, barNum)) return;

...
abstract class BaseIterStrategy<P extends StrategyParams> implements Strategy {
  ...
  protected nextBar(bar: Bar): { series: TokenSeries; barNum: number } {
    if (!this.series.has(bar.tokenId)) { this.series.set(bar.tokenId, { closes: [] }); this.bars.set(bar.tokenId, 0); }
...
export interface StratIter185BParams extends StrategyParams { rsi_period: number; rsi_oversold: number; stop_loss: number; profit_target: number; max_hold_bars: number; risk_percent: number; }
export class StratIter185BStrategy extends BaseIterStrategy<StratIter185BParams> {
  constructor(params: Partial<StratIter185BParams> = {}) { super('strat_iter185_b.params.json', { rsi_period: 21, rsi_oversold: 30, stop_loss: 0.08, profit_target: 0.15, max_hold_bars: 24, risk_percent: 0.25 }, params); }
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (shouldSkipPrice(bar.close) || rsiVal === null) return;
    if (rsiVal < this.params.rsi_oversold) { this.open(ctx, bar, barNum, this.params.risk_percent); }
  }
}

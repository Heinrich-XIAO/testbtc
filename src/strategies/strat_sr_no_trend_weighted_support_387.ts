...
/**
 * SR No Trend Weighted Support 387
 * - Based on 362 but with weighted support calculation
 * - Weight recent lows higher in support calculation
 */

export interface SRNoTrendWeightedSupport387Params extends StrategyParams {
  ...
  risk_percent: number;
  support_weight_recent: boolean;
}

const defaultParams: SRNoTrendWeightedSupport387Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 50,
  ...
  max_hold_bars: 28,
  risk_percent: 0.30,
  support_weight_recent: true,
};

function loadSavedParams(): Partial<SRNoTrendWeightedSupport387Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_weighted_support_387.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendWeightedSupport387Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number' || typeof value === 'boolean') {
          params[key as keyof SRNoTrendWeightedSupport387Params] = value as number & boolean;
        }
      }
    }
    return params;
...
export class SRNoTrendWeightedSupport387Strategy implements Strategy {
  params: SRNoTrendWeightedSupport387Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendWeightedSupport387Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendWeightedSupport387Params;
  }
  ...
  private findWeightedSupportLevel(data: TokenData, lookback: number): number | null {
    if (data.lows.length < lookback) return null;
    const recentLows = data.lows.slice(-lookback);
    const sortedWithIndex = recentLows
      .map((low, originalIndex) => ({ low, originalIndex }))
      .sort((a, b) => a.low - b.low);
    const lowest3 = sortedWithIndex.slice(0, 3);
    if (lowest3.length === 0) return null;

    const length = lowest3.length;
    let weightedSum = 0;
    let weightSum = 0;

    for (let i = 0; i < length; i++) {
      const weight = this.params.support_weight_recent
        ? (lowest3[i].originalIndex + 1) / lookback
        : (lookback - lowest3[i].originalIndex) / lookback;
      weightedSum += lowest3[i].low * weight;
      weightSum += weight;
    }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const support = this.findWeightedSupportLevel(data, adaptiveLookback);

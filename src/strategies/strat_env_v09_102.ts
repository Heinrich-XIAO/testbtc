...
export interface EnvV09StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV09StrategyParams = {
  ma_period: 12,
  envelope_pct: 0.04,
  stop_loss: 0.07,
  risk_percent: 0.15,
...
function loadSavedParams(): Partial<EnvV09StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_v09_102.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvV09StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV09StrategyParams] = value;
        }
      }
    }
...
export class EnvV09Strategy implements Strategy {
  params: EnvV09StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV09StrategyParams> = {}) {

...
export interface ChanV02StrategyParams extends StrategyParams {
...
const defaultParams: ChanV02StrategyParams = {
  channel_period: 5,
  channel_width: 0.4,
  stop_loss: 0.04,
  trailing_stop: 0.03,
...
function loadSavedParams(): Partial<ChanV02StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_chan_v02_115.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ChanV02StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ChanV02StrategyParams] = value;
        }
      }
    }
...
export class ChanV02Strategy implements Strategy {
  params: ChanV02StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<ChanV02StrategyParams> = {}) {

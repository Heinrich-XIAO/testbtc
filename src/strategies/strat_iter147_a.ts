...
export interface StratIter147AParams extends StrategyParams {
  micro_move_threshold: number;
  quick_profit: number;
  tight_stop: number;
...
const defaultParams: StratIter147AParams = {
  micro_move_threshold: 0.005,
  quick_profit: 0.06,
  tight_stop: 0.04,
  stoch_oversold: 18,
  max_hold_bars: 8,
...
function loadSavedParams(): Partial<StratIter147AParams> | null {
  const paramsPath = path.join(__dirname, 'strat_iter147_a.params.json');
...
export class StratIter147AStrategy implements Strategy {
  params: StratIter147AParams;
  ...
  constructor(params: Partial<StratIter147AParams> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as StratIter147AParams;
  }
  ...
  private detectMicroMove(closes: number[]): boolean {
    if (closes.length < 2) return false;
    const change = Math.abs(closes[closes.length - 1] - closes[closes.length - 2]) / closes[closes.length - 2];
    return change >= this.params.micro_move_threshold;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    const microMove = this.detectMicroMove(closes);
    const nearSupport = bar.low <= sr.support * 1.01;
    const stochOversold = k !== null && k < this.params.stoch_oversold;

    if (microMove && nearSupport && stochOversold) {

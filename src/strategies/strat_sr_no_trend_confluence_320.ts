...
/**
 * SR No Trend Confluence 320
 * - Based on 302 but adds PRICE CONFLUENCE requirement
 * - Requires stronger alignment: support + stochastic + momentum all align
 * - Stricter bounce: requires 2 consecutive up bars (vs 1 in 302)
 * - Confluence check: price pattern must support the signal
 */

export interface SRNoTrendConfluence320Params extends StrategyParams {
  ...
  confluence_lookback: number;
  min_higher_lows: number;
  stop_loss: number;
...
const defaultParams: SRNoTrendConfluence320Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendConfluence320Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_confluence_320.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendConfluence320Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendConfluence320Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendConfluence320Strategy implements Strategy {
  params: SRNoTrendConfluence320Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendConfluence320Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendConfluence320Params;
  }
  ...
  private checkPriceConfluence(data: TokenData, currentLow: number): boolean {
    if (data.lows.length < this.params.confluence_lookback) return false;
    
    const recentLows = data.lows.slice(-this.params.confluence_lookback);
    ...
    return higherLowsCount >= this.params.min_higher_lows;
  }
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      const priceConfluence = this.checkPriceConfluence(data, bar.low);

      if (nearSupport && multiBarBounce && stochOversold && momentumOk && priceConfluence) {

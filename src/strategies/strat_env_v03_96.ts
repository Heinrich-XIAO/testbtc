...
export interface EnvV03StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV03StrategyParams = {
  ma_period: 5,
  envelope_pct: 0.025,
  stop_loss: 0.05,
  risk_percent: 0.12,
...
function loadSavedParams(): Partial<EnvV03StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_v03_96.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvV03StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV03StrategyParams] = value;
        }
      }
    }
...
export class EnvV03Strategy implements Strategy {
  params: EnvV03StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV03StrategyParams> = {}) {

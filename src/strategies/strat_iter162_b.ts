...
export interface StratIter162BParams extends StrategyParams {
  sr_lookback: number;
  macd_fast: number;
  macd_slow: number;
  macd_signal: number;
...
export class StratIter162BStrategy extends BaseIterStrategy<StratIter162BParams> {
  ...
  constructor(params: Partial<StratIter162BParams> = {}) {
    super('strat_iter162_b.params.json', {
      sr_lookback: 50,
      macd_fast: 12,
      macd_slow: 26,
      macd_signal: 9,
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (!sr || shouldSkipPrice(bar.close) || mh.length < 2 || kv.length < 2) return;

    const prevHist = mh[mh.length - 2];
    const currHist = mh[mh.length - 1];
    const histTurnsPositive = prevHist < 0 && currHist >= 0;

    const nearSupport = bar.low <= sr.support * 1.015;
    const stochRecover = kv[kv.length - 2] < this.params.stoch_oversold && kv[kv.length - 1] >= this.params.stoch_oversold;

    if (histTurnsPositive && nearSupport && stochRecover) {

...
/**
 * Wide MA envelope for larger moves
 */

export interface EnvWideStrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvWideStrategyParams = {
  ma_period: 12,
  envelope_pct: 0.05,
  stop_loss: 0.1,
  risk_percent: 0.15,
...
function loadSavedParams(): Partial<EnvWideStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_wide_39.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvWideStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvWideStrategyParams] = value;
        }
      }
    }
...
export class EnvWideStrategy implements Strategy {
  params: EnvWideStrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvWideStrategyParams> = {}) {

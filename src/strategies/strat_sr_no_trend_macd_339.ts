...
/**
 * SR No Trend MACD 339
 * - Based on 302 (SR No Trend Filter)
 * - Adds MACD(12,26,9) filter: only enter when MACD < signal (oversold momentum)
 */

export interface SRNoTrendMACD339Params extends StrategyParams {
...
const defaultParams: SRNoTrendMACD339Params = {
  base_lookback: 18,
  min_lookback: 10,
  max_lookback: 36,
...
function loadSavedParams(): Partial<SRNoTrendMACD339Params> | null {
  const paramsPath = path.join(__dirname, 'strat_sr_no_trend_macd_339.params.json');
  if (!fs.existsSync(paramsPath)) return null;
  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<SRNoTrendMACD339Params> = {};
    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') params[key as keyof SRNoTrendMACD339Params] = value;
      }
    }
    return params;
  } catch { return null; }
...
export class SRNoTrendMACD339Strategy implements Strategy {
  params: SRNoTrendMACD339Params;
  private tokenData: Map<string, TokenData> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<SRNoTrendMACD339Params> = {}) {
    const savedParams = loadSavedParams();
    this.params = { ...defaultParams, ...savedParams, ...params } as SRNoTrendMACD339Params;
  }
  ...
  private calculateMACD(data: TokenData): { macd: number; signal: number } | null {
    const fast = this.params.macd_fast;
    const slow = this.params.macd_slow;
    const signal = this.params.macd_signal;
    
    if (data.prices.length < slow) return null;
    
    const price = data.prices[data.prices.length - 1];
    
    const calcEMA = (values: number[], period: number): number => {
      if (values.length < period) return price;
      const k = 2 / (period + 1);
    ...
    const emaFast = calcEMA(allPrices, fast);
    const emaSlow = calcEMA(allPrices, slow);
    const macd = emaFast - emaSlow;
    
    data.emaFast.push(emaFast);
    data.emaSlow.push(emaSlow);
    if (data.emaFast.length > 100) data.emaFast.shift();
    if (data.emaSlow.length > 100) data.emaSlow.shift();
    
    data.macdLine.push(macd);
    if (data.macdLine.length > 100) data.macdLine.shift();
    
    if (data.macdLine.length < signal) return null;
    
    const k = 2 / (signal + 1);
    let sig = data.macdLine.slice(0, signal).reduce((a, b) => a + b, 0) / signal;
    for (let i = signal; i < data.macdLine.length; i++) {
      sig = data.macdLine[i] * k + sig * (1 - k);
    }
    
    data.signalLine.push(sig);
    if (data.signalLine.length > 100) data.signalLine.shift();
  ...
  onNext(ctx: BacktestContext, bar: Bar): void {
    ...
    if (position && position.size > 0) {
      ...
      const macdOk = macd !== null && macd.macd < macd.signal;

      if (nearSupport && multiBarBounce && stochOversold && momentumOk && macdOk) {

...
export interface EnvV12StrategyParams extends StrategyParams {
  ma_period: number;
  envelope_pct: number;
  stop_loss: number;
  risk_percent: number;
...
const defaultParams: EnvV12StrategyParams = {
  ma_period: 15,
  envelope_pct: 0.05,
  stop_loss: 0.04,
  risk_percent: 0.05,
...
function loadSavedParams(): Partial<EnvV12StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_env_v12_105.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<EnvV12StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof EnvV12StrategyParams] = value;
        }
      }
    }
...
export class EnvV12Strategy implements Strategy {
  params: EnvV12StrategyParams;
  private smaMap: Map<string, SimpleMovingAverage> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<EnvV12StrategyParams> = {}) {

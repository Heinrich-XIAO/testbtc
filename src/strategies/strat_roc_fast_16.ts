...
/**
 * Fast rate of change momentum
 */

export interface ROCFastStrategyParams extends StrategyParams {
  lookback: number;
...
const defaultParams: ROCFastStrategyParams = {
  lookback: 3,
...
function loadSavedParams(): Partial<ROCFastStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_roc_fast_16.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<ROCFastStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof ROCFastStrategyParams] = value;
        }
      }
    }
...
export class ROCFastStrategy implements Strategy {
  params: ROCFastStrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();
  private barsHeld: Map<string, number> = new Map();

  constructor(params: Partial<ROCFastStrategyParams> = {}) {

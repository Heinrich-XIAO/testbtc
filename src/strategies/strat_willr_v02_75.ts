...
export interface WillRV02StrategyParams extends StrategyParams {
  period: number;
...
const defaultParams: WillRV02StrategyParams = {
  period: 5,
  oversold_level: -80,
  overbought_level: -20,
  stop_loss: 0.04,
...
function loadSavedParams(): Partial<WillRV02StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_willr_v02_75.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<WillRV02StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof WillRV02StrategyParams] = value;
        }
      }
    }
...
export class WillRV02Strategy implements Strategy {
  params: WillRV02StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<WillRV02StrategyParams> = {}) {

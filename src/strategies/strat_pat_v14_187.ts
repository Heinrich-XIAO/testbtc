...
export interface PatV14StrategyParams extends StrategyParams {
  consec_bars: number;
  exit_bars: number;
  stop_loss: number;
...
const defaultParams: PatV14StrategyParams = {
  consec_bars: 3,
  exit_bars: 1,
  stop_loss: 0.06,
  trailing_stop: 0.04,
...
function loadSavedParams(): Partial<PatV14StrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_pat_v14_187.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<PatV14StrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof PatV14StrategyParams] = value;
        }
      }
    }
...
export class PatV14Strategy implements Strategy {
  params: PatV14StrategyParams;
  private priceHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();
  private highestPrice: Map<string, number> = new Map();

  constructor(params: Partial<PatV14StrategyParams> = {}) {

...
/**
 * Slow RSI divergence for longer trends
 */

export interface RSIDivSlowStrategyParams extends StrategyParams {
  rsi_period: number;
...
const defaultParams: RSIDivSlowStrategyParams = {
  rsi_period: 8,
...
function loadSavedParams(): Partial<RSIDivSlowStrategyParams> | null {
  const paramsPath = path.join(__dirname, 'strat_rsi_div_slow_31.params.json');
  if (!fs.existsSync(paramsPath)) return null;

  try {
    const content = fs.readFileSync(paramsPath, 'utf-8');
    const saved = JSON.parse(content);
    const params: Partial<RSIDivSlowStrategyParams> = {};

    for (const [key, value] of Object.entries(saved)) {
      if (key !== 'metadata' && key in defaultParams) {
        if (typeof value === 'number') {
          params[key as keyof RSIDivSlowStrategyParams] = value;
        }
      }
    }
...
export class RSIDivSlowStrategy implements Strategy {
  params: RSIDivSlowStrategyParams;
  private rsiMap: Map<string, RSI> = new Map();
  private priceHistory: Map<string, number[]> = new Map();
  private rsiHistory: Map<string, number[]> = new Map();
  private entryPrice: Map<string, number> = new Map();

  constructor(params: Partial<RSIDivSlowStrategyParams> = {}) {
